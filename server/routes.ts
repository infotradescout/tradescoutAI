Warning: truncated output (original token count: 206329)
Total output lines: 22000
  ...
  app.post("/api/auth/setup-profile", isAuthenticated, async (req: any, res: any) => {
    try {
      ...
      const createdProfile = await storage.createProfileForOwner(userId, {
        ...
        roleCon…6329 tokens truncated…onst dbRole = typeof (user as any)?.role === "string" ? String((user as any).role) : "";
      const dbActiveRole =
