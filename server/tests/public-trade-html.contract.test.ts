...
describe("public trade SEO failure contracts", () => {
  it("trade county html propagates listing failures instead of serving an empty success", () => {
    const source = read("server/publicTradeHtml.ts");
    expect(source).not.toContain("serving fallback page without listings");
    expect(source).toContain("const rows = await db");
  });
  ...
  it("keeps crawlable county failures strict while the noindex best page handles column drift", () => {
    ...
    expect(countySource).not.toContain("isMissingColumnError");
