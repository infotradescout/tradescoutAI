...
describe("public profile image sitemap audit scheduling", () => {
  it("runs the corrected audit once in production after the feeds are available", () => {
    ...
    expect(service).toContain('import("./publicProfileImageSitemapAuditV2")');
    expect(service).toContain("runPublicProfileImageSitemapAuditV2");
  ...
  it("proves deployed XML without turning a sitemap into ranking evidence", () => {
    ...
    const corrected = read("server/services/publicProfileImageSitemapAuditV2.ts");
    ...
    expect(corrected).toContain("emitted image:loc references");
    expect(corrected).toContain("unique normalized page and image URLs");
    expect(corrected).toContain("It is not proof of indexing, ranking, image visibility");
  });
});
