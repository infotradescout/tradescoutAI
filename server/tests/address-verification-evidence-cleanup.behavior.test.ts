...
async function harness(scenario: Scenario = {}) {
            ...
            return Promise.resolve()
              .then(() => {
                ...
                assert.deepEqual(condition, {
                  left: addressVerifications.documentUrl,
                  right: state.key,
                });
  ...
  function mutation() {
    const query: any = {
      values(value: { documentUrl: string }) {
        state.key = value.documentUrl;
        return query;
      },
      set(value: { documentUrl: string }) {
        state.key = value.documentUrl;
        return query;
      },
      where() {
  ...
  const mocks: Record<string, any> = {
    ...
    "./businessVerificationWorkflow": {
      isOwnedPrivateObjectKey: (key: string, id: string) => key === sourceKey && id === owner,
    },
    "../services/businessVerificationWorkflow": {
      isOwnedPrivateObjectKey: (key: string, id: string) => key === sourceKey && id === owner,
    },
    "../localStorage": {
      LocalStorageService: class {
        async getPrivateFilePathFromObjectKey() {
          return sourcePath;
        }
      },
    ...
    "../r2Client": {
      ...
      createR2Client: () => ({
        async send(command: any, options?: any) {
          ...
          if (command.kind === "delete" && scenario.cleanupFailure === "delete")
            throw cleanupError();
          return {};
        },
      }),
  ...
  if (scenario.cleanupFailure === "delete" && !scenario.r2) {
    mocks["node:fs/promises"] = {
      ...nativeRequire("node:fs/promises"),
      unlink: async () => {
        throw cleanupError();
      },
    };
  ...
  function load(relativePath: string): any {
    ...
    vm.runInNewContext(
      compiled.outputText,
      {
        ...
        process: {
          env: scenario.r2
            ? { R2_BUCKET_NAME: "isolated-test-bucket", R2_ACCESS_KEY_ID: "test-only" }
            : {},
        },
        require: (id: string) => {
  ...
  for (const method of ["post", "put", "get"]) {
    app[method] = (url: string, ...handlers: any[]) =>
      registered.set(`${method.toUpperCase()} ${url}`, handlers.at(-1));
  }
  ...
  return {
    ...
    oldPath,
    copyExists: () =>
      existsSync(path.join(root, "address-evidence", owner, state.key.split("/").at(-1) || "none")),
    ...
    async invoke() {
      ...
      await registered.get(`${method} ${url}`)!(
        {
          user: { id: owner },
          params: { id: "verification-a" },
          body: {
            documentUrl: sourceKey,
...
describe("address evidence lifecycle through registered submission routes", () => {
  for (const method of ["POST", "PUT"] as const) {
    it(`${method}: removes only the new copy after a rolled-back write`, () =>
      withHarness({ method, writeFailure: true }, async (h) => {
        ...
        assert.equal(
          JSON.stringify(h.state.cleanupOptions),
          JSON.stringify({ isolationLevel: "read committed" })
        );
    ...
    it(`${method}: keeps committed evidence without starting cleanup`, () =>
      withHarness({ method }, async (h) => {
        const response = await h.invoke();
    ...
    it(`${method}: keeps evidence when the commit succeeded but its response was lost`, () =>
      withHarness({ method, commitResponseLost: true }, async (h) => {
        assert.equal((await h.invoke()).statusCode, 400);
        assert.equal(h.copyExists(), true);
        assert.ok(h.events.includes("cleanup-reference-read"));
      }));
    it(`${method}: observes references committed while the cleanup lock waited`, () =>
      withHarness({ method, writeFailure: true, referenceAfterLock: true }, async (h) => {
        await h.invoke();
        assert.equal(h.copyExists(), true);
        assert.equal(
          JSON.stringify(h.state.cleanupOptions),
    ...
    for (const cleanupFailure of ["transaction", "lock", "read", "delete"] as const) {
      it(`${method}: retains evidence when cleanup ${cleanupFailure} fails`, () =>
        withHarness({ method, writeFailure: true, cleanupFailure }, async (h) => {
          assert.equal((await h.invoke()).statusCode, 400);
          assert.equal(h.copyExists(), true);
          assert.deepEqual(h.warnings, [["Address verification evidence cleanup deferred"]]);
        }));
    }
    it(`${method}: retains evidence when the account lock cannot establish ownership`, () =>
      withHarness({ method, writeFailure: true, cleanupAccountMissing: true }, async (h) => {
        await h.invoke();
        assert.equal(h.copyExists(), true);
  ...
  it("preserves the exact original error when cleanup also fails", () =>
    withHarness({ cleanupFailure: "transaction" }, async (h) => {
      const original = new Error("original rejection");
      await assert.rejects(
        h.lifecycle().withAddressEvidenceTransaction(owner, async (_tx: unknown, snapshot: any) => {
  ...
  it("does not clean up a source read that failed before creating evidence", () =>
    withHarness({}, async (h) => {
  ...
  it("local disposal is idempotent and cannot target a member upload or another owner", () =>
    withHarness({}, async (h) => {
      ...
      await assert.rejects(
        evidence.discardAddressVerificationEvidence(
          "private/address-evidence/user-a/../original.pdf",
          owner
        )
      );
      assert.equal(existsSync(h.sourcePath), true);
  ...
  it("R2 rollback deletes only the generated copy with a bounded abort signal", () =>
    withHarness({ r2: true, writeFailure: true }, async (h) => {
      assert.equal((await h.invoke()).statusCode, 400);
  ...
  it("R2 preserves referenced evidence after a lost commit response", () =>
    withHarness({ r2: true, commitResponseLost: true }, async (h) => {
      await h.invoke();
      assert.equal(
        h.r2Commands.some((command) => command.kind === "delete"),
        false
      );
    }));

  it("R2 deletion failures do not replace the submission response or expose private diagnostics", () =>
    withHarness({ r2: true, writeFailure: true, cleanupFailure: "delete" }, async (h) => {
      assert.equal((await h.invoke()).statusCode, 400);
      assert.deepEqual(h.warnings, [["Address verification evidence cleanup deferred"]]);
    }));
});
