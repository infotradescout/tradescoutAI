...
describe("scoutDeterministicIntent", () => {
  describe("deriveDeterministicIntent", () => {
    it("detects send invoice intents", () => {
      expect(deriveDeterministicIntent("Can you send the invoice for this job?"))
        .toBe("send_invoice");
      expect(deriveDeterministicIntent("invoice ready, please send"))
        .toBe("send_invoice");
    });

    it("detects mark invoice paid intents", () => {
      expect(deriveDeterministicIntent("Mark this invoice paid"))
        .toBe("mark_invoice_paid");
      expect(deriveDeterministicIntent("payment received, mark it paid"))
        .toBe("mark_invoice_paid");
    });

    it("detects contract intents", () => {
      expect(deriveDeterministicIntent("Send the contract for this project"))
        .toBe("send_contract");
      expect(deriveDeterministicIntent("I need to sign the contract"))
        .toBe("sign_contract");
    });

    it("detects open deal room intents", () => {
      expect(deriveDeterministicIntent("Open the deal room"))
        .toBe("open_deal_room");
      expect(deriveDeterministicIntent("open the project tracker"))
        .toBe("open_deal_room");
    });

    it("returns null when no deterministic intent matches", () => {
      expect(deriveDeterministicIntent("How's the weather?"))
        .toBeNull();
    });
  });
  ...
  describe("maybeHandleDeterministicIntent", () => {
    ...
    it("returns deterministic navigation + metadata when allowed", () => {
      ...
      expect(result.actions[0].path).toBe("/lead-management?jobId=job-123");
