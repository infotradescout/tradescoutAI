...
describe("provider search state storage contracts", () => {
  it("scopes contractors through contractor county assignments and canonical counties", () => {
    const contractorRepository = read("server/storage/repositories/social-and-leaderboards.ts");
    const start = contractorRepository.indexOf("async getContractors(");
    const method = contractorRepository.slice(start, start + 5500);
  ...
  it("casts the verification enum before applying the public-business fallback", () => {
    const publication = read("server/publicationBusiness.ts");

    expect(publication).toContain("COALESCE(CAST(${users.verificationStatus} AS text), '')");
    expect(publication).not.toContain("COALESCE(${users.verificationStatus}, '')");
  });
});
...
describe("provider search public/contact boundary", () => {
  ...
  it("requires canonical public-profile authority before returning a contractor", () => {
    ...
    expect(providerRoute).toContain("canonicalProfileUrlByUserId.get(");
    expect(providerRoute).toContain("canonicalBusinessProfileUrl,");
