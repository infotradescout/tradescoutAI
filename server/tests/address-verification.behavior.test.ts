...
const lifecycleSource = ts.createSourceFile(
  "addressVerificationEvidenceTransaction.ts",
  fs.readFileSync("server/services/addressVerificationEvidenceTransaction.ts", "utf8"),
  ts.ScriptTarget.Latest,
  true
);
const lifecycleFunction = lifecycleSource.statements.find(
  (statement): statement is ts.FunctionDeclaration =>
    ts.isFunctionDeclaration(statement) && statement.name?.text === "withAddressEvidenceTransaction"
);
if (!lifecycleFunction) throw new Error("Missing canonical address evidence transaction owner");
const compiledLifecycle = ts.transpileModule(
...
function harness(initial: any[] = [], verified = false) {
  ...
  const lifecycleDependencies = {
    ...
    discardAddressVerificationEvidence: vi.fn(async () => undefined),
  };
  const withAddressEvidenceTransaction = new Function(
