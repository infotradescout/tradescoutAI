...
describe("JW Stone private pricing HTTP path", () => {
  ...
  const review = (body: unknown, viewer = "member") =>
    request(app).post(JW_STONE_CART_REVIEW_PATH).set("x-fixture-viewer", viewer).send(body);
  beforeAll(async () => {
    await database.exec(`
  ...
  beforeEach(async () => {
    ...
    await database.exec(
      "UPDATE profile_accounts SET status='active', identity_kind='business'; UPDATE profile_account_entitlements SET status='pending_verification'; UPDATE user_profiles SET verification_status='pending'; UPDATE profiles SET slug='jw-stone'; DELETE FROM stone_inventory_positions; DELETE FROM stone_asset_passports;"
    );
    await database.query(
      "INSERT INTO stone_asset_passports VALUES ('fixture-passport',$1,'verified',$2::jsonb)",
      [
    ...
    await database.exec(
      "INSERT INTO stone_inventory_positions VALUES ('private-position-id','fixture-passport','jw-business',3,0,'slabs','available','published_current',NOW(),'{\"actor\":\"fixture-owner\"}')"
    );
  });
  afterAll(async () => {
    vi.unstubAllEnvs();
  ...
  it("rejects client prices, totals, private IDs, and combined quantities above the bound", async () => {
    ...
    expect((await review({ lines: [{ ...bodyFor(1).lines[0], heldQuantity: 0 }] })).status).toBe(
      400
    );
  });
  ...
  it("prices only the remaining slab when other slabs are already held", async () => {
    ...
    expect(() => parseJwStoneCartReview(response.body, "member", bodyFor(1))).not.toThrow();
    expect(JSON.stringify(response.body)).not.toMatch(
      /heldQuantity|held_quantity|position_id|private-position|allocation|buyer_user/i
    );
  });
  ...
  it.each([null, "NaN", "Infinity", "-1", "0.5", "4"])(
    "does not invent availability from an invalid held counter: %s",
    async (held) => {
      await database.query("UPDATE stone_inventory_positions SET held_quantity=$1::numeric", [
        held,
      ]);
      const response = await review(bodyFor(1));
      expect(response.status).toBe(200);
  ...
  it.each([
    ...
    "denies changed or unprovable current stock rather than using a stale seller snapshot: %s",
    async (sql) => {
      await database.exec(sql);
      const response = await review(bodyFor(1));
      expect(response.status).toBe(200);
  ...
  it("rejects ambiguous public-stock positions for the same seller", async () => {
    await database.exec(
      "INSERT INTO stone_inventory_positions SELECT 'duplicate-position',asset_passport_id,holder_business_id,quantity,held_quantity,unit,lifecycle_status,public_availability_status,published_at,publication_evidence FROM stone_inventory_positions"
    );
    const response = await review(bodyFor(1));
    expect(response.body.lines[0].status).toBe("unavailable");
    expect(response.body.subtotalCents).toBeNull();
  ...
  it("rechecks released stock without changing any allocation or creating an order", async () => {
    ...
    expect(
      (await database.query("SELECT held_quantity FROM stone_inventory_positions")).rows[0]
        .held_quantity
    ).toBe("2");
  ...
  it("does not fall back to full stock when the allocation query fails", async () => {
    databaseBridge.query.mockImplementation((sql, args) => {
      if (String(sql).includes("position.held_quantity"))
        throw new Error("Private allocation connection failed");
    ...
    try {
      ...
      expect(response.body.subtotalCents).toBeUndefined();
...
vi.mock("../routes/jw-stone-features", () => ({ registerJwStoneFeatureRoutes: vi.fn() }));
