...
describeDb("Phase 2B ingress hardening", () => {
  it(
    "marketplace create rejects duplicate seller canonical listing",
    async () => {
  ...
  it(
    "marketplace update ignores protected fields and scrubs contact-bearing text",
    async () => {
      const { agent, user } = await createAuthedAgent({
        role: "homeowner",
        addressVerified: true,
  ...
  it(
    "marketplace create enforces exchange category requirements server-side",
    async () => {
      const { agent, user } = await createAuthedAgent({
        role: "homeowner",
        addressVerified: true,
  ...
  it(
    "homescout manual create rejects duplicate canonical property record",
    async () => {
  ...
  it(
    "homescout create scrubs contact-bearing text before persistence",
    async () => {
      const { agent } = await createAuthedAgent({ role: "homeowner", addressVerified: true });
      const county = await getCountyFixture();
  ...
  it(
    "homescout update scrubs contact-bearing text while preserving canonical ownership",
    async () => {
