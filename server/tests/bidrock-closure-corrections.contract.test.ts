...
describe("BidRock closure migration and readiness contracts", () => {
  it("registers the ordered recovery migrations in the journal used by db:migrate", () => {
    ...
    expect(migrateScript).toContain("await runVerifiedMigration({");
    expect(migrateScript).toContain(
      'runCommand("npx", ["drizzle-kit", "migrate", `--config=${config}`]'
    );
    expect(migrateScript).toContain("runCommand(process.execPath, [requiredSchemaEntrypoint()]");
