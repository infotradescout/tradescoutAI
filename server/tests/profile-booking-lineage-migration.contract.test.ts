...
describe("profile booking exact-Profile lineage migration", () => {
  it("adds explicit deletion-safe lineage and canonical release authority", () => {
    const migration = read("migrations/0132_profile_booking_request_profile_lineage.sql");
  ...
  it("registers booking lineage after the released migration history", () => {
    const journal = JSON.parse(read("migrations/meta/_journal.json"));
    expect(journal.entries).toContainEqual(
      expect.objectContaining({
        idx: 135,
        tag: "0132_profile_booking_request_profile_lineage",
        breakpoints: false,
      })
    );
  });
