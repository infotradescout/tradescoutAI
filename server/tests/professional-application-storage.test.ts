...
describe("professional application transactional storage", () => {
  ...
  it("fails migration preflight on legacy duplicates instead of deleting data", () => {
    const migration = fs.readFileSync(
      path.resolve(process.cwd(), "migrations/0133_professional_application_integrity.sql"),
      "utf8"
    );
  ...
  it("returns the existing application when Drizzle wraps the native unique-user violation", async () => {
    const existing = {
      id: "realtor-existing",
      ...realtorApplication(),
      verificationStatus: "pending",
    ...
    const fake = createFakeProfessionalDatabase({ realtors: [existing] });
    const nativeError = Object.assign(new Error("duplicate user"), {
      code: "23505",
      constraint: "uq_realtor_profiles_user_id",
    });
    fake.database.transaction = async () => {
      throw new DrizzleQueryError("insert into realtor_profiles", [], nativeError);
    };

    const result = await createProfessionalApplicationPersistence(
      fake.database
    ).submitRealtorApplication(realtorApplication());

    expect(result).toEqual({ outcome: "duplicate", profile: existing });
    expect(fake.state.realtors).toEqual([existing]);
    expect(fake.state.events).toEqual([]);
  });

  it("does not hide an unrelated wrapped unique constraint behind an existing application", async () => {
    const existing = {
      id: "realtor-existing",
      ...realtorApplication(),
      verificationStatus: "pending",
    ...
    const fake = createFakeProfessionalDatabase({ realtors: [existing] });
    const nativeError = Object.assign(new Error("duplicate audit id"), {
      code: "23505",
      constraint: "events_pkey",
    });
    const wrapped = new DrizzleQueryError("insert into events", [], nativeError);
    fake.database.transaction = async () => {
      throw wrapped;
    };

    await expect(
      createProfessionalApplicationPersistence(fake.database).submitRealtorApplication(
        realtorApplication()
      )
  ...
  it("propagates cyclic non-database errors without looping", async () => {
    const fake = createFakeProfessionalDatabase();
    const error = new Error("unrelated failure") as Error & { cause: unknown };
    error.cause = error;
    fake.database.transaction = async () => {
      throw error;
    };
