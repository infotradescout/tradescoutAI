...
describe("public Express Direct Connect phone gate", () => {
  ...
  it("limits legacy call release to registered business contact and never selects private owner phone", () => {
    const route = read("server/routes/tradepartner-express.ts");

    expect(route).toContain("isRegisteredDirectProfileSlug(req.params.slug)");
    expect(route).toMatch(
      /isRegisteredDirectProfileSlug\(normalizedSlug\)\s*&&\s*isOwnerConfirmedDirectProfile\(directProfileCandidate\)/
    );
  ...
  it("keeps ordinary public calls in the request form and gates registered business calls", () => {
    ...
    expect(panel).toContain("allowCall && isRegisteredDirectProfileSlug(profileSlug)");
    expect(panel).toContain("if (!canRevealRegisteredBusinessPhone || busy) return;");
    expect(panel).toContain("controller.signal.aborted");
