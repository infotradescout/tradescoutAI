  ...
  lines: [
    {
      ...
      status: "ready",
      landedCostCents: 1,
      privateSourceId: "test-private-field",
    },
  ],
});
const send = (overrides: Partial<Parameters<typeof reviewJwStoneOffer>[0]> = {}) =>
  reviewJwStoneOffer({
    profileSlug: "jw-stone",
    viewerId: "member",
    user: { id: "member" },
...
beforeEach(() => {
  vi.clearAllMocks();
  mocks.access.mockResolvedValue("member");
  mocks.review.mockResolvedValue(snapshot());
});
describe("JW Stone offer amount and strict input", () => {
  it.each([
    ...
    expect(parseJwStoneOfferDollars(String(value))).toBe(cents)
  );
  it.each([
    "",
    "0",
    ...
    "999999999999999999",
    "1.2.3",
  ])("rejects invalid amount %s", (value) => expect(parseJwStoneOfferDollars(value)).toBeNull());
  it.each([
    { paymentAllowed: true },
    { status: "confirmed" },
    { confirmedAt: "2026-09-16" },
    { finalPayableTotalCents: 1 },
    { offeredTotalCents: 0 },
    { offeredTotalCents: 1.5 },
    { termsAcknowledged: false },
  ])("rejects invalid client-supplied input %j", (override) =>
    expect(jwStoneOfferInputSchema.safeParse({ ...input, ...override }).success).toBe(false)
  );
  it("does not accept multiple selections as a single-stone offer", () =>
    expect(
      jwStoneOfferInputSchema.safeParse({
        ...input,
        selection: {
          lines: [
            ...selection.lines,
            { inventoryPublicId: "stone_" + "b".repeat(32), quantity: 1 },
          ],
        },
      }).success
    ).toBe(false));
...
describe("JW Stone authoritative pending-offer intake", () => {
  it("records listed and proposed totals without enabling payment or reserving stock", async () => {
    ...
    expect(summarizeJwStoneOffer(offer)).toContain(
      "No payment is accepted until JW Stone confirms"
    );
  });
  it.each(["none", "internal"])("rejects %s access before reading prices", async (access) => {
    mocks.access.mockResolvedValue(access);
    await expect(send()).rejects.toMatchObject({ status: 403 });
    expect(mocks.review).not.toHaveBeenCalled();
  });
  it("requires sign-in", async () => {
    await expect(send({ viewerId: "" })).rejects.toMatchObject({ status: 401 });
    expect(mocks.access).not.toHaveBeenCalled();
  });
  it("cannot submit an offer to another business", async () => {
    await expect(send({ profileSlug: "another-business" })).rejects.toMatchObject({ status: 400 });
    expect(mocks.review).not.toHaveBeenCalled();
  });
  it("rejects a changed displayed total", async () => {
    await expect(send({ input: { ...input, expectedSubtotalCents: 14999 } })).rejects.toMatchObject(
      { code: "OFFER_PRICE_CHANGED", status: 409 }
    );
  });
  it("rejects unavailable stock", async () => {
    mocks.review.mockResolvedValue({
      ...
      materialReady: false,
      subtotalCents: null,
      lines: [{ inventoryPublicId: stockId, requestedQuantity: 1, status: "unavailable" }],
    });
    await expect(send()).rejects.toMatchObject({ code: "OFFER_STOCK_CHANGED" });
  });
  it("fails closed when pricing cannot respond", async () => {
    mocks.review.mockRejectedValue(new Error("Source unavailable"));
    await expect(send()).rejects.toThrow("Source unavailable");
  });
  it("keeps a full-cart offer pending even when it equals the listed total", async () => {
    expect(
      await send({ input: { ...input, scope: "cart", offeredTotalCents: 15000 } })
