...
describeDb("Phase 2C privileged path hardening", () => {
  ...
  it("assistant override create requires reason and emits denied privileged audit", async () => {
  ...
  it("assistant override delete emits completed privileged audit with explicit reason", async () => {
    const adminUser = await createUserOnly({ role: "admin" as any });
  ...
  it("assistant knowledge upsert requires reason and emits denied privileged audit", async () => {
    const adminUser = await createUserOnly({ role: "admin" as any });
  ...
  it("legacy admin reset-password rejects email-only privileged targeting", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const targetUser = await createUserOnly({ role: "contractor" });
  ...
  it("legacy admin reset-password requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const targetUser = await createUserOnly({ role: "contractor" });
  ...
  it("legacy admin role update now requires explicit reason", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const targetUser = await createUserOnly({ role: "contractor" });
  ...
  it("admin module roles patch requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const targetUser = await createUserOnly({ role: "contractor" });
  ...
  it("admin module badges patch requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const targetUser = await createUserOnly({ role: "contractor" });

    const missingReason = await adminAgent.patch(`/api/admin/users/${targetUser.id}/badges`).send({
  ...
  it("admin community post delete requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const county = await getCountyFixture();
    ...
    const missingReason = await adminAgent.delete(`/api/admin/community/posts/${post.id}`).send({});
    expect(missingReason.status).toBe(400);
  ...
  it("affiliate commission update requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
  ...
  it("affiliate payout create requires reason and emits completed privileged audit", async () => {
    const { agent: adminAgent } = await createAuthedAgent({ role: "super_admin" });
    const affiliateUser = await createUserOnly({ role: "contractor" });
    const [affiliate] = await db
      .insert(affiliateAccounts)
