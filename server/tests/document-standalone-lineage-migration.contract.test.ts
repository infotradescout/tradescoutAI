...
describe("document accounting-lineage backfill migration", () => {
  it("atomically classifies standalone and profile-offer acct groups without merging them", () => {
    const migration = read("migrations/0134_document_standalone_lineage_backfill.sql");
  ...
  it("registers document lineage after the professional integrity migration", () => {
    const journal = JSON.parse(read("migrations/meta/_journal.json"));
    expect(journal.entries).toContainEqual(
      expect.objectContaining({
        idx: 137,
        tag: "0134_document_standalone_lineage_backfill",
        breakpoints: false,
      })
    );
  });
