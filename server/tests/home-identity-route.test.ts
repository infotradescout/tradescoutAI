...
function app(authenticated = true) {
  ...
  instance.use((req: any, _res, next) => {
    if (authenticated) req.user = { id: "session-owner" };
    next();
  });
  instance.use(createHomeIdentityRouter({ load, save }));
  return instance;
}
...
describe("canonical property edit transport", () => {
  ...
  it("gets authority from the session, never a body or query owner", async () => {
    ...
    const payload = {
      ...
      changes: { nickname: "Renamed" },
      ownerUserId: "body-attacker",
    };
    const response = await request(app())
      .patch("/api/homes/property/identity?userId=query-attacker")
      .send(payload)
      .expect(200);
  ...
  it.each([
    [404, "HOME_NOT_FOUND"],
    [409, "PROPERTY_CHANGED"],
    [400, "PROPERTY_COUNTY_MISMATCH"],
  ])("preserves actionable %s errors without fake success", async (status, code) => {
    save.mockRejectedValue(
      new HomeIdentityError(Number(status), String(code), "Safe error", {
        countyFips: "Choose a county",
      })
    );
    const response = await request(app())
      .patch("/api/homes/property/identity")
      .send({})
      .expect(Number(status));
    expect(response.body).toEqual({
      message: "Safe error",
      code,
      fieldErrors: { countyFips: "Choose a county" },
    });
  });
