...
$acl = [System.IO.FileInfo]::new($env:TRADESCOUT_ACL_TEST_FILE).GetAccessControl()
$sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User
$rules = @($acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]))
@{ protected = $acl.AreAccessRulesProtected; currentUserOwns = $acl.GetOwner([System.Security.Principal.SecurityIdentifier]).Value -eq $sid.Value; ruleCount = $rules.Count; onlyCurrentUser = @($rules | Where-Object { $_.IdentityReference.Value -ne $sid.Value }).Count -eq 0; fullControl = $rules[0].FileSystemRights -eq 'FullControl'; allowed = $rules[0].AccessControlType -eq 'Allow' } | ConvertTo-Json -Compress
`,
        ],
        {
          env: { ...process.env, TRADESCOUT_ACL_TEST_FILE: path.join(root, ".env.test") },
          encoding: "utf8",
          windowsHide: true,
          timeout: 15_000,
        }
  ...
  it("preserves existing settings and removes the empty temporary file if platform protection fails", async () => {
    const root = fixture();
    const original = "FEATURE_TEST=true\n";
    fs.writeFileSync(path.join(root, ".env.test"), original);
    const { Client, connections } = databaseDriver();
    if (process.platform === "win32") {
      vi.mocked(spawnSync).mockReturnValueOnce({ status: 1, stdout: "", stderr: "denied" } as any);
    } else {
      const open = fs.openSync;
      vi.spyOn(fs, "openSync").mockImplementation((file, flags, mode) => {
        if (String(file).endsWith(".tmp") && flags === "wx") throw new Error("private open denied");
    ...
    expect(connections).toHaveLength(2);
    expect(fs.readFileSync(path.join(root, ".env.test"), "utf8")).toBe(original);
    expect(fs.readdirSync(root)).toEqual([".env.test"]);
  });
});
