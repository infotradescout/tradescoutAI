...
import {
  ...
  PACKAGE_HOME_ID,
  resolveHomeView,
} from "../../client/src/pages/homeid/homeWorkspaceModel";
...
describe("HomeID workspace feature preservation", () => {
  it("defaults to the property overview and keeps explicit package/passport links", () => {
    expect(entry).toContain("HomeOverview");
    expect(entry).toContain("useSearch()");
  ...
  it("retains all nine property sections and every existing save workflow", () => {
    ...
    for (const action of [
      ...
      expect(record).toContain(`const ${action} = useMutation`);
    for (const action of [
      "Upload Documents",
      "Continue Planning",
      "Start a Request",
      ...
      expect(record).toContain(action);
  });
  it("separates real property editing, property creation, and note entry", () => {
    expect(record).toContain("HomeIdentityEditor");
    expect(record).toContain("Edit address & details");
    expect(record).toContain("New property");
    expect(record).toContain("Add a property note");
  ...
  it("retains all six specialist sections and their saved operational data", () => {
    expect(LAUNCH_TABS.map((item) => item.id)).toEqual([
      "control",
      "scope",
      "packages",
      "partners",
    ...
    for (const field of [
      "metadata.launchBoard",
      "metadata.partnerPipeline",
      "partnerPipeline.primaryTargets",
      "metadata.packageExecution",
      "packageExecution.anchorScopeMatrix",
      "packageExecution.packageLevels",
      "packageExecution.executionSteps",
      "metadata.sourceFilesUsed",
      "metadata.sourceFilesExcluded",
      "metadata.firstPackageQuoteTemplate",
      "metadata.builderHandoffTemplate",
      "metadata.ownershipActivationTemplate",
    ])
    ...
    for (const action of [
      ...
      "Ownership protection activation",
      "Open full systems record",
      "Open full HomeID timeline",
    ])
      expect(launch).toContain(action);
  });
  it("keeps prospective partners private and the contact owner unchanged", () => {
    expect(launch).toContain("Private source-review pipeline");
    expect(launch).toContain("No signed partner claim");
    expect(launch).toContain("unconfirmed");
    expect(launch).toContain("No pay-per-lead requirement");
    expect(launch).toContain("/admin/tradepartners");
    expect(launch).not.toContain('apiRequest("POST"');
  });
  it("preserves uploads and adds authenticated downloads without equating references to files", () => {
    expect(record).toContain("uploadPrivateObject(docFile)");
    expect(record).toContain("documentDownloadHref(homeId, item.id)");
    expect(record).toContain("Reference only");
    expect(record).toContain("No files are stored yet");
  ...
  it("does not manufacture readiness or category-based coverage in either workspace", () => {
    for (const source of [record, launch]) {
      expect(source).not.toContain("Ready for next gate");
      expect(source).not.toContain("Design and property screening");
      expect(source).not.toContain("COVERED.has");
      expect(source).not.toMatch(/\.length\s*\|\|\s*(?:17|18|22|12|3)/);
    }
    expect(record).toContain("savedProjectStage(project)");
    expect(launch).toContain("scopeConfirmation(model)");
  });
  it("scopes private reads by viewer and derives both navigation states from the URL", () => {
    ...
    expect(record).toContain("readRecordDetail(await apiRequest");
    expect(record).toContain("recordTab(search)");
    expect(launch).toContain("launchTab(search)");
    expect(launch).toContain('"launch", user?.id');
    expect(record).not.toContain("history.replaceState");
    expect(launch).not.toContain("history.replaceState");
  ...
  it("bounds the application record screens without global or custom-profile theme changes", () => {
    ...
    expect(css).toContain("var(--ts-surface)");
