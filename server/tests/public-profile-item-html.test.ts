...
describe("public profile item HTML", () => {
  ...
  it("builds the exact host-local sitemap set for canonical JW stones, materials, and gallery", async () => {
    profileRecord.contentBlocks = [
      jwPublicDiscoveryBlock,
      {
        id: "recent-work",
    ...
    expect(inventoryItems).toHaveLength(JW_STONE_CANONICAL_INVENTORY_SUMMARY.stoneCount);
    expect(materialCategories).toHaveLength(7);
    expect(locations).toEqual(expectedLocations);
    expect(locations).toHaveLength(
      1 + JW_STONE_CANONICAL_INVENTORY_SUMMARY.stoneCount + materialCategories.length + 1
    );
