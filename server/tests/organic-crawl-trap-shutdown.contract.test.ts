...
describe("organic crawl trap shutdown contracts", () => {
  it("hard-prunes inactive trade, state, county, and city scopes", () => {
    ...
    const county = read("server/publicCountyHtml.ts");
    ...
    expect(county).toContain("if (activeTradeScopes.length === 0) return null;");
    expect(tradeCity).toContain("if (rows.length === 0) return null;");
    expect(city).toContain("if (rows.length === 0) return null;");
    expect(
      serverIndex.match(/res\.status\(404\)\.send\("Trade page not found"\)/g)?.length
    ).toBeGreaterThanOrEqual(4);
    expect(serverIndex).toContain('res.status(404).send("County page not found")');
  });
  ...
  it("does not convert snapshot or listing failures into empty successful pages", () => {
    ...
    expect(county).not.toContain("County directory query failed; serving page without listings");
    expect(county).not.toContain("isMissingColumnError");
    expect(county).not.toContain("runCountyQuery(false)");
    expect(county).toContain("eq(businesses.publicDiscoveryEnabled, true as any)");
    ...
    expect(county).toContain('<meta name="robots" content="noindex, follow" />');
  });
