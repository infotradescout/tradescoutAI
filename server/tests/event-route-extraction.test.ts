...
describe("first-party event route ownership", () => {
  it("registers the legacy Direct Connect bridge before the hardened event endpoint", () => {
    const registrations: Array<[string, number]> = [];
    registerEventRoutes(
      {
        post: (route: string, ...handlers: unknown[]) =>
          registrations.push([route, handlers.length]),
      } as any,
  ...
  it("keeps the event owner narrow and registered ahead of broad analytics", () => {
    ...
    expect(eventModule).toContain("DIRECT_CONNECT_FRICTION_EVENT_TYPES");
...
describe("HTTP failure classification", () => {
  ...
  it("returns a clean 404 for unsupported CMS probes", async () => {
    const app = express();
...
describe("event registries", () => {
  it("accepts the seven existing public demand events", () => {
  ...
  it("accepts the exact ten Direct Connect friction signals", () => {
...
describe("public demand event safety", () => {
  ...
  it("sanitizes attribution field by field", () => {
    expect(
      sanitizeDemandAttribution({
        ref: "partner_123",
        utmSource: "facebook",
...
describe("Direct Connect friction safety", () => {
  it("keeps only allowed metadata and templates identifying routes", () => {
    expect(
      sanitizeDirectConnectEventData({
  ...
  it("rejects private-looking values placed in allowed fields", () => {
    expect(
      sanitizeDirectConnectEventData({
...
describe("first-party event behavior", () => {
  it("responds 204 before an unresolved write", async () => {
  ...
  it("persists a Direct Connect signal through the hardened endpoint", async () => {
    const logEvent = vi.fn().mockResolvedValue(undefined);
    const response = await request(
      createEventApp({ logEvent }, { id: "session-user", contractorId: "session-contractor" })
    )
      .post("/api/events")
      .set("User-Agent", "Mozilla/5.0 Mobile")
      .send({
  ...
  it("intercepts an old cached Direct Connect client before broad shell enrichment", async () => {
    const logEvent = vi.fn().mockResolvedValue(undefined);
  ...
  it("silently drops unknown and oversized payloads", async () => {
    const logEvent = vi.fn().mockResolvedValue(undefined);
    const app = createEventApp({ logEvent });
  ...
  it("keeps asynchronous and synchronous storage failures fail-soft", async () => {
    ...
    const thrown = await request(
      createEventApp({ logEvent: vi.fn(() => { throw new Error("sync"); }) } as any)
    )
      .post("/api/events")
      .send({ eventType: "direct_connect_api_request_failed" });
    await flushEventWrite();
    expect(thrown.status).toBe(204);
