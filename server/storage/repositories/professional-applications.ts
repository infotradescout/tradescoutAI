...
function postgresUniqueViolation(error: unknown): { constraint?: unknown } | undefined {
  const seen = new Set<object>();
  let current = error;
  while (current && typeof current === "object" && !seen.has(current)) {
    seen.add(current);
    if (String((current as any).code || "") === "23505") return current;
...
export function createProfessionalApplicationPersistence(database: any) {
  ...
  async function submit<TProfile extends ProfessionalProfile>(
    ...
    try {
      ...
      // Drizzle wraps native PostgreSQL errors in cause. Read the code and
      // constraint from the same error so unrelated uniqueness failures escape.
      const violation = postgresUniqueViolation(error);
      const violatedConstraint = String(violation?.constraint || "");
      if (
        violation &&
