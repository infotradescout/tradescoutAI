...
export const ACTION_VERIFICATION_REQUIREMENTS = {
  ...
  MESSAGE_USER: {
    ...
    C2_approach: 'ASYMMETRIC: sender gets explainAndOfferVerification(); recipient is checked but not gated',
  ...
  REQUEST_CONTRACTOR_QUOTE: {
    ...
    risk: 'medium',
    jurisdiction: 'local',
    blocking: false,  // IMPLIED from MESSAGE_USER gate
    why: 'Contractors need to verify homeowners to ensure legitimate leads',
    current_gate: 'Implied through message flow + addressVerified',
    C2_approach: 'EXPLICIT: offer verification when homeowner first clicks REQUEST_QUOTE',
    alternate_path: 'Use Scout to search contractors instead (no direct contact needed)',
  },
  ...
  APPLY_AS_CONTRACTOR: {
    ...
    C2_approach: 'EXPLAIN & OFFER: "Most contractors upload license + insurance during signup (10 min). Skip for now and apply later."',
  ...
  ACCEPT_CONTRACTOR_PAYMENT: {
    ...
    C2_approach: 'Trigger when contractor first tries to mark invoice PAID; offer during contract finalization',
  ...
  PUBLISH_PUBLIC_PROFILE: {
    ...
    C2_approach: 'SOFT GATE: offer verification when user first tries to publish; show as "optional but recommended"',
