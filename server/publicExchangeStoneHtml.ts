...
export function renderExchangeStoneLanding(input: StoneLandingInput): { html: string; robots: string } {
  ...
  const cards = visible.map((item, index) => `<article class="stone-card"><a href="${e(link(stoneListingPath(item.id)!, input))}" aria-label="View ${e(item.title)}"><img width="800" height="600" loading="${index ? "lazy" : "eager"}" decoding="async" src="/api/exchange/stone-media/${e(item.id)}" alt="${e(item.title)}"></a><div class="card-body"><p class="seller">Sold by TradeScout</p><h2><a href="${e(link(stoneListingPath(item.id)!, input))}">${e(item.title)}</a></h2><p class="price">${e(stonePriceLabel(item.price, item.specifications?.priceUnit))}</p><p class="price-note">Material only. Delivery, fabrication and installation are separate.</p><p>${e(item.description)}</p><div class="actions"><a class="button" href="${e(link(stoneInquiryPath(item.id, "availability")!, input))}">Check availability</a><a class="button secondary" href="${e(link(stoneInquiryPath(item.id, "callback")!, input))}">Request a call</a></div></div></article>`).join("");
  const status = !input.publicationReady ? "The stone catalog is being prepared. No public selling prices have been released here yet."
    ...
    : input.audience.allowed ? "No stone listings are currently published for this area."
