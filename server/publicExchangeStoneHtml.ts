import { stoneListingPath, stoneInquiryPath, stonePriceLabel } from "../shared/exchangeStoneBuyerFlow";
...
export function renderExchangeStoneLanding(input: StoneLandingInput): { html: string; robots: string } {
  ...
  const visible = input.publicationReady && input.audience.allowed ? input.items.filter(item =>
    stoneListingPath(item.id) && stonePriceLabel(item.price, item.specifications?.priceUnit) &&
  ...
  const cards = visible.map((item, index) => `<article class="stone-card"><a href="${e(link(stoneListingPath(item.id)!, input))}" aria-label="View ${e(item.title)}"><img width="800" height="600" loading="${index ? "lazy" : "eager"}" decoding="async" src="${e(link(`/api/exchange/stone-media/${item.id}`, input))}" alt="${e(item.title)}"></a><div class="card-body"><p class="seller">Sold by TradeScout</p><h2><a href="${e(link(stoneListingPath(item.id)!, input))}">${e(item.title)}</a></h2><p class="price">${e(stonePriceLabel(item.price, item.specifications?.priceUnit))}</p><p class="price-note">Material only. Delivery, fabrication and installation are separate.</p><p>${e(item.description)}</p><div class="actions"><a class="button" href="${e(link(stoneInquiryPath(item.id, "availability")!, input))}">Check availability</a><a class="button secondary" href="${e(link(stoneInquiryPath(item.id, "callback")!, input))}">Request a call</a></div></div></article>`).join("");
