...
export function renderExchangeStoneLanding(input: StoneLandingInput): { html: string; robots: string } {
  ...
  const cardMarkup = visible.map((item, index) => {
    ...
    return `<article class="stone-card"><a href="${e(link(stoneListingPath(item.id)!, input))}" aria-label="View ${e(displayTitle)}"><img width="800" height="600" loading="${index ? "lazy" : "eager"}" decoding="async" src="${e(link(`/api/exchange/stone-media/${item.id}`, input))}" alt="${e(displayTitle)}"></a><div class="card-body"><p class="seller">Sold by TradeScout</p><h2><a href="${e(link(stoneListingPath(item.id)!, input))}">${e(displayTitle)}</a></h2><div class="price-block"><p class="price-label">${e(slabPrice.primaryLabel)}</p><p class="price">${e(slabPrice.primaryPrice)}</p>${slabPrice.secondaryPrice ? `<p class="price-secondary">${e(slabPrice.secondaryPrice)}</p>` : ""}<p class="price-note">${e(note)}</p></div><div class="actions"><a class="button" href="${e(link(stoneInquiryPath(item.id, "availability")!, input))}">Ask TradeScout about availability</a><a class="button secondary" href="${e(link(stoneInquiryPath(item.id, "callback")!, input))}">Request a call</a></div></div></article>`;
  });
