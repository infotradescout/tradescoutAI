...
export type StoneLandingInput = {
  audience: { allowed: boolean; reason: string };
  items: StoneLandingItem[];
...
function attributionParams(tags: Record<string, string> = {}): URLSearchParams {
  ...
  for (const key of TAG_KEYS)
    if (typeof tags[key] === "string" && /^[a-zA-Z0-9_.-]{1,80}$/.test(tags[key]))
      params.set(key, tags[key]);
  return params;
}
function link(path: string, input: StoneLandingInput): string {
  const url = new URL(path, ORIGIN);
  for (const [key, value] of attributionParams(input.acquisitionTags))
...
export function renderExchangeStoneLanding(input: StoneLandingInput): {
  html: string;
  robots: string;
} {
  const hasAreaSelection = Boolean(input.market?.state || input.market?.city);
  const selectedLandingPath = input.audience.allowed
    ? audienceQualifiedStonePath("/exchange/stone", input.market)
    : null;
  const robots = input.publicationReady && !hasAreaSelection
    ? "index, follow"
    : "noindex, follow";
  const visible =
    input.publicationReady && input.audience.allowed && selectedLandingPath
      ? input.items.filter(
          (item) =>
            stoneListingPath(item.id) &&
            stoneSlabMaterialPrice(
              item.price,
            ...
            item.images.some((image) => image === `/api/exchange/stone-media/${item.id}`)
        )
      : [];
  const stateOptions = input.states
    .map(
      (s) =>
        `<option value="${e(s)}"${input.market?.state === s ? " selected" : ""}>${e(s)}</option>`
    )
    .join("");
  const hiddenTags = [...attributionParams(input.acquisitionTags)]
    .map(([key, value]) => `<input type="hidden" name="${e(key)}" value="${e(value)}">`)
    .join("");
  const description =
    "Shop stone slabs through TradeScout Exchange. Compare published material prices and photos, then ask TradeScout about availability, exact dimensions and delivery.";
  const cardMarkup = visible.map((item, index) => {
    ...
    const displayTitle = item.title.replace(/\s*\|\s*TradeScout\s*$/i, "").trim() || item.title;
    const note =
      slabPrice.kind === "size_required"
        ? "Material rate only · confirm slab dimensions."
        : slabPrice.kind === "estimated"
          ? "Reference-size estimate · confirm the selected slab."
          : "For the identified slab · confirm availability.";
    return `<article class="stone-card"><div class="card-body"><p class="seller">Sold by TradeScout</p><h2><a href="${e(link(stoneListingPath(item.id)!, input))}">${e(displayTitle)}</a></h2><div class="price-block"><p class="price-label">${e(slabPrice.primaryLabel)}</p><p class="price">${e(slabPrice.primaryPrice)}</p>${slabPrice.secondaryPrice ? `<p class="price-secondary">${e(slabPrice.secondaryPrice)}</p>` : ""}<p class="price-note">${e(note)}</p></div></div><a class="stone-photo" href="${e(link(stoneListingPath(item.id)!, input))}" aria-label="View ${e(displayTitle)}"><img width="800" height="600" loading="${index ? "lazy" : "eager"}" decoding="async" src="${e(link(`/api/exchange/stone-media/${item.id}`, input))}" alt="${e(displayTitle)}"></a><div class="actions"><a class="button" href="${e(link(stoneInquiryPath(item.id, "availability")!, input))}">Ask TradeScout about availability</a><a class="button secondary" href="${e(link(stoneInquiryPath(item.id, "callback")!, input))}">Request a call</a></div></article>`;
  });
  const firstCards = cardMarkup.slice(0, 12).join("");
  const moreGroups = Array.from(
    { length: Math.ceil(Math.max(0, cardMarkup.length - 12) / 12) },
    (_, group) => {
      const start = 12 + group * 12;
      const end = Math.min(start + 12, cardMarkup.length);
      const label =
        start + 1 === end
          ? `Show stone ${end} of ${cardMarkup.length}`
  ...
  const areaSummary =
    [input.market?.city, input.market?.state].filter(Boolean).join(", ") || "your selected area";
...
<label>City (required for Florida)<input name="audienceCity" maxlength="160" autocomplete="address-level2" value="${e(input.market?.city || "")}"></label>`;
  const searchArea = areaIsSelected
    ...
    : areaFields;
  const changeArea = areaIsSelected
    ? `<details class="area-change"><summary><span>Showing ${e(areaSummary)}</span><span>Change area</span></summary>
...
${areaFields}<button type="submit">Show listings for this area</button></form></details>`
  ...
  const status = !input.publicationReady
    ? "The stone catalog is being prepared. No public selling prices have been released here yet."
    : visible.length > 12
      ? `${visible.length} ${stoneWord} for ${areaSummary}. First 12 shown; open more groups below.`
      : visible.length
        ? `${visible.length} ${stoneWord} for ${areaSummary}. No account is needed to view prices.`
        : input.audience.reason === "pensacola" || input.audience.reason === "outside_us"
          ? "Stone listings are not displayed for your selected area."
          : input.audience.allowed
            ? "No matching stone listings are currently published for this area."
            : "Choose your state to see published listings. For Florida, include your city. No sign-in is required to browse prices.";
  const schema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Stone Slabs | TradeScout Exchange",
    url: ORIGIN + (selectedLandingPath || "/exchange/stone"),
    description,
    publisher: { "@type": "Organization", name: "TradeScout", url: ORIGIN },
    ...(visible.length
      ? {
          mainEntity: {
            "@type": "ItemList",
            numberOfItems: visible.length,
            itemListElement: visible.map((item, position) => ({
              "@type": "ListItem",
              position: position + 1,
              name: item.title,
              url: ORIGIN + audienceQualifiedStonePath(stoneListingPath(item.id)!, input.market),
            })),
          },
        }
      : {}),
  };
  const canonicalTag = hasAreaSelection ? "" : `<link rel="canonical" href="${ORIGIN}/exchange/stone">`;
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Stone Slabs | TradeScout Exchange</title><meta name="description" content="${e(description)}"><meta name="robots" content="${robots}">${canonicalTag}<meta property="og:type" content="website"><meta property="og:site_name" content="TradeScout"><meta property="og:title" content="Stone Slabs | TradeScout Exchange"><meta property="og:description" content="${e(description)}"><meta property="og:url" content="${ORIGIN}${e(selectedLandingPath || "/exchange/stone")}"><script type="application/ld+json">${JSON.stringify(schema).replace(/</g, String.fromCharCode(92) + "u003c")}</script><style>
...
.stone-card{display:flex;flex-direction:column}.stone-card .stone-photo{order:-1;display:block}.stone-card .card-body{padding-bottom:0}.stone-card .price-block{margin-bottom:0}.stone-card .actions{margin:0;padding:12px 16px 16px}@media(max-width:700px){.stone-card .stone-photo{order:0}.stone-card .card-body{padding:10px 14px 0}.stone-card .seller{margin:0;font-size:.78rem;line-height:1.2}.stone-card .card-body h2{margin:2px 0 0}.stone-card .price-block{margin-top:5px;padding:5px 0 9px;border-bottom:0}.stone-card .price-label{line-height:1.2}.stone-card .price-note{margin-top:3px}.stone-card .actions{padding:14px}}@media(max-width:350px){.hero h1{font-size:1.85rem;margin:6px 0}.hero p{line-height:1.35}.stone-card .price{font-size:1.2rem;letter-spacing:-.02em}}
