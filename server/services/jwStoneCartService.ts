...
import {
  reviewJwStoneCart,
  type JwStoneCartReviewRequest,
  type JwStoneCartRate,
  type JwStoneCartStock,
...
export async function getJwStoneCartReview(viewerId: string, request: JwStoneCartReviewRequest) {
  const lines = request.lines;
  const [target, context] = await Promise.all([
    getStoneInventoryProfileTarget("jw-stone"),
    getPublicProfileTrustContext("jw-stone"),
  ]);
  if (!target || !context?.businessId || target.businessId !== context.businessId)
    throw new Error("JW Stone inventory is unavailable");
  // One query provides a coherent inventory snapshot, scoped to exactly JW Stone.
  const result = await pool.query(
    `SELECT ap.public_id, ap.source_asset_ref, ap.asset_kind, ap.passport_status,
    ...
    WHERE ip.holder_business_id = $1 AND ap.public_id = ANY($2::text[])`,
    [target.businessId, lines.map((line) => line.inventoryPublicId)]
  );
  const now = new Date();
  ...
  const records = result.rows.map((row) => {
    ...
    if (received && condition.jwReceiving?.state === "published") {
      try {
        ...
        if (jwStoneReceiptPublicId(receipt.receiptId) === row.public_id) {
          rate = {
            unit: receipt.priceUnit,
            sellPriceCents: receipt.sellPriceCents,
            bundlePriceCents: receipt.bundlePriceCents,
            bundleMinSlabs: receipt.bundleMinSlabs,
          };
          const receivedAt = condition.jwReceiving.receivedAt;
          const received = typeof receivedAt === "string" ? new Date(receivedAt) : null;
          if (saleReady && received && !Number.isNaN(received.getTime()))
            sourceDates.push(received.toISOString());
        }
      } catch {
        /* Missing or invalid lot rates must never fall back to a catalog price. */
      }
    }
    const rawDimensions = object(row.dimensions_json);
    const dimensions = {
      length: rawDimensions.length ?? rawDimensions.width ?? null,
      height: rawDimensions.height ?? null,
      thickness: rawDimensions.thickness ?? null,
      unit:
        rawDimensions.unit === "in"
          ? ("in" as const)
          : rawDimensions.unit === "mm"
            ? ("mm" as const)
            : null,
    };
    const stock: JwStoneCartStock = {
      publicId: String(row.public_id),
      materialName: String(condition.ownerConfirmedName || row.canonical_name || ""),
      materialSlug: String(row.material_slug),
      assetKind: String(row.asset_kind),
      unit: String(row.unit || ""),
      quantity: Number(row.quantity),
      heldQuantity: Number(row.held_quantity),
      saleReady,
      dimensions,
      rate,
    };
  ...
  if (catalogRecords.length) {
    try {
      ...
      sourceDates.push(snapshot.sourceUpdatedAt);
      ...
      for (const row of catalogRecords) {
        ...
        if (price)
          row.stock = {
            ...
            rate: {
              unit: "square_foot",
              sellPriceCents: price.slabPriceCents,
              bundlePriceCents: price.bundlePriceCents,
              bundleMinSlabs: price.bundleMinSlabs,
            },
          };
      }
    } catch {
      /* Available received lots remain reviewable during a workbook outage. */
    }
  }
  // The oldest contributing source date avoids suggesting that every lot was
  // repriced when just one receipt or the catalog workbook changed.
  const sourceUpdatedAt = sourceDates.sort()[0] ?? null;
  return reviewJwStoneCart(
    viewerId,
    lines,
    records.map((row) => row.stock),
    now,
    request.fulfillment,
    sourceUpdatedAt
  );
