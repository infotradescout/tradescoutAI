...
(() => {
  ...
  async function refreshList() { try { const records=await api('');list.replaceChildren();if(!records.length)el('p','No submitted stone offers yet. Make an Offer from a stone or your bundle cart to begin.',list);for(const row of records){button(row.title+' · '+row.status.replaceAll('_',' '),list,()=>load(row.requestId));} }catch(e){showError(e.message);} }
  ...
  async function command(payload) {
    ...
    const body={...payload,expectedRevision:current.state.revision};
  ...
  function paint() {
    ...
    const {state,intake}=current, box=el('article',null,detail);
    el('h2',current.role==='seller'?'Review customer offer':'Your stone offer',box);
    el('p','Order / request '+current.requestId,box,'muted');
    el('h3',state.status.replaceAll('_',' '),box);
    if(current.testMode)el('p','TEST MODE — this is not a live payment.',box,'notice');
    el('p',intake.scope==='cart'?'Whole-cart offer · '+intake.lines.reduce((n,l)=>n+l.quantity,0)+' slabs':'Individual-stone offer',box);
    ...
    el('p','Original listed materials: '+money(intake.listedSubtotalCents)+' · Customer offered: '+money(intake.offeredTotalCents),box);
    ...
    if(current.role==='seller'&&canQuote){const form=el('form',null,box);el('h3','Confirm terms or counteroffer',form);const decisionLabel=el('label','Decision',form), decision=el('select',null,decisionLabel);for(const [value,text]of[['accept_offer','Accept the customer’s material offer'],['counter_offer','Counteroffer with a different material amount']]){const o=el('option',text,decision);o.value=value;}
      const material=amount('Material amount ($)',intake.offeredTotalCents,form,true),tax=amount('Confirmed tax ($) — enter 0 if applicable',null,form),delivery=amount('Confirmed delivery ($)',intake.fulfillment.method==='pickup'?0:null,form,intake.fulfillment.method==='pickup');decision.onchange=()=>{material.readOnly=decision.value==='accept_offer';if(material.readOnly)material.value=(intake.offeredTotalCents/100).toFixed(2);};
      ...
      const submit=button('Send final quote',form,()=>{if(!form.reportValidity())return;try{command({action:'quote',decision:decision.value,materialCents:cents(material),taxCents:cents(tax),deliveryCents:cents(delivery),expiresAt:new Date(expiry.value).toISOString(),notes:notes.value});}catch(e){showError(e.message);}},true);
      button('Decline offer',form,()=>{if(!notes.value.trim()){showError('Enter a decline reason.');return;}command({action:'decline',notes:notes.value});});form.addEventListener('submit',event=>event.preventDefault());
    }
    if(current.role==='buyer'&&state.quote&&['quoted','payment_failed','payment_expired'].includes(state.status)){
      ...
      if(current.methods.length){const label=el('label',null,box),accept=el('input',null,label);accept.type='checkbox';label.append(document.createTextNode('I accept this exact final quote and total of '+money(state.quote.totalCents)+'.'));
