...
export class HomeIdentityError extends Error {
  constructor(
    ...
    public code: string,
    message: string,
    public fieldErrors?: Partial<Record<HomeIdentityField, string>>
  ) {
    super(message);
  }
}
const selection = {
  ...
  propertyType: userHomes.propertyType,
  yearBuilt: userHomes.yearBuilt,
  address1: userHomes.address1,
  address2: userHomes.address2,
  city: userHomes.city,
  stateCode: userHomes.stateCode,
  countyFips: userHomes.countyFips,
  zipCode: userHomes.zipCode,
};
function snapshot(row: HomeIdentity): HomeIdentitySnapshot {
  const identity = {
    id: row.id,
    ...Object.fromEntries(HOME_IDENTITY_FIELDS.map((field) => [field, row[field] ?? null])),
  } as HomeIdentity;
...
function requirePrincipal(userId: string, homeId: string) {
  if (!userId.trim())
    throw new HomeIdentityError(
      401,
      "AUTHENTICATION_REQUIRED",
      "Sign in before editing a property."
    );
  if (!homeId.trim() || homeId.length > 120)
    throw new HomeIdentityError(400, "INVALID_HOME_ID", "Choose a property first.");
}
export async function loadHomeIdentity(
  userId: string,
  homeId: string,
  database: Database = db
): Promise<HomeIdentitySnapshot> {
...
export async function saveHomeIdentity(
  userId: string,
  homeId: string,
  input: unknown,
  database: Database = db
  ...
  return database.transaction(async (tx) => {
    ...
    if (!parsed.success) {
      ...
      for (const issue of parsed.error.issues) {
        ...
        if (
          typeof field === "string" &&
          HOME_IDENTITY_FIELDS.includes(field as HomeIdentityField)
        ) {
      ...
      throw new HomeIdentityError(
        ...
        "Check the highlighted property details.",
        fieldErrors
      );
    }
    if (snapshot(current).revision !== parsed.data.revision) {
      throw new HomeIdentityError(
        409,
        "PROPERTY_CHANGED",
        "This property changed in another session. Review the latest details before saving."
      );
    }
    const changes = parsed.data.changes;
    const next = { ...current, ...changes } as HomeIdentity;
    const changedFields = HOME_IDENTITY_FIELDS.filter(
      (field) => Object.hasOwn(changes, field) && next[field] !== current[field]
    );
    ...
    if (HOME_LOCATION_FIELDS.some((field) => changedFields.includes(field))) {
      ...
      if (hasLocation && (!next.stateCode || !next.countyFips)) {
        throw new HomeIdentityError(
          400,
          "PROPERTY_LOCATION_REQUIRED",
          "Choose the property's state and county or parish before saving its location.",
          {
      ...
      if (next.stateCode && next.countyFips) {
        const [county] = await tx
          ...
          .from(counties)
          .where(and(eq(counties.fips, next.countyFips), eq(counties.stateCode, next.stateCode)))
          .limit(1);
        if (!county)
          throw new HomeIdentityError(
            400,
            "PROPERTY_COUNTY_MISMATCH",
            "The selected county or parish does not belong to that state.",
            { countyFips: "Choose a county or parish from the selected state." }
          );
      }
    }
