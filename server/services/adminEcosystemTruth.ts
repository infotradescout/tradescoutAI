...
async function buildOwners(): Promise<EcosystemTruthOwner[]> {
  ...
  const professionalVerticalsOwner = buildOwner(
    {
      id: "professional_verticals",
      label: "Realtor and car-sales verticals",
      owns: "Professional realtor and vehicle-sales profiles, verification state, and role-specific operating surfaces",
      authority: "Realtor Profiles and Car Salesman Profiles",
      workspacePath: "/admin/users",
    },
    async () => {
      const { rows } = await pool.query(`
        SELECT
          (SELECT COUNT(*)::int FROM realtor_profiles) AS realtor_profiles_total,
          (SELECT COUNT(*)::int FROM realtor_profiles WHERE is_active IS TRUE) AS active_realtor_profiles,
          (SELECT COUNT(*)::int FROM realtor_profiles WHERE verification_status::text = 'pending') AS pending_realtor_verification,
          (SELECT COUNT(*)::int
             FROM realtor_profiles profile
             LEFT JOIN users owner ON owner.id = profile.user_id
            WHERE owner.id IS NULL) AS orphan_realtor_profiles,
          (SELECT COUNT(*)::int FROM car_salesman_profiles) AS car_sales_profiles_total,
          (SELECT COUNT(*)::int FROM car_salesman_profiles WHERE is_active IS TRUE) AS active_car_sales_profiles,
          (SELECT COUNT(*)::int FROM car_salesman_profiles WHERE verification_status::text = 'pending') AS pending_car_sales_verification,
          (SELECT COUNT(*)::int
             FROM car_salesman_profiles profile
             LEFT JOIN users owner ON owner.id = profile.user_id
            WHERE owner.id IS NULL) AS orphan_car_sales_profiles
      `);
      const row = rows[0] || {};
      const orphanRealtors = numberValue(row, "orphan_realtor_profiles");
      const orphanCarSales = numberValue(row, "orphan_car_sales_profiles");
      const attention = orphanRealtors + orphanCarSales > 0;
      return {
        state: attention ? "attention" : "confirmed",
        summary: attention
          ? "The professional verticals remain active, but some profile-owner links need repair."
          : "Realtor and car-sales remain active closed-beta verticals with their existing profile authorities.",
        counts: {
          realtorProfiles: numberValue(row, "realtor_profiles_total"),
          activeRealtorProfiles: numberValue(row, "active_realtor_profiles"),
          pendingRealtorVerification: numberValue(row, "pending_realtor_verification"),
          orphanRealtorProfiles: orphanRealtors,
          carSalesProfiles: numberValue(row, "car_sales_profiles_total"),
          activeCarSalesProfiles: numberValue(row, "active_car_sales_profiles"),
          pendingCarSalesVerification: numberValue(row, "pending_car_sales_verification"),
          orphanCarSalesProfiles: orphanCarSales,
        },
        findings: [
          attention
            ? `${orphanRealtors} realtor profile(s) and ${orphanCarSales} car-sales profile(s) lack their expected owner.`
            : "The checked realtor and car-sales profiles retain their expected owner links.",
          "Neither professional vertical is retired, disabled, or treated as abandoned by this report.",
        ],
      };
    }
  );
    ...
    state:
      availableSources === 0
        ...
        : needsEvidence > 0 ||
          ...
          ? "attention"
          : "confirmed",
    recordsReviewed: availableSources === 3 ? items.length : null,
    needsEvidence: availableSources === 3 ? needsEvidence : null,
    conflicts: conflictResult.available ? conflicts : null,
    items,
    findings: [
      ...
      ...(!conflictResult.available ? ["Commercial conflict evidence could not be read."] : []),
    ],
  };
}
...
async function buildOutcomeCoverage(): Promise<AdminEcosystemTruthReport["outcomeCoverage"]> {
  ...
  const availableSources = sources.filter((source) => source.state !== "unavailable");
  const unlinked = availableSources.reduce(
    (sum, source) => sum + (source.unlinkedCount ?? 0),
    0
  );
...
export async function runAdminEcosystemTruthReport(): Promise<AdminEcosystemTruthReport> {
  ...
  const availableOutcomeSources = outcomeCoverage.sources.filter(
    (source) => source.state !== "unavailable"
  );
  const unlinkedOutcomeEvents =
    availableOutcomeSources.length !== outcomeCoverage.sources.length
      ? null
      : availableOutcomeSources.reduce(
          (sum, source) => sum + (source.unlinkedCount ?? 0),
          0
        );
  ...
  return {
    ...
    summary: {
      ...
      decisionRecords:
        decisionProvenance.state === "unavailable" ? null : decisionProvenance.items.length,
    ...
    protections: [
      ...
      "Realtor and car-sales remain active closed-beta professional verticals.",
      "Stone Core remains the only stone material and physical-inventory authority.",
