...
export const receivingHash = (bytes: Buffer | string) =>
  createHash("sha256").update(bytes).digest("hex");
let cachedToken: { token: string; expiresAt: number } | undefined;
export function receivingConfigured(): boolean {
  return (
    process.env.JW_STONE_RECEIVING_ENABLED === "true" &&
    Boolean(process.env.JW_STONE_RECEIVING_DRIVE_FOLDER_ID?.trim())
  );
}
async function accessToken(): Promise<string> {
  ...
  if (encoded) credentials = JSON.parse(Buffer.from(encoded, "base64").toString("utf8"));
    ...
    credentials = JSON.parse(await readFile(process.env.GOOGLE_APPLICATION_CREDENTIALS, "utf8"));
  ...
  if (credentials?.client_email && credentials.private_key) {
    ...
    body = new URLSearchParams({
      ...
      assertion: `${unsigned}.${signer.sign(credentials.private_key).toString("base64url")}`,
    });
  } else {
    const {
      GOOGLE_CLIENT_ID: id,
      GOOGLE_CLIENT_SECRET: secret,
      JW_STONE_DRIVE_REFRESH_TOKEN: refresh,
    } = process.env;
    if (!id || !secret || !refresh)
      throw new Error("Receiving Drive authorization is not configured");
    body = new URLSearchParams({
      client_id: id,
  ...
  cachedToken = {
    ...
    expiresAt:
      Date.now() + Math.max(1, Math.min(Number(data.expires_in) || 3600, 3600) - 60) * 1000,
  };
...
export async function createReceivingMediaSession() {
  ...
  const metadata = await request(
    `${API}/files/${encodeURIComponent(folderId)}?supportsAllDrives=true&fields=id,mimeType,trashed,parents,capabilities(canAddChildren)`
  );
  if (!metadata.ok) throw new Error("Receiving source folder is unavailable");
  const folder = (await metadata.json()) as {
    mimeType?: string;
    trashed?: boolean;
    parents?: string[];
  ...
  if (
    ...
    folder.trashed ||
    !folder.parents?.includes(rootId) ||
    !folder.capabilities?.canAddChildren
  )
    throw new Error("Receiving requires a writable subfolder of the JW Stone source folder");
  // Receipt manifests include costs. Never put them in a publicly shared folder.
  let pageToken = "";
  do {
    const response = await request(
      `${API}/files/${encodeURIComponent(folderId)}/permissions?supportsAllDrives=true&fields=nextPageToken,permissions(type)&pageSize=100${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ""}`
    );
    ...
    if (
      ...
      permissions.permissions.some((p) => p.type === "anyone" || p.type === "domain")
    )
      throw new Error("Receiving source folder must be private to JW Stone staff");
  ...
  return {
    ...
    async allocateIds(count: number): Promise<string[]> {
      ...
      if (
        !data.ids ||
        data.ids.length !== count ||
        data.ids.some((id) => !/^[a-zA-Z0-9_-]+$/.test(id))
      )
        throw new Error("Invalid receiving source identifiers");
      return data.ids;
    },
    async putFile(
      id: string,
      ...
      const existing = await request(
        `${API}/files/${encodeURIComponent(id)}?supportsAllDrives=true&fields=id,mimeType,trashed,parents,appProperties,md5Checksum`
      );
      if (existing.ok) {
        const item = (await existing.json()) as {
          mimeType?: string;
          trashed?: boolean;
          parents?: string[];
        ...
        // App properties survive Drive edits. Its blob checksum must also match
        // before a lost-response retry can reuse a photo or private manifest.
        const contentHash = createHash("md5").update(bytes).digest("hex");
        if (
          item.trashed ||
          item.mimeType !== mimeType ||
          !item.parents?.includes(folderId) ||
          item.appProperties?.receiptId !== receiptId ||
          item.appProperties?.sha256 !== hash ||
          item.md5Checksum !== contentHash
        )
          throw new Error("Receiving source file changed; a manager must resolve the conflict");
        return;
      }
      ...
      const metadata = {
        ...
        mimeType,
        parents: [folderId],
        appProperties: { receiptId, sha256: hash },
      };
      ...
      if (!response.ok)
        throw new Error(
          `Receiving source upload failed (${response.status}); retry the same arrival`
        );
    },
