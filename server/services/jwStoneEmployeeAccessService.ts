...
import {
  JW_STONE_EMPLOYEE_SCOPES,
  JwStoneEmployeeAccessError,
  isJwStonePlatformManager,
  jwStoneActorId,
  jwStoneConfiguredEmployeeIds,
  type JwStoneEmployeeAccount,
  type JwStoneEmployeeAccessChange,
...
function accountView(row: Row, target: StoneInventoryProfileTarget): JwStoneEmployeeAccount {
  ...
  const source = owner
    ...
    : admin
      ...
      : hasRecord
        ? "manual"
        : configured().includes(row.userId)
...
export async function getJwStoneEmployeeAccess(user: unknown): Promise<{
  ...
  if (!target || target.profileSlug !== "jw-stone")
    return { target: null, allowed: false, canManageStaff: false };
  ...
  const result = await pool.query(
    `SELECT status, scopes, updated_at::text AS revision, (revoked_at IS NOT NULL) AS revoked,
      (expires_at IS NOT NULL AND expires_at <= NOW()) AS expired
    FROM stone_inventory_delegations
    WHERE holder_business_id = $1 AND delegate_user_id = $2 AND delegate_business_id IS NULL`,
    [target.businessId, id]
  );
  if (result.rows.length > 1)
    throw new JwStoneEmployeeAccessError("Employee access records need manager review.", 503);
...
export async function jwStoneEmployeeTarget(
  user: unknown
...
async function managerTarget(user: unknown): Promise<StoneInventoryProfileTarget> {
  const access = await getJwStoneEmployeeAccess(user);
  if (!access.canManageStaff || !access.target)
    throw new JwStoneEmployeeAccessError(
      "Only the JW Stone business owner or a platform administrator can assign employee access.",
      403
    );
...
export async function listJwStoneEmployeeAccounts(user: unknown) {
  ...
  const result = await pool.query(
    `SELECT ${columns} FROM users u ${join}
    WHERE d.delegate_user_id IS NOT NULL OR u.id = ANY($2::text[]) OR u.id = $3
    ORDER BY lower(u.email), u.id LIMIT 501`,
    [target.businessId, configured(), target.businessOwnerUserId]
  );
  return {
    viewerId: jwStoneActorId(user),
    accounts: result.rows.slice(0, 500).map((row) => accountView(row, target)),
...
export async function findJwStoneEmployeeAccount(user: unknown, email: string) {
  ...
  const result = await pool.query(
    `SELECT ${columns} FROM users u ${join}
    WHERE lower(u.email) = $2 ORDER BY u.id LIMIT 2`,
    [target.businessId, email]
  );
  if (!result.rows.length)
    throw new JwStoneEmployeeAccessError(
      "No account matches that email. Have the employee create their JW Stone sign-in first.",
      404
    );
  if (result.rows.length !== 1)
    throw new JwStoneEmployeeAccessError(
      "More than one account matches that email. Resolve the duplicate accounts before assigning access.",
      409
    );
...
export async function setJwStoneEmployeeAccess(user: unknown, change: JwStoneEmployeeAccessChange) {
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))", [
      `jw-stone-staff:${target.businessId}`,
      change.userId,
    ]);
    const found = await client.query(
      `SELECT ${columns} FROM users u ${join}
      WHERE u.id = $2 FOR UPDATE OF u`,
      [target.businessId, change.userId]
    );
    if (found.rows.length !== 1)
      throw new JwStoneEmployeeAccessError("That account no longer exists. Look it up again.", 404);
    const previous = accountView(found.rows[0], target);
    if (previous.source === "owner" || previous.source === "platform_admin") {
      throw new JwStoneEmployeeAccessError(
        "Owner and platform-administrator access is managed through their existing roles, not the employee list.",
        409
      );
    }
    if (previous.revision !== change.expectedRevision)
      throw new JwStoneEmployeeAccessError(
        "This account's permissions changed. Refresh or look up the account again before saving.",
        409
      );
    const scopes = change.allowed ? [...JW_STONE_EMPLOYEE_SCOPES] : [];
    const status = change.allowed ? "active" : "revoked";
    const updated = await client.query(
      `INSERT INTO stone_inventory_delegations
      ...
      [target.businessId, change.userId, scopes, status, actorId, change.expectedRevision]
    );
    if (updated.rows.length !== 1)
      throw new JwStoneEmployeeAccessError(
        "This account's permissions changed. Refresh and try again.",
        409
      );
    ...
    await client.query(
      ...
      [
        change.allowed ? "jw_stone_employee_access_granted" : "jw_stone_employee_access_revoked",
        actorId,
        change.userId,
        JSON.stringify({
    ...
    if (error?.code === "55P03" || error?.code === "40P01")
      throw new JwStoneEmployeeAccessError(
        "Another access update is in progress. Refresh and try again.",
        409
      );
    throw error;
  } finally {
