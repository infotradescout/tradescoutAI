...
/** Orders own durable allocations; temporary-cart expiry never owns payment allocations. */
export class JwStoneSales {
  ...
  private async context(db: Queryable, requestId: string, actor: string | null, lock = false): Promise<Context> {
    ...
    if (found.rows.length !== 1) fail("jw_order_not_found", "JW Stone order or offer not found.", 404);
    const row = found.rows[0];
    const purchase = await db.query("SELECT buyer_user_id,seller_business_id,intake FROM jw_stone_purchase_requests WHERE request_id=$1", [requestId]);
    let intake: Context["intake"];
    if (purchase.rows.length) {
      if (purchase.rows.length!==1 || purchase.rows[0].buyer_user_id!==row.buyer_id || purchase.rows[0].seller_business_id!==row.seller_id) fail("jw_order_identity","The purchase identity needs review.");
      intake = jwStonePurchaseIntakeSchema.parse(purchase.rows[0].intake);
    } else {
      ...
      if (original.length !== 1) fail("jw_offer_not_found", "This request does not contain a verified JW Stone offer or purchase.", 404);
    ...
    if (new Set(intake.lines.map(line=>line.inventoryPublicId)).size!==intake.lines.length) fail("jw_offer_invalid","The request contains duplicate stock identities.");
    return { requestId, buyerId: row.buyer_id, sellerId: row.seller_id, sellerUserId: row.seller_user_id, requestStatus: row.request_status, intake };
  }
  ...
  private async locked<T>(requestId: string, actor: string | null, action: (db: PoolClient, context: Context, state: JwStoneSaleState) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (context.sellerId !== initial.sellerId) fail("jw_seller_changed", "The seller changed. Reload this order.");
  ...
  async purchase(actor: string, input: unknown) {
    const requestId = await createJwStonePurchase({ database:this.database, actor, input,
      admit:(db,target,intake)=>this.admit(db,{requestId:"",buyerId:actor,sellerId:target.sellerId,sellerUserId:target.sellerUserId,requestStatus:"routed",intake}) });
    return this.read(requestId,actor);
  }
  async list(actor: string) {
    if (!actor) fail("jw_sign_in", "Sign in to view your JW Stone orders and offers.", 401);
    const result = await this.database.query(
      `SELECT wr.id,wr.title,wr.created_at,s.state,p.business_id,b.owner_user_id
       ...
       WHERE p.slug='jw-stone' AND wr.source='direct_connect' AND wr.visibility='private'
         ...
         AND (EXISTS (SELECT 1 FROM jw_stone_purchase_requests purchase WHERE purchase.request_id=wr.id AND purchase.buyer_user_id=wr.created_by_user_id AND purchase.seller_business_id=p.business_id)
           OR EXISTS (SELECT 1 FROM work_request_events e WHERE e.work_request_id=wr.id AND e.type='created' AND e.metadata->>'businessSlug'='jw-stone' AND e.metadata->>'requestType'='make_offer' AND e.metadata->'stoneOffer' IS NOT NULL))
  ...
  async command(requestId: string, actor: string, input: unknown) {
    ...
    const prepared=await this.locked(requestId,actor,async(db,context,state)=>{
      ...
      if(state.revision!==command.expectedRevision) fail("jw_order_changed","This order changed. Reload and review the current terms before continuing.");
      ...
      if(command.action==="quote"||command.action==="decline"){
        ...
        if("intent" in context.intake){
          ...
          if(command.decision==="confirm_purchase") fail("jw_offer_amount","Use offer acceptance or a counteroffer for this submitted offer.");
  ...
  private async checkPriorPayments(context: Context, state: JwStoneSaleState, actor: string | null): Promise<boolean> {
    const prior=await this.database.query(
      `SELECT DISTINCT ON (state->'attempt'->>'id') state FROM jw_stone_sale_events
       ...
       ORDER BY state->'attempt'->>'id',revision DESC LIMIT 11`,[context.requestId,state.quote&&state.attempt?state.attempt.id:null]);
  ...
  async reconcile(requestId: string, actor: string | null): Promise<void> {
    ...
    await this.locked(requestId,actor,async(db,ctx,current)=>{
      ...
      if(["payment_failed","payment_expired"].includes(next.status)&&current.allocations.length){await this.release(db,ctx,current);next={...next,allocations:[]};}
