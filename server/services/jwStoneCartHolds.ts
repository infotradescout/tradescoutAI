...
import { jwStoneCartHoldStatusSchema, type JwStoneCartHoldStatus } from "@shared/jwStoneCartHolds";
import { z } from "zod";
...
function money(value: number): number {
  if (!Number.isSafeInteger(value) || value <= 0 || value > MAX_CENTS)
    throw fault("price_unavailable", "The material price needs to be checked again.");
  return value;
}
function canonicalRequest(input: unknown): JwStoneCartHoldRequest {
  ...
  if (!parsed.success)
    throw new JwStoneCartHoldError(
      400,
      "invalid_hold",
      "Check the selected stock, quantities and delivery choice."
    );
  return {
    ...parsed.data,
    lines: combineJwStoneCartLines(parsed.data.lines).sort((a, b) =>
...
function fingerprint(request: JwStoneCartHoldRequest): string {
  return createHash("sha256")
    .update(
      JSON.stringify({
        lines: request.lines,
        expectedSubtotalCents: request.expectedSubtotalCents,
        fulfillment:
          request.fulfillment.method === "pickup"
            ? { method: "pickup" }
            : { method: "delivery", postalCode: request.fulfillment.postalCode },
      })
    )
    .digest("hex");
}
...
export class JwStoneCartHolds {
  ...
  private async transaction<T>(
    ...
    if (!sellerId.trim())
      throw new JwStoneCartHoldError(400, "invalid_seller", "JW Stone could not be resolved.");
    ...
    try {
      ...
      if (
        ["40P01", "40001", "55P03", "57014"].includes(String((error as { code?: string }).code))
      ) {
  ...
  private async membership(client: PoolClient, buyer: string, seller: string): Promise<Row> {
    const result = await client.query(
      `SELECT account.id, account.business_profile_id
      ...
      FOR SHARE OF account, target, business, entitlement`,
      [buyer, seller, JW_STONE_MEMBER_PRICING_PRODUCT_KEY]
    );
  ...
  private async receipt(client: PoolClient, hold: Row): Promise<JwStoneCartHoldReceipt> {
    const result = await client.query(
      "SELECT * FROM jw_stone_cart_hold_items WHERE hold_id=$1::uuid ORDER BY inventory_public_id",
      [hold.id]
    );
    const time = await client.query("SELECT clock_timestamp() AS now");
    return {
      reservationId: hold.public_id,
      status: hold.status,
      expiresAt: new Date(hold.expires_at).toISOString(),
      serverTime: new Date(time.rows[0].now).toISOString(),
      currency: "USD",
      materialSubtotalCents: Number(hold.subtotal_cents),
      paymentStatus: "not_started",
      readyForCheckout: false,
      fulfillment: hold.fulfillment,
      deliveryFeeCents: null,
      estimatedDeliveryDate: null,
      lines: result.rows.map((line) => ({
        inventoryPublicId: line.inventory_public_id,
        materialName: line.material_name,
        quantity: Number(line.quantity),
        unitRateCents: Number(line.unit_rate_cents),
        oneSlabTotalCents: Number(line.one_slab_total_cents),
        lineTotalCents: Number(line.line_total_cents),
        pricingTier: line.pricing_tier,
      })),
    };
  }
  ...
  private async retire(
    ...
    const items = await client.query(
      "SELECT * FROM jw_stone_cart_hold_items WHERE hold_id=$1::uuid ORDER BY inventory_position_id",
      [hold.id]
    );
    if (!items.rows.length)
      throw new JwStoneCartHoldError(
        ...
        "The reservation needs a stock review before release."
      );
    for (const item of items.rows) {
      const updated = await client.query(
        ...
        RETURNING id`,
        [item.inventory_position_id, hold.seller_business_id, item.quantity]
      );
    ...
    const updated = await client.query(
      `UPDATE jw_stone_cart_holds
      SET status=$2, released_at=clock_timestamp() WHERE id=$1::uuid AND status='active' RETURNING *`,
      [hold.id, status]
    );
    if (updated.rowCount !== 1)
      throw fault("hold_changed", "The reservation changed. Reload it before continuing.");
    return updated.rows[0];
  }

  private async expireLocked(client: PoolClient, seller: string): Promise<number> {
    const stale = await client.query(
      `SELECT * FROM jw_stone_cart_holds
      WHERE seller_business_id=$1 AND status='active' AND expires_at<=clock_timestamp()
      ORDER BY expires_at,id LIMIT 100 FOR UPDATE`,
      [seller]
    );
  ...
  async expireSeller(sellerBusinessId: string): Promise<number> {
    return this.transaction(sellerBusinessId, (client) =>
      this.expireLocked(client, sellerBusinessId)
    );
  }
  ...
  async reserve(args: {
  ...
  }): Promise<JwStoneCartHoldReceipt> {
    ...
    return this.transaction(args.sellerBusinessId, async (client) => {
      ...
      // The membership query proves the canonical JW seller. This is a separate commercial admission check.
      const flag = await client.query(
        "SELECT enabled, config FROM feature_flags WHERE key=$1 FOR SHARE",
        [JW_STONE_FEATURE_KEY]
      );
      if (!readStoredJwStoneFeatures(flag.rows[0]).enabled)
        throw new JwStoneCartHoldError(
          403,
          "JW_STONE_FEATURE_UNAVAILABLE",
          "New reservations are unavailable. Existing reservations can still be released."
        );
      ...
      if (replay.rows[0]) {
        ...
        if (hold.request_fingerprint !== digest)
          throw fault(
            "idempotency_conflict",
            "This request key already belongs to a different cart."
          );
        const due = await client.query("SELECT $1::timestamptz<=clock_timestamp() AS due", [
          hold.expires_at,
        ]);
        if (hold.status === "active" && due.rows[0].due)
          hold = await this.retire(client, hold, "expired");
        return this.receipt(client, hold);
      }
      const active = await client.query(
        "SELECT id FROM jw_stone_cart_holds WHERE seller_business_id=$1 AND buyer_user_id=$2 AND status='active' LIMIT 1",
        [args.sellerBusinessId, args.buyerUserId]
      );
      ...
      const stock = await client.query(
        `SELECT position.id AS position_id, position.quantity, position.held_quantity,
      ...
      if (
        stock.rows.length !== request.lines.length ||
        new Set(stock.rows.map((row) => row.public_id)).size !== request.lines.length
      ) {
      ...
      await client.query(
        "SELECT id FROM bidrock_listings WHERE inventory_position_id=ANY($1::uuid[]) ORDER BY id FOR UPDATE",
        [positionIds]
      );
      const auctions = await client.query(
        `SELECT auction.id FROM bidrock_auctions auction
        JOIN bidrock_listings listing ON listing.id=auction.listing_id
        WHERE listing.inventory_position_id=ANY($1::uuid[]) AND auction.status IN ('scheduled','live','extended','ended') LIMIT 1`,
        [positionIds]
      );
      if (auctions.rows.length)
        throw fault(
          "stock_in_auction",
          "A selected lot is committed to an auction and cannot be reserved here."
        );
      ...
      const staged = request.lines.map((line) => {
        const row = stock.rows.find((entry) => entry.public_id === line.inventoryPublicId)!;
        const condition = object(row.condition_json),
          dimensions = object(row.dimensions_json);
        const physical = count(row.quantity),
          held = count(row.held_quantity);
        if (
          row.lifecycle_status !== "available" ||
          row.passport_status !== "verified" ||
          row.public_availability_status !== "published_current" ||
          !row.published_at ||
          Object.keys(object(row.publication_evidence)).length === 0 ||
          !["slab", "bundle"].includes(row.asset_kind) ||
          !/^slabs?$/i.test(String(row.unit).trim()) ||
          ...
          held === null ||
          held > physical ||
          physical - held < line.quantity
        ) {
          throw fault(
            "stock_unavailable",
            "Some selected stock is no longer available in the requested quantity."
          );
        }
        ...
        if (!price)
          throw fault("price_unavailable", "A selected material needs a current JW Stone price.");
        const slabRateCents = money(price.slabPriceCents),
          bundleRateCents = money(price.bundlePriceCents);
        ...
        if (
          ...
          throw fault("dimensions_required", "Exact slab dimensions are needed before reserving.");
        const area = (length * scale * height * scale) / 144;
        return {
          ...line,
          ...
          bundlePricing: {
            ...
            regularOneSlabCents: money(Math.round(area * slabRateCents)),
            bundleOneSlabCents: money(Math.round(area * bundleRateCents)),
          },
        };
      });
      const progress = getJwStoneBundleProgress(staged);
      const prepared = staged.map((line) => {
        const price = priceJwStoneBundleLine(line.bundlePricing, line.quantity, progress.unlocked);
        return {
          ...line,
          tier: price.pricingTier,
          rate: money(price.unitRateCents),
          slab: money(price.oneSlabTotalCents),
          total: money(price.lineTotalCents),
        };
      });
      const subtotal = money(prepared.reduce((sum, line) => sum + line.total, 0));
      if (subtotal !== request.expectedSubtotalCents)
        throw fault(
          "price_changed",
          "The material total changed. Review the new total before reserving."
        );
      const hold = await client.query(
        `WITH timer AS (SELECT clock_timestamp() AS t)
        ...
        [
          args.buyerUserId,
          membership.business_profile_id,
          args.sellerBusinessId,
          membership.id,
          request.idempotencyKey,
          digest,
          subtotal,
          JSON.stringify(request.fulfillment),
          JW_STONE_CART_HOLD_MINUTES,
        ]
      );
      for (const line of prepared) {
        const held = await client.query(
          `UPDATE stone_inventory_positions SET held_quantity=held_quantity+$3,version=version+1,updated_at=NOW()
        ...
        if (held.rowCount !== 1)
          throw fault("stock_unavailable", "Selected stock was allocated to another order.");
        await client.query(
          `INSERT INTO jw_stone_cart_hold_items (hold_id,inventory_position_id,inventory_public_id,
  ...
  async get(args: {
    ...
    if (!jwStoneCartHoldIdSchema.safeParse(args.reservationId).success)
      throw new JwStoneCartHoldError(404, "hold_not_found", "Reservation not found.");
    return this.transaction(args.sellerBusinessId, async (client) => {
      await this.membership(client, args.buyerUserId, args.sellerBusinessId);
      const found = await client.query(
        `SELECT *,expires_at<=clock_timestamp() AS due FROM jw_stone_cart_holds
        WHERE public_id=$1 AND buyer_user_id=$2 AND seller_business_id=$3 FOR UPDATE`,
        [args.reservationId, args.buyerUserId, args.sellerBusinessId]
      );
      if (!found.rows[0])
        throw new JwStoneCartHoldError(404, "hold_not_found", "Reservation not found.");
      const hold =
        found.rows[0].status === "active" && found.rows[0].due
          ? await this.retire(client, found.rows[0], "expired")
          : found.rows[0];
      return this.receipt(client, hold);
    });
  }

  /** Read only owned identity/quantities. It never grants access to stored member prices. */
  async recover(args: {
    buyerUserId: string;
    sellerBusinessId: string;
    operationId?: string;
  }): Promise<JwStoneCartHoldStatus | null> {
    if (!args.buyerUserId.trim())
      throw new JwStoneCartHoldError(
        403,
        "hold_owner_required",
        "Sign in to recover your reservation."
      );
    if (args.operationId !== undefined && !z.string().uuid().safeParse(args.operationId).success)
      throw new JwStoneCartHoldError(
        400,
        "invalid_operation",
        "Invalid reservation request identity."
      );
    return this.transaction(args.sellerBusinessId, async (client) => {
      const result = await client.query(
        `SELECT *, expires_at<=clock_timestamp() AS due FROM jw_stone_cart_holds
        WHERE buyer_user_id=$1 AND seller_business_id=$2 AND ${args.operationId ? "idempotency_key=$3::uuid" : "status='active'"}
        ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
        [args.buyerUserId, args.sellerBusinessId, ...(args.operationId ? [args.operationId] : [])]
      );
      if (!result.rows[0]) return null;
      const found = result.rows[0];
      const hold =
        found.status === "active" && found.due
          ? await this.retire(client, found, "expired")
          : found;
      const items = await client.query(
        "SELECT inventory_public_id, material_name, quantity FROM jw_stone_cart_hold_items WHERE hold_id=$1::uuid ORDER BY inventory_public_id",
        [hold.id]
      );
      const clock = await client.query("SELECT clock_timestamp() AS now");
      return jwStoneCartHoldStatusSchema.parse({
        reservationId: hold.public_id,
        status: hold.status,
        expiresAt: new Date(hold.expires_at).toISOString(),
        serverTime: new Date(clock.rows[0].now).toISOString(),
        totalSlabs: items.rows.reduce((sum, item) => sum + Number(item.quantity), 0),
        lines: items.rows.map((item) => ({
          inventoryPublicId: item.inventory_public_id,
  ...
  /** Trusted idle cleanup is independent of membership and the sales toggle. */
  async expireDueSellers(): Promise<{ expired: number; failedSellers: number }> {
    const due = await this.database.query(
      "SELECT DISTINCT seller_business_id FROM jw_stone_cart_holds WHERE status='active' AND expires_at<=clock_timestamp() ORDER BY seller_business_id LIMIT 100"
    );
    let expired = 0,
      failedSellers = 0;
    for (const row of due.rows) {
      try {
        expired += await this.expireSeller(String(row.seller_business_id));
      } catch {
        failedSellers++;
      }
    }
    return { expired, failedSellers };
  }
  ...
  async release(args: {
    ...
    sellerBusinessId: string;
    reservationId: string;
  }): Promise<{ reservationId: string; status: "released" | "expired" }> {
    if (!jwStoneCartHoldIdSchema.safeParse(args.reservationId).success)
      throw new JwStoneCartHoldError(404, "hold_not_found", "Reservation not found.");
    return this.transaction(args.sellerBusinessId, async (client) => {
      const found = await client.query(
      ...
      if (!found.rows[0])
        throw new JwStoneCartHoldError(404, "hold_not_found", "Reservation not found.");
      const hold = await this.retire(
        client,
        found.rows[0],
        found.rows[0].due ? "expired" : "released"
      );
