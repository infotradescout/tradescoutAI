...
export async function ensureTrustLedgerEventsTable(): Promise<void> {
  const reg = await pool.query<{ reg: string | null }>(
    `SELECT to_regclass('public.trust_ledger_events') as reg`
  );
  if (reg.rows[0]?.reg) return;

  // Deploy-time migrations remain canonical. This is a narrow startup repair so
  // Trust/CVS profile provisioning cannot run before its immutable ledger exists.
