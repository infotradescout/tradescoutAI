...
type TradePartnerTarget = {
  ...
  businessName: string;
  ownerUserId: string;
  approvedDirectPhone: string | null;
...
async function resolveTradePartnerTarget(slug: string): Promise<TradePartnerTarget | null> {
  ...
  return {
    ...
    // Only the exact registered business release may expose its deliberately
    // supplied contact. A public profile cannot release its owner's private phone.
    approvedDirectPhone:
      isRegisteredDirectProfileSlug(normalizedSlug) &&
      isOwnerConfirmedDirectProfile(directProfileCandidate)
        ? String(profileData.phone || "").trim() || null
        : null,
    notificationEmail,
    deliveryCustody,
  };
...
export function registerTradePartnerExpressRoutes(app: Express) {
  ...
  // Preserve the explicitly released direct-business call path. Ordinary
  // profiles still require the private request/accepted-authority lifecycle.
  // The decision alone never grants access: the server rechecks exact business
  // release custody and reads only that business's supplied contact.
  app.post(
    "/api/tradepartner-profiles/:slug/express-contact/reveal",
    revealLimiter,
    async (req: OptionalAuthedRequest, res: Response) => {
      if (isRegisteredDirectProfileSlug(req.params.slug)) {
        ...
        try {
          ...
          const phone = normalizeDirectConnectPhone(target.approvedDirectPhone || "");
          ...
          console.error("[tradepartner-express] approved business call unavailable", error);
