  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await storage.createAuditLog({
        ...
        adjustmentReason:
          actualValue !== contribution.estimatedValue
            ? "Value adjusted during verification"
            : undefined,
        notes,
      });
...
router.get("/payouts/pending", requireCBAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const conditions = [eq(builderPayouts.status, "pending" as any)];
    if (countyId) conditions.push(eq(builderPayouts.countyId, countyId));
...
router.get("/county/:countyId/stats", requireCBAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const builders = await storage.getBuildersByCounty(countyId);
    const contributions = await storage.getCountyContributions(countyId);
    const verifiedContributions = contributions.filter(
      (contribution) => contribution.status === "verified"
    );
    const pendingApproval = contributions.filter(
      (contribution) => contribution.status === "pending_approval"
    ).length;

    const totalValue = verifiedContributions.reduce(
      (sum, contribution) => sum + parseDecimal(contribution.actualValue),
      0
    );
    const totalHours = verifiedContributions.reduce(
      (sum, contribution) => sum + parseDecimal(contribution.actualHours),
      0
    );

    const [payoutTotalRow] = await db
      .select({
        total: sql<string>`COALESCE(SUM(${builderPayouts.amount}), '0')`,
      })
      .from(builderPayouts)
      .where(
        and(eq(builderPayouts.countyId, countyId), eq(builderPayouts.status, "completed" as any))
      );

    const totalPayouts = parseDecimal(payoutTotalRow?.total);
...
router.get("/reconciliation", requireCBAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const countyIds = countyId ? [countyId as string] : []; // For MVP, require countyId parameter
