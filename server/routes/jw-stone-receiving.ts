...
import {
  jwStoneEmployeeTarget,
  receivingUserId,
  receiveJwStoneArrival,
  JwStoneReceivingConflict,
} from "../services/jwStoneReceivingService";
import {
  JW_STONE_RECEIVING_MAX_PHOTOS,
  JW_STONE_RECEIVING_MAX_PHOTO_BYTES,
  JwStoneReceivingInputError,
  parseJwStoneReceipt,
  jwStoneReceiptMemberPrice,
  jwStoneReceivingPhotoKey,
...
export function registerJwStoneReceivingRoutes(app: Express): void {
  ...
  const upload = multer({
    ...
    limits: {
      files: JW_STONE_RECEIVING_MAX_PHOTOS,
      fileSize: JW_STONE_RECEIVING_MAX_PHOTO_BYTES,
      fields: 1,
      fieldSize: 16384,
      parts: JW_STONE_RECEIVING_MAX_PHOTOS + 1,
    },
  }).array("photos", JW_STONE_RECEIVING_MAX_PHOTOS);
  const limiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    limit: 40,
    standardHeaders: "draft-7",
    legacyHeaders: false,
    keyGenerator: (req) => receivingUserId(req.user),
    message: { message: "Too many receiving attempts. Your entries are still on this screen." },
  });
  ...
  const photo = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (result !== "served" && !res.headersSent) {
        ...
        res.status(result === "not_found" ? 404 : 503).end();
      }
    } catch {
      if (!res.headersSent) {
        res.setHeader("Cache-Control", "no-store");
  ...
  app.head(photoRoute, photo);
  app.get(photoRoute, photo);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!context?.businessId || !target || context.businessId !== target.businessId) {
          res.status(404).json({ message: "JW Stone inventory is unavailable." });
          return;
        }
        const items = /\/inventory\/?$/.test(req.path)
          ? await listPublicCurrentStoneInventory(target)
          : await listPublicStoneNewArrivals(target);
        res.json({
  ...
  app.get(`${BASE}/access`, isAuthenticated, async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.json({
        viewerId: receivingUserId(req.user),
        allowed: access.allowed,
        canManageStaff: access.canManageStaff,
        enabled: receivingConfigured(),
      });
    } catch {
      res.status(503).json({ message: "Employee inventory access is temporarily unavailable." });
    }
  });
  app.get(
    `${BASE}/receipts`,
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!target) {
          res.status(403).json({ message: "Assigned JW Stone employee access is required." });
          return;
        }
        const result = await pool.query(
          `SELECT ap.public_id AS "publicId", ap.condition_json->'jwReceiving'->'receipt' AS receipt,
        ...
        ORDER BY ip.received_at DESC LIMIT 50`,
          [target.businessId]
        );
        res.json({ viewerId: receivingUserId(req.user), items: result.rows });
      } catch {
        res.status(503).json({ message: "Recent arrivals are temporarily unavailable." });
      }
    }
  );
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const visible = new Set(
          (await listPublicCurrentStoneInventory(target)).map((item) => item.id)
        );
        const result = await pool.query(
          `SELECT ap.public_id, ap.condition_json->'jwReceiving'->'receipt' AS receipt
        FROM stone_asset_passports ap JOIN stone_inventory_positions ip ON ip.asset_passport_id = ap.id
        WHERE ip.holder_business_id = $1 AND ap.condition_json->'jwReceiving'->>'state' = 'published'`,
          [target.businessId]
        );
        const prices = result.rows
          .filter((row) => visible.has(row.public_id))
          .map((row) => jwStoneReceiptMemberPrice(parseJwStoneReceipt(row.receipt)));
        res.json({ viewerId, prices }); // Explicit projection: never return the receipt or landed cost.
      } catch {
        res.status(503).json({ message: "Arrival pricing is temporarily unavailable." });
      }
    }
  );
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (req.get("X-JW-Receiving") !== "1") {
        res.status(403).json({ message: "Use the JW Stone receiving screen." });
        return;
      }
      let target;
      try {
        target = await jwStoneEmployeeTarget(req.user);
      } catch {
        res.status(503).json({ message: "Employee inventory access is temporarily unavailable." });
        return;
      }
      ...
      if (!receivingConfigured()) {
        res
          .status(503)
          .json({ message: "Employee receiving has not been enabled by your manager." });
        return;
      }
      ...
      upload(req, res, async (uploadError: unknown) => {
        if (uploadError) {
          res.status(400).json({ message: "Use one to eight photos, each no larger than 10 MB." });
          return;
        }
        try {
          let value: unknown;
          try {
            value = JSON.parse(String(req.body?.receipt || ""));
          } catch {
            throw new JwStoneReceivingInputError("Enter the arrival details.");
          }
          const receipt = parseJwStoneReceipt(value);
          const files = Array.isArray(req.files) ? req.files : [];
          const result = await receiveJwStoneArrival(
            employeeTarget,
            receivingUserId(req.user),
            receipt,
            files.map((file) => file.buffer)
          );
          res.status(result.alreadySaved ? 200 : 201).json(result);
        } catch (error) {
          if (error instanceof JwStoneReceivingInputError) {
            res.status(400).json({ message: error.message });
            return;
          }
          if (error instanceof JwStoneReceivingConflict) {
