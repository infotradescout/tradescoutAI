...
import {
  ...
  jwStoneActorId,
  parseJwStoneEmployeeAccessChange,
  parseJwStoneEmployeeLookup,
} from "@shared/jwStoneEmployeeAccess";
import {
  listJwStoneEmployeeAccounts,
  findJwStoneEmployeeAccount,
...
function failure(res: Response, error: unknown) {
  ...
  res
    ...
    .json({
      message:
        "Employee access could not be updated or loaded. Refresh the account before retrying.",
    });
}
export function registerJwStoneEmployeeAccessRoutes(app: Express): void {
  ...
  const limit = rateLimit({
    ...
    legacyHeaders: false,
    keyGenerator: (req) => jwStoneActorId(req.user),
    message: { message: "Too many access requests. No additional access change was made." },
  });
  app.get(
    BASE,
    isAuthenticated,
    ...
    async (req: Request, res: Response) => {
      try {
        res.json(await listJwStoneEmployeeAccounts(req.user));
      } catch (error) {
        failure(res, error);
      }
    }
  ...
  app.post(
    ...
    requireCriticalSchema("stone_inventory"),
    async (req: Request, res: Response) => {
      if (!isJwStoneStaffSameOrigin(req)) {
        res
          .status(403)
          .json({ message: "Manage employee access from the signed-in JW Stone website." });
        return;
      }
      try {
        res.json(await findJwStoneEmployeeAccount(req.user, parseJwStoneEmployeeLookup(req.body)));
      } catch (error) {
        failure(res, error);
      }
    }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        res.json(
          await setJwStoneEmployeeAccess(req.user, parseJwStoneEmployeeAccessChange(req.body))
        );
      } catch (error) {
        failure(res, error);
      }
