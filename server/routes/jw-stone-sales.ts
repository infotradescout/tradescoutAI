...
import { JW_STONE_PURCHASE_PATH } from "@shared/jwStonePurchase";
...
function privateResponse(res: Response) {
  res.setHeader("Cache-Control", "private, no-store");res.setHeader("X-Robots-Tag", "noindex, nofollow");res.vary("Cookie");res.vary("Authorization");
}
...
/** Optional dependencies support isolated acceptance. Production retains canonical session/write guards. */
export function registerJwStoneSalesRoutes(app: Express, sales = new JwStoneSales(pool), authenticate: RequestHandler = isAuthenticated): void {
  if (mounted.has(app)) return;mounted.add(app);
  ...
  app.get(JW_STONE_ORDERS_PATH + "/workspace.js", (_req, res) => {privateResponse(res);res.type("application/javascript").send(jwStoneOrdersBrowserScript);});
  ...
  app.post(JW_STONE_PURCHASE_PATH, authenticate, requireJwStoneCartHoldWriteIntent, mutationLimit, async (req,res) => {
    privateResponse(res);try {res.status(201).json(await sales.purchase(actor(req),req.body));} catch(error) {errorResponse(res,error);}
  });
  ...
  app.post(JW_STONE_PAYMENT_WEBHOOK_PATH, async (req,res) => {
    privateResponse(res);
    const raw=(req as Request & {rawBody?:Buffer}).rawBody, signature=req.get("stripe-signature");
    if(!Buffer.isBuffer(raw)||!signature){res.status(400).json({message:"Signed original webhook bytes are required."});return;}
    let requestId:string|null;
    try {requestId=await sales.provider.webhookRequest(raw,signature);} catch{res.status(400).json({message:"Webhook identity could not be verified."});return;}
