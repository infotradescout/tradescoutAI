...
router.post("/", ...scoutRequestLimiters, async (req: Request, res: Response) => {
      ...
      finalizeScoutResponse(payload, {
        ...
        requestMessage:
          typeof (req.body as { message?: unknown } | undefined)?.message === "string"
            ...
            : null,
        workingMemoryUpdate:
          ((res.locals as any)?.scoutWorkingMemoryUpdate as Record<string, unknown> | undefined) ??
          null,
      })
    );
  ...
  try {
    ...
    const requestState = {
      ...
      last_intent: boundedHistory.messages.length > 0 ? "continuation" : "new_conversation",
      locality: {
        county: countyCode,
        state: stateCode,
