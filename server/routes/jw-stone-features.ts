...
import {
  ...
  JW_STONE_FEATURES_PATH,
  JW_STONE_FEATURE_ADMIN_PATH,
  JwStoneFeatureError,
  projectJwStoneFeatures,
...
function errorResponse(res: Response, error: unknown) {
  ...
  res
    ...
    .json({
      code: known ? error.code : "FEATURE_CONTROL_UNAVAILABLE",
...
function requireSameOrigin(req: Request) {
  ...
  if (!origin || origin !== `${req.protocol}://${req.get("host")}`) {
    throw new JwStoneFeatureError(
      ...
      "Open feature control on this site's authenticated administration page."
    );
  }
}
...
export function registerJwStoneFeatureRoutes(app: Express): void {
  ...
  app.get(JW_STONE_FEATURES_PATH, async (_req, res) => {
    noStore(res);
    try {
      const { audit: _privateAudit, ...state } = await store.read();
      res.json(projectJwStoneFeatures(state));
    } catch (error) {
  ...
  app.get("/admin/jw-stone-features", isAuthenticated, isSuperAdmin, async (_req, res) => {
    ...
    try {
      res.type("html").send(renderJwStoneFeatureControl(await store.read(), randomUUID()));
    } catch (error) {
      errorResponse(res, error);
    }
  });
  app.put(JW_STONE_FEATURE_ADMIN_PATH, isAuthenticated, isSuperAdmin, async (req, res) => {
    ...
    try {
      ...
      res.json(await store.change(String((req.user as { id?: string })?.id || ""), req.body));
    } catch (error) {
      errorResponse(res, error);
    }
  });
  app.post(
    JW_STONE_FEATURE_ADMIN_PATH + "/form",
    isAuthenticated,
    isSuperAdmin,
    ...
    async (req, res) => {
      ...
      try {
        requireSameOrigin(req);
        if (!req.body || !["enabled", "base"].includes(req.body.mode))
          throw new JwStoneFeatureError(400, "INVALID_FEATURE_MODE", "Choose the feature mode.");
        await store.change(String((req.user as { id?: string })?.id || ""), {
          ...
          preserveBaseServices: req.body.preserveBaseServices === "true",
          note: req.body.note,
        });
  ...
  app.use(async (req: Request, res: Response, next) => {
    ...
    if (
      ...
      /^\/api\/admin\/feature-flags(?:\/|$)/i.test(req.path)
    ) {
      try {
        const rawId = req.path.split("/")[4];
        const id = rawId ? decodeURIComponent(rawId) : undefined;
        const reserved =
          req.body?.key === JW_STONE_FEATURE_KEY ||
          (id &&
            (
              await pool.query("SELECT key FROM feature_flags WHERE id = $1 AND key = $2", [
                id,
                JW_STONE_FEATURE_KEY,
              ])
            ).rows.length > 0);
        if (reserved) {
          ...
          res
            ...
            .json({
              message:
                "Use the dedicated JW Stone feature control to change this protected setting.",
            });
          return;
        }
      } catch (error) {
    ...
    const decision = await decideJwStoneFeatureAccess(
      {
        path: req.path,
        method: req.method,
        body: req.body,
        bidRockProfileSlug: req.path.startsWith("/api/admin/bidrock/jw-stone/")
    ...
    noStore(res);
    res.status(decision.status).json({ code: decision.code, message: decision.message });
  });
}
