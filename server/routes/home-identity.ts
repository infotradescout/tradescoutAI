...
import {
  ...
  loadHomeIdentity,
  saveHomeIdentity,
} from "../services/homeIdentityService";
...
function failure(res: Response, error: unknown) {
  if (error instanceof HomeIdentityError) {
    return res
      .status(error.status)
      .json({ message: error.message, code: error.code, fieldErrors: error.fieldErrors });
  }
  return res
    .status(500)
    .json({
      message: "Property details could not be saved or loaded. Please retry.",
...
export function createHomeIdentityRouter(
  dependencies: {
  ...
  router.get("/api/homes/:homeId/identity", authenticate, async (req, res) => {
    ...
    try {
      return res.json(await load(userId(req), String(req.params.homeId || "").trim()));
    } catch (error) {
  ...
  router.patch("/api/homes/:homeId/identity", authenticate, async (req, res) => {
    res.setHeader("Cache-Control", "private, no-store");
    try {
      return res.json(await save(userId(req), String(req.params.homeId || "").trim(), req.body));
    } catch (error) {
      return failure(res, error);
    }
  });
