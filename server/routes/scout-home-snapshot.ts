...
const INTENT_PROMPTS: Record<
  string,
  { text: string; category: string; icon: string }
> = {
  marketplace: {
...
async function getLocalSnapshot(
  ...
  try {
    ...
    activeListingsDelta =
      Number(thisWeekListings[0]?.count ?? 0) -
      Number(lastWeekListings[0]?.count ?? 0);
  } catch (_) {}
  ...
  try {
    ...
    if (fips) {
      const prosResult = await db
        ...
        .innerJoin(
          contractorCounties,
          eq(contractors.id, contractorCounties.contractorId)
        )
        .where(
          and(
            eq(contractorCounties.countyFips, fips),
      ...
      const prosResult = await db
        ...
        .from(contractors)
        .where(
          and(eq(contractors.isActive, true), eq(contractors.verifiedLicensed, true))
        );
  ...
  try {
    ...
    const membersResult = await db
      ...
      .where(
        and(
          sql`lower(${users.county}) like lower(${`%${countyName}%`})`,
          eq(users.isActive as any, true)
        )
      );
    ...
    try {
      const eventsMetric = await db
        ...
        .where(
          and(
            ...
            eq(countyMetrics.metricKey, "events_this_week")
          )
        )
        .limit(1);
      ...
      const eventsTodayMetric = await db
        ...
        .where(
          and(
            eq(countyMetrics.countyFips, fips),
            eq(countyMetrics.metricKey, "events_today")
          )
        )
        .limit(1);
...
async function getTrendingPrompts(
  ...
  try {
    ...
    const whereClause = fips
      ? and(
          eq(scoutInteractions.countyFips, fips),
