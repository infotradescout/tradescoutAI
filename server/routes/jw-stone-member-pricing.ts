...
import { jwStoneCartReviewRequestSchema } from "@shared/jwStoneCart";
...
export function projectJwStonePricingResponse(args: {
  ...
  const prices = args.snapshot.prices.map((price) =>
    Object.freeze({
    ...
    return Object.freeze({
      ...
      prices: Object.freeze(
        prices.map((price, index) =>
          Object.freeze({ ...price, landedCostCents: args.snapshot.prices[index].landedCostCents })
        )
      ),
    });
  ...
  app.get(base, isAuthenticated, async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (access === "none") {
        res
          .status(403)
          .json({ message: "An active JW Stone business membership is required to view pricing." });
        return;
      }
  ...
  app.get(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        res.status(503).json({ message: "JW Stone cart access is temporarily unavailable." });
      }
    }
  );
  app.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        const request = jwStoneCartReviewRequestSchema.safeParse(req.body);
        if (!request.success) {
          res
            .status(400)
            .json({
              message:
                "Choose up to 50 stock selections with valid slab quantities and pickup or delivery details.",
            });
          return;
        }
        res.json(await getJwStoneCartReview(viewerId, request.data));
      } catch (error) {
