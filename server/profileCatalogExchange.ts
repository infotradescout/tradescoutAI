...
const PROFILE_CATALOG_PUBLISHED_AT = "2026-07-26T00:00:00.000Z";

const PROFILE_CATALOG_IMAGES: Record<ProfileCatalogExchangeSpotlight["profileSlug"], string> = {
  "jw-stone": "/images/businesses/jw-stone/video/hero-poster.jpg",
  "issa-build": "/images/businesses/issa-build/video/hero-poster.jpg",
};
...
function toPublicProfileCatalogItem(
  ...
  return {
    ...
    images: [PROFILE_CATALOG_IMAGES[spotlight.profileSlug]],
    location: "Profile catalog",
    seller: {
...
export function listProfileCatalogExchangeItems(filters?: {
  ...
  return PROFILE_CATALOG_EXCHANGE_SPOTLIGHTS.filter((spotlight) => {
    ...
    const haystack =
      `${spotlight.title} ${spotlight.description} ${spotlight.businessName}`.toLowerCase();
