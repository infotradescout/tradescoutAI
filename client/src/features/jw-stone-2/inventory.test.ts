...
describe("JW Stone 2.0 inventory adapter", () => {
  ...
  it("makes all 119 records reachable once across the five color paths", () => {
    const reachableIds = JW_STONE_2_COLOR_DIRECTION_OPTIONS.flatMap((option) =>
      filterJwStone2Inventory({
    ...
    expect(JW_STONE_2_COLOR_DIRECTION_OPTIONS).toHaveLength(5);
    expect(reachableIds).toHaveLength(119);
    expect(new Set(reachableIds).size).toBe(119);
    expect(reachableIds.filter((id) => id === "panda")).toHaveLength(1);
  });
