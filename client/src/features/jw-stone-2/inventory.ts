...
export const JW_STONE_2_COLOR_DIRECTION_OPTIONS: readonly JwStone2ColorDirectionOption[] = [
  { id: "all", label: "All current selections" },
...
export function filterJwStone2Inventory(
  ...
  return inventory.filter((item) => {
    if (filters.color !== "all" && item.colorDirection !== filters.color) return false;
  ...
  return {
    colors: JW_STONE_2_COLOR_DIRECTION_OPTIONS.map((option) => ({
      ...
      count:
        option.id === "all"
          ? inventory.length
          : inventory.filter((item) => item.colorDirection === option.id).length,
    })),
