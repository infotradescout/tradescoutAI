...
>;
const loadAccount = createJwStoneComponentLoader<AccountProps>(() =>
  import("@/components/profile/PublicProfileAccountDialog").then(
    (module) => module.PublicProfileAccountDialog
  )
);
...
export function PublicProfileAccountDialog(props: AccountProps) {
  const { Component: Account, failed } = useJwStoneDeferredComponent(loadAccount, props.open);
      ...
      <DialogContent className="border-stone-200 bg-white text-stone-950 sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {failed
              ? "The account form could not load. Reload this page to try again."
              : JW_STONE_PORTAL_COPY.opening}
          </DialogDescription>
        </DialogHeader>
        {failed ? (
          <button
            type="button"
            className="min-h-11 rounded-full bg-stone-950 px-5 font-bold text-white"
