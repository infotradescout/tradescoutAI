...
import { JwStoneMemberPriceDisplay, useJwStoneMemberPrice, useJwStoneShopping, estimateJwStoneSlabCost } from "./JwStoneMemberPricing";
import { parseJwStoneBundleStock, searchJwStoneBundleStock, type JwStoneBundleStock } from "./bundleStock";
const money = (cents: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(cents / 100);

function StockCard({ item, selected, checking, standalone }: { item: JwStoneBundleStock; selected: number; checking: boolean; standalone: boolean }) {
  ...
  const estimate = memberPrice ? estimateJwStoneSlabCost(memberPrice.slabPriceCents, item.dimensions) : null;
  return <article className="min-w-0 border border-[var(--jw-border)] p-3" data-testid="jw-bundle-stock-option" data-stock-id={item.id}>
      ...
      <fieldset disabled={checking || selected >= Math.min(999, item.quantity)} className="min-w-0 border-0 p-0 disabled:opacity-50" aria-label={`Add ${item.materialName} to your bundle`}>
        {standalone ? <>
          <dl className="mt-3 space-y-1 text-xs">
            <div className="flex justify-between gap-2"><dt>Single-slab rate</dt><dd>{money(memberPrice.slabPriceCents)} / sq. ft.</dd></div>
            <div className="flex justify-between gap-2"><dt>Bundle rate</dt><dd>{money(memberPrice.bundlePriceCents)} / sq. ft.</dd></div>
            {estimate ? <div className="flex justify-between gap-2"><dt>Approx. slab total</dt><dd>{money(estimate.minimumTotalCents)}</dd></div> : null}
          </dl>
          <button type="button" className="mt-3 min-h-11 w-full border border-[var(--jw-accent)] px-3 text-sm font-semibold"
            onClick={() => shopping.addToCart({ id: `stock:${item.id}`, inventoryPublicId: item.id, stoneName: memberPrice.stoneName, stoneKey: memberPrice.stoneKey })}>Add to bundle</button>
        </> : <JwStoneMemberPriceDisplay stoneName={item.materialName} slabDimensions={item.dimensions} inventoryPublicId={item.id} showOfferAction={false} />}
      </fieldset>
      {selected >= Math.min(999, item.quantity) ? <p className="mt-2 text-xs">All listed slabs from this lot are selected. Adjust quantities in {standalone ? "Your bundle" : "your cart below"}.</p> : null}
...
/** Selection only. Authoritative cart review still validates prices and unheld stock. */
export default function JwStoneBundleStockPicker({ review, checking, standalone = false }: { review?: JwStoneCartReview; checking: boolean; standalone?: boolean }) {
  ...
  return <div className="mt-4 min-w-0 space-y-3" data-testid="jw-bundle-stock-picker">
    <p className="text-xs leading-5">Choose any seven eligible slabs—even one each of seven different stones. {standalone ? "Build your selection here, then review it in your cart." : "Add them here without leaving your cart."} Each stone keeps its own bundle rate.</p>
    <label className="block text-xs font-semibold">Find stone for your bundle
    ...
    {inventory.isPending ? <p role="status" className="text-sm">Loading published slab lots…</p> : inventory.isError ? <div role="alert" className="text-sm">
      ...
      <div className={standalone ? "grid gap-3 md:grid-cols-2" : "space-y-3"}>
        {visible.map(item => <StockCard key={item.id} item={item} standalone={standalone}
          selected={standalone ? shopping.items.filter(line => line.inventoryPublicId === item.id).reduce((sum, line) => sum + line.quantity, 0) : review?.lines.find(line => line.inventoryPublicId === item.id)?.requestedQuantity ?? 0}
          checking={checking || inventory.isFetching} />)}
      </div>
    ...
    <p className="text-xs leading-5 text-[var(--jw-muted)]">Listed quantities are not a reservation. Your selection rechecks current availability, exact slab totals, and bundle eligibility after each change.</p>
  </div>;
}
