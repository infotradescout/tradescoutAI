...
function readMemberCart(viewerId: string): readonly JwStoneCartSelection[] {
  ...
  try {
    const raw =
      window.localStorage.getItem(`${JW_STONE_CART_STORAGE_PREFIX}${viewerId}`) ??
...
function writeMemberCart(viewerId: string, items: readonly JwStoneCartSelection[]): void {
  ...
  try {
    window.localStorage.setItem(
      ...
      JSON.stringify(restoreJwStoneCart(items))
    );
    window.localStorage.removeItem(`${JW_STONE_LEGACY_CART_STORAGE_PREFIX}${viewerId}`);
  } catch {
    /* An unavailable browser store must not prevent in-memory shopping. */
  }
}
...
export function JwStoneMemberPricingProvider({
      ...
      setCart((current) => {
        ...
        if (existing)
          return current.map((entry) =>
            entry.id === item.id ? { ...entry, quantity: Math.min(999, entry.quantity + 1) } : entry
          );
  ...
  const updateCartStock = useCallback((id: string, inventoryPublicId: string | undefined) => {
    if (
      ...
      return;
    setCart((current) =>
      current.map((item) => (item.id === id ? { ...item, inventoryPublicId } : item))
    );
  }, []);
  const value = useMemo<JwStoneMemberPricingContextValue>(() => {
    ...
    return Object.freeze({
      ...
      cartEnabled,
      cartCount: cartEnabled ? cart.reduce((sum, item) => sum + item.quantity, 0) : 0,
      addToCart: cartEnabled ? addToCart : () => undefined,
      makeOffer: cartEnabled ? makeOffer : () => undefined,
    });
  }, [
    addToCart,
    makeOffer,
    ...
    <JwStoneMemberPricingContext.Provider value={value}>
      ...
      {value.cartEnabled ? (
        <>
          ...
          {stoneOffer && stoneOffer.viewerId === normalizedViewerId ? (
            <Suspense fallback={<p role="status">Loading offer form…</p>}>
              <JwStoneOfferPanel
                key={normalizedViewerId}
                open
                onClose={() => setStoneOffer(null)}
                ...
                initialStoneName={stoneOffer.scope === "stone" ? stoneOffer.stoneName : undefined}
                jwStoneOffer={stoneOffer}
              />
            </Suspense>
          ) : null}
        </>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <dt className="inline text-[var(--jw-muted)]">
            {price.bundleMinSlabs ? `${price.bundleMinSlabs}+ slabs ` : "7+ slabs "}
          </dt>
          <dd className="inline font-semibold text-[var(--jw-ink)]">
      ...
      {context.cartEnabled ? (
        <button
          type="button"
          data-testid={`jw-stone-make-offer-${presentation}`}
          onClick={() =>
            context.makeOffer({
              id: stockId.success
                ? `stock:${stockId.data}`
          ...
          className="ml-2 mt-2 inline-flex min-h-11 items-center justify-center border border-[var(--jw-accent)] px-3 text-xs font-semibold text-[var(--jw-ink)]"
        >
          Make an offer
        </button>
      ) : null}
      {context.cartEnabled &&
      price.bundlePriceCents < price.slabPriceCents &&
      (price.bundleMinSlabs ?? 7) <= 7 ? (
        <p className="mt-2 text-xs text-[var(--jw-muted)]">
          Build a bundle: mix 7 eligible slabs for bundle pricing.
        </p>
      ) : null}
    </div>
  );
