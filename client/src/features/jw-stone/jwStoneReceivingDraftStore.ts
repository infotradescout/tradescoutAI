...
export class ReceivingDraftConflict extends Error {
  constructor() {
    super(
      "This receiving draft changed in another tab. Reload to recover the latest saved draft before submitting."
    );
  }
}
...
function validateDraft(draft: ReceivingDraft | null): void {
  ...
  if (
    ...
    (draft.frozenReceipt !== null &&
      (typeof draft.frozenReceipt !== "object" || Array.isArray(draft.frozenReceipt)))
  ) {
...
function openDatabase(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    if (typeof indexedDB === "undefined") {
      reject(new Error("This browser cannot save receiving photos for recovery."));
      return;
    }
    let settled = false;
    ...
    request.onupgradeneeded = () => {
      if (!request.result.objectStoreNames.contains(STORE))
        request.result.createObjectStore(STORE, { keyPath: "viewerId" });
    };
    request.onblocked = () =>
      fail("Close other JW Stone tabs, then reload to enable draft recovery.");
    request.onerror = () =>
      fail("Receiving draft storage could not be opened. Check browser storage permissions.");
    request.onsuccess = () => {
      if (settled) {
...
function snapshot(value: unknown): ReceivingDraftSnapshot {
  ...
  if (!Number.isSafeInteger(row?.revision) || row.revision < 1)
    throw new Error("Receiving draft version is invalid; existing data was preserved.");
...
export async function readReceivingDraft(viewerId: string): Promise<ReceivingDraftSnapshot> {
  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.oncomplete = () => {
      database.close();
      try {
        resolve(snapshot(request.result));
      } catch (error) {
        reject(error);
      }
    };
    transaction.onabort = () => {
      database.close();
      reject(new Error("The receiving draft could not be read."));
    };
  });
}
...
export async function writeReceivingDraft(
  ...
  if (
    ...
    throw new Error("Invalid receiving draft revision.");
  const database = await openDatabase();
  return new Promise((resolve, reject) => {
    ...
    transaction.onabort = () => {
      ...
      reject(
        failure ||
          new Error(
            "Draft photos were not saved. Free browser storage and retry before leaving this screen."
          )
      );
    };
  });
