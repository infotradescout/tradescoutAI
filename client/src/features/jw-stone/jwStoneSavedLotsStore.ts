...
export function createJwStoneSavedLotsStore(storage: StoragePort | null) {
  ...
  function read(): SavedLotsSnapshot {
    if (!storage)
      return {
        ...
        restored: true,
        persisted: false,
        notice: "This browser cannot store saved inventory lots.",
      };
    try {
      const raw = storage.getItem(JW_STONE_SAVED_LOTS_KEY);
      if (raw === null) return { lots: [], restored: true, persisted: true, notice: "" };
      const value = JSON.parse(raw);
      if (value?.version !== 1 || !Array.isArray(value.lots))
        throw new Error("Unsupported saved lots");
      ...
      return {
        ...
        persisted: true,
        notice:
          lots.length === value.lots.length
            ? ""
            : "Some invalid saved lot references could not be restored.",
      };
    } catch {
      return {
        ...snapshot,
        ...
        persisted: false,
        notice:
          "Saved inventory lots could not be read. Existing browser data has not been replaced.",
      };
    }
  }
  function latest() {
    return ephemeral ? snapshot : read();
  }
  function write(lots: readonly JwStoneSavedLot[]) {
    ...
    try {
      ...
      /* Keep this visit's changes without claiming a durable save. */
    }
    ephemeral = !persisted;
    publish({
      lots: safe,
      ...
      notice: persisted
        ? ""
        : "These saved-lot changes last for this visit only; browser storage could not be updated.",
    });
  }
  return {
    ...
    toggle(value: unknown) {
      const lot = savedJwStoneLot(value);
      if (!lot) {
        publish({ ...latest(), notice: "This inventory lot could not be saved." });
        return;
      }
      const current = latest();
      if (current.lots.some((item) => item.id === lot.id)) {
        write(current.lots.filter((item) => item.id !== lot.id));
        return;
      }
      if (current.lots.length >= JW_STONE_SAVED_LOTS_MAX) {
        publish({
          ...current,
          notice: "You can save up to 50 inventory lots. Remove one before adding another.",
        });
        return;
      }
    ...
    remove(id: string) {
      write(latest().lots.filter((lot) => lot.id !== id));
    },
    clear() {
      write([]);
    },
