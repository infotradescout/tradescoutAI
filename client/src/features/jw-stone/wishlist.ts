...
import type { WishlistEnvelope, WishlistSnapshot, WishlistStorage, WishlistWriteResult } from "./types";
...
export function reconcileWishlistIds(value: unknown, eligibleIds: ReadonlySet<string> = JW_STONE_NAMED_IDS): string[] {
...
function envelope(ids: readonly string[]): WishlistEnvelope { return { version: JW_STONE_WISHLIST_VERSION, ids }; }
export function getBrowserWishlistStorage(): WishlistStorage | null {
  if (typeof window === "undefined") return null;
  try { return window.localStorage; } catch { return null; }
}
export function saveWishlist(storage: WishlistStorage | null | undefined, ids: readonly string[], eligibleIds: ReadonlySet<string> = JW_STONE_NAMED_IDS): WishlistWriteResult {
  const reconciled = reconcileWishlistIds(ids, eligibleIds);
  if (!storage) return { ids: reconciled, persisted: false };
  try { storage.setItem(JW_STONE_WISHLIST_STORAGE_KEY, JSON.stringify(envelope(reconciled))); return { ids: reconciled, persisted: true }; }
  catch { return { ids: reconciled, persisted: false }; }
}
export function loadWishlist(storage: WishlistStorage | null | undefined, eligibleIds: ReadonlySet<string> = JW_STONE_NAMED_IDS): WishlistSnapshot {
  if (!storage) return { ids: [], status: "unavailable", persisted: false };
  let raw: string | null, legacy = false;
  try {
    raw = storage.getItem(JW_STONE_WISHLIST_STORAGE_KEY);
    if (raw === null) { raw = storage.getItem(JW_STONE_LEGACY_WISHLIST_STORAGE_KEY); legacy = true; }
  } catch { return { ids: [], status: "unavailable", persisted: false }; }
  ...
  if (candidate.version !== JW_STONE_WISHLIST_VERSION) return { ids: [], status: "unsupported", persisted: false };
  ...
  if (legacy || reconciled) { const result = saveWishlist(storage, ids, eligibleIds); return { ids: result.ids, status: "reconciled", persisted: result.persisted }; }
  return { ids, status: "restored", persisted: true };
}
export function toggleWishlistId(ids: readonly string[], id: string, eligibleIds: ReadonlySet<string> = JW_STONE_NAMED_IDS): string[] {
...
export function removeWishlistId(ids: readonly string[], id: string, eligibleIds: ReadonlySet<string> = JW_STONE_NAMED_IDS): string[] { return reconcileWishlistIds(ids, eligibleIds).filter(value => value !== id); }
export function clearWishlist(storage: WishlistStorage | null | undefined): WishlistWriteResult {
  ...
  try {
    ...
    try { storage.removeItem(JW_STONE_LEGACY_WISHLIST_STORAGE_KEY); } catch { /* Empty current record is authoritative. */ }
