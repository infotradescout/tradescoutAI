...
export function useJwStoneWishlist() {
  ...
  return {
    ...snapshot, items, savedLots: savedLots.lots, count: items.length + savedLots.lots.length,
    ...
    toggle: store.toggle,
    remove: (id: string) => { if (/^stone_[a-f0-9]{32}$/.test(id)) savedLots.remove(id); else store.remove(id); },
    clear: () => { store.clear(); savedLots.clear(); },
  };
}
