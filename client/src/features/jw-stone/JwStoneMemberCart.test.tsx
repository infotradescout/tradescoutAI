...
vi.mock("@/pages/profile-sites/ExpressDirectConnectPanel", () => ({
  default: ({ open, initialMessage }: { open: boolean; initialMessage?: string }) =>
    open ? <div data-testid="native-quote-handoff">{initialMessage}</div> : null,
}));
const stockId = `stone_${"a".repeat(32)}`;
const stockItem: PublicStoneInventoryItem = {
  id: stockId,
  ...
  unit: "slabs",
  assetKind: "slab",
  lastConfirmedAt: "2026-09-12T00:00:00.000Z",
  confirmationExpiresAt: "2099-09-12T00:00:00.000Z",
...
describe("JW Stone member cart", () => {
      ...
      root.render(
        <QueryClientProvider client={client}>
          <JwStoneMemberPricingProvider viewerId={viewer || null}>
            ...
            />
            <JwStoneLotCartButton item={stockItem} />
  ...
  beforeEach(() => {
    ...
    api.mockImplementation(async (first: string, second?: any) => {
      ...
      if (url.endsWith("/cart-access"))
        return { viewerId: viewer, allowed: access === "member" && !denied };
      if (url.endsWith("/member-pricing")) {
        if (workbookOffline) throw new Error("Workbook offline");
  ...
  it("restores legacy selections but removes persisted price authority", async () => {
    ...
    expect(document.querySelector('[data-testid="jw-cart-reviewed-subtotal"]')).toBeNull();
    expect(window.localStorage.getItem(JW_STONE_LEGACY_CART_STORAGE_PREFIX + viewer)).toContain(
      "old-selection"
    );
    expect(window.localStorage.getItem(JW_STONE_CART_STORAGE_PREFIX + viewer)).toBeNull();
    click(button("Increase Honey Onyx quantity"));
  ...
  it("does not display or overwrite the previous member's cart when accounts change", async () => {
    ...
    expect(window.localStorage.getItem(JW_STONE_CART_STORAGE_PREFIX + "member-b")).toBeNull();
  });
  ...
  it("received lots still open the canonical cart during a catalog workbook outage", async () => {
    workbookOffline = true;
    render();
    await eventually(() =>
      expect(
        [...document.querySelectorAll("button")].find((element) =>
          element.textContent?.includes("Add this lot to cart")
        )
      ).toBeDefined()
    );
    click(
    ...
    expect(document.querySelectorAll('[data-testid="jw-stone-member-cart-button"]').length).toBe(1);
  });
  it("hides the open cart after the independent cart-access observer is revoked", async () => {
    render(stockId);
    await add();
    denied = true;
    await act(async () => {
      await client.invalidateQueries({ queryKey: ["jw-stone", "cart-access", viewer] });
    });
    await eventually(() =>
      expect(document.querySelector('[data-testid="jw-stone-member-cart"]')).toBeNull()
    );
    expect(document.querySelector('[data-testid="jw-stone-member-cart-button"]')).toBeNull();
    expect(window.localStorage.getItem(JW_STONE_CART_STORAGE_PREFIX + viewer)).toContain(stockId);
    expect(document.querySelector('[data-testid="jw-stone-member-price-card"]')).toBeNull();
  });
  it("keeps quota-failed edits visible and tells the member they last only for this visit", async () => {
    render(stockId);
    await add();
    ...
    click(button("Increase Honey Onyx quantity"));
    await eventually(() =>
      expect(document.body.textContent).toContain("These changes last for this visit only")
    );
    expect(
      (document.querySelector('input[aria-label="Quantity for Honey Onyx"]') as HTMLInputElement)
        .value
