...
function buttonContaining(root: ParentNode | null, label: string): HTMLButtonElement | null {
  return root ? Array.from(root.querySelectorAll("button")).find(button => (button.textContent || "").includes(label)) || null : null;
}
...
describe("StoneDetailDialog", async () => {
  ...
  it("shows confirmed facts and one primary Make an Offer instead of Ask", async () => {
    ...
    const onAsk = vi.fn();
    await act(async () => root.render(<StoneDetailDialog stone={stone} saved={false} onOpenChange={vi.fn()} onToggleSaved={vi.fn()} onAsk={onAsk} />));
    const dialog = document.querySelector<HTMLElement>('[role="dialog"]');
    expect(dialog).not.toBeNull(); expect(dialog?.textContent).toContain(stone.displayName || "");
    expect(dialog?.textContent).not.toContain("Colors from photo"); expect(dialog?.textContent).not.toContain("Pairs with");
    expect(dialog?.querySelector('[aria-label^="Colors #"]')).toBeNull(); expect(dialog?.querySelector('[aria-label^="Pairs with #"]')).toBeNull();
    if (stone.materialLabel) expect(dialog?.textContent).toContain(stone.materialLabel);
    if (stone.origin) { expect(dialog?.textContent).toMatch(/Country of origin/i); expect(dialog?.textContent).toContain(stone.origin.country); }
    if (stone.sourceEvidence?.counts?.length) { expect(dialog?.textContent).toMatch(/Confirm current availability with JW Stone/i); expect(dialog?.textContent).not.toMatch(/Available now|slabs available/i); }
    const offer = buttonContaining(dialog, "Make an Offer"), save = buttonContaining(dialog, "Save this stone");
    expect(offer).not.toBeNull(); expect(save).not.toBeNull();
    expect(dialog?.querySelectorAll('[data-testid="jw-stone-make-offer-detail"]')).toHaveLength(1);
    expect(dialog?.querySelector('[data-testid="jw-stone-detail-actions"]')?.contains(offer!)).toBe(true);
    expect(buttonContaining(dialog, "Ask JW")).toBeNull();
    expect(dialog?.textContent).not.toMatch(/Recorded source counts|Full slabs vary|Learn about stone/i); expect(dialog?.textContent).not.toMatch(/Dual Finish/i);
  ...
  it("keeps a stable detail stage while photos move through a native momentum rail", async () => {
    ...
    const dialog = document.querySelector<HTMLElement>('[role="dialog"]'); expect(dialog).not.toBeNull();
  ...
  it("omits gallery chrome for single-image stones", async () => {
    const stone = JW_STONE_CATALOG.find(entry => entry.images.length === 1);
    expect(stone).toBeTruthy(); if (!stone) throw new Error("Expected a single-image stone");
    await act(async () => root.render(<StoneDetailDialog stone={stone} saved={false} onOpenChange={vi.fn()} onToggleSaved={vi.fn()} onAsk={vi.fn()} />));
    const dialog = document.querySelector<HTMLElement>('[role="dialog"]');
    expect(dialog?.querySelector('[data-testid="jw-stone-detail-photo-prev"]')).toBeNull(); expect(dialog?.querySelector('[data-testid="jw-stone-detail-photo-next"]')).toBeNull(); expect(dialog?.querySelector('[data-testid="jw-stone-detail-photo-thumbs"]')).toBeNull();
  });
  it("keeps request-details and Share for unidentified First Cut without invented offer specifications", async () => {
    ...
    const onAsk = vi.fn();
    await act(async () => root.render(<StoneDetailDialog stone={stone} saved={false} onOpenChange={vi.fn()} onToggleSaved={vi.fn()} onAsk={onAsk} />));
    const dialog = document.querySelector<HTMLElement>('[role="dialog"]'); expect(dialog).not.toBeNull(); expect(dialog?.textContent).toContain("First Cut");
    expect(dialog?.querySelector('[data-testid="jw-stone-detail-pending"]')?.textContent).toMatch(/First Cut Exclusive/i);
    expect(dialog?.querySelector("dl")).toBeNull(); expect(dialog?.textContent).not.toContain("Available now"); expect(dialog?.textContent).not.toContain("Approximate slab dimensions");
    ...
    expect(dialog?.querySelector('[data-testid="jw-stone-share"]')).not.toBeNull(); expect(dialog?.querySelector('[data-testid="jw-stone-detail-save"]')).toBeNull(); expect(dialog?.querySelector('[data-testid="jw-stone-detail-room"]')).toBeNull(); expect(buttonContaining(dialog, "Save this stone")).toBeNull();
    click(request ?? null); expect(onAsk).toHaveBeenCalledWith(stone);
  });
});
