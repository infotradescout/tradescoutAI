import { JW_FINISHED_WORK_PHOTOS } from "./storyBackgrounds";
import { jw } from "./brand";

/**
 * Finished-work gallery — separated from browse rails / inventory.
 * Uses the original profile finished-space photos (not quarry / yard chrome).
 * No visible section heading — process copy + photos carry the section.
 *
 * Copy aligned with `/u/jw-stone` profile presentation:
 * - story eyebrow/heading: "From source to finished space" /
 *   "Stone selected with the final room in mind."
 * - footer: "Quarry-direct sourcing…"
 * - wholesaler differentiators: selection → fabrication/finish → delivery
 */
export function JwFinishedWorkBridge() {
  if (!JW_FINISHED_WORK_PHOTOS.length) return null;

  return (
    <>
      {/* Hard break from browse / inventory chrome above */}
      <div
        aria-hidden="true"
        className="h-10 bg-[var(--jw-bg)] sm:h-14"
        data-testid="jw-finished-work-separator"
      />
      <section
        id="jw-finished-work"
        data-testid="jw-finished-work-bridge"
        aria-label="JW Stone end-to-end stone process"
        className={`border-y border-[var(--jw-border-strong)] bg-[var(--jw-dark)] ${jw.scrollTarget}`}
      >
        <div className="mx-auto max-w-[1600px] px-5 py-10 sm:px-9 sm:py-14 lg:px-12 lg:py-16">
          <p className="max-w-2xl text-base leading-7 text-white/90 sm:text-lg">
            JW Stone handles the entire process — from sourcing through fabrication to delivery —
            with quarry-direct pricing.
          </p>
          <p className="mt-3 max-w-xl text-sm leading-6 text-white/70">
            Stone selected with the final room in mind — kitchens, bars, and architectural installs.
          </p>

          <ul
            className="scrollbar-hide mt-8 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:mt-10 sm:gap-4 [&::-webkit-scrollbar]:hidden"
            aria-label="Finished stone installations"
          >
            {JW_FINISHED_WORK_PHOTOS.map((image) => (
              <li
                key={image.src}
                className="w-[85vw] max-w-[36rem] flex-none snap-center sm:w-[70vw] md:w-[48vw] lg:w-[38vw]"
              >
                <figure className="overflow-hidden bg-black">
                  <span className="relative block aspect-[4/3] sm:aspect-[16/11]">
                    <img
                      src={image.src}
                      alt={image.alt}
                      loading="lazy"
                      decoding="async"
                      className="absolute inset-0 h-full w-full object-cover"
                    />
                  </span>
                  {image.label ? (
                    <figcaption className="border-t border-white/10 px-4 py-3 text-sm font-semibold text-white">
                      {image.label}
                    </figcaption>
                  ) : null}
                </figure>
              </li>
            ))}
          </ul>
        </div>
      </section>
    </>
  );
}
