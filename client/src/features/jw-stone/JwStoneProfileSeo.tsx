...
export function resolveMarketplaceSeo(location: string): MarketplaceSeo {
  ...
  if (parsed.stone) {
    ...
    const materialSuffix = name.toLowerCase().endsWith(material.toLowerCase())
      ? ""
      : ` ${material}`;
    const description = `${country ? `Country of origin: ${country}. ` : ""}${stone?.thicknessCm ? `Thickness: ${stone.thicknessCm} cm. ` : ""}View ${name}${materialSuffix} slab photos from JW Stone Logistics in Pensacola, Florida.`;
    return {
      title: country
