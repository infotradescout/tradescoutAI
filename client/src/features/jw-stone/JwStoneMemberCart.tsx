...
import { jwStoneCartBatch } from "./jwStoneCartStore";
...
export function JwStoneMemberCart({
  ...
  const batch = useMemo(() => jwStoneCartBatch(allItems, page), [allItems, page]);
  ...
  useEffect(() => {
    ...
    queryClient.setQueryData(["jw-stone", "cart-access", viewerId], { viewerId, allowed: false });
    onClose();
  ...
  const requestMessage = [
    ...
    ...(batch.pages > 1
      ? [
          `Batch ${batch.page + 1} of ${batch.pages}: selections ${batch.start + 1}–${batch.start + items.length} of ${allItems.length}. Other selections are not included in this request.`,
        ]
      : []),
    ...(preferences.jobReference.trim() ? [`Job / PO: ${preferences.jobReference.trim()}`] : []),
    ...items.map(
      (item) =>
        `${item.quantity} slab(s): ${item.stoneName.length > 18 ? item.stoneName.slice(0, 17) + "…" : item.stoneName}${item.inventoryPublicId ? ` — stock ${item.inventoryPublicId}` : " — select exact stock with JW Stone"}`
    ),
            ...
            <div className="flex items-start justify-between gap-3 border-b border-[var(--jw-border)] px-5 py-4">
              <div>
                ...
                <Dialog.Description className="mt-1 text-xs leading-5 text-[var(--jw-muted)]">
                  JW Stone business members ·{" "}
                  {persisted ? "Saved on this browser" : "Changes saved for this visit only"}
                </Dialog.Description>
              </div>
              <Dialog.Close
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
              {!persisted ? (
                <p role="status" className="mb-4 text-sm">
                  Browser storage could not be updated. These changes last for this visit only;
                  unreadable saved data is not overwritten.
                </p>
              ) : null}
              {notice ? (
              ...
              {batch.pages > 1 ? (
                <nav
                  aria-label="Saved cart batches"
                  className="mb-4 border border-[var(--jw-border)] p-3 text-sm"
                >
                  <p>
                    All {allItems.length} saved selections are preserved. Review and request one
                    batch of up to 50 at a time.
                  </p>
                  <p role="status" className="mt-2 font-semibold">
                    Selections {batch.start + 1}–{batch.start + items.length} of {allItems.length}
                  </p>
                  <div className="mt-2 flex justify-between gap-3">
                    <button
                      type="button"
                      disabled={batch.page === 0}
                      onClick={() => setPage(batch.page - 1)}
                    ...
                    <button
                      type="button"
                      disabled={batch.page + 1 === batch.pages}
                      onClick={() => setPage(batch.page + 1)}
                      className="min-h-11 px-3 disabled:opacity-40"
                    >
                      Next batch
                      ...
                      >
                        ...
                        {checked?.status === "ready" ? (
                          <div className="mt-3 text-sm" data-testid="jw-cart-line-total">
                            ...
                            <p className="mt-1 text-xs text-[var(--jw-muted)]">
                              {money(checked.unitRateCents)} /{" "}
                              {checked.priceUnit === "slab" ? "slab" : "sq. ft."} ·{" "}
            ...
            <div className="border-t border-[var(--jw-border)] px-5 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              <div role="status" aria-live="polite" className="text-sm">
                {review?.materialReady && review.subtotalCents != null ? (
                  ...
                  >
                    <span>{batch.pages > 1 ? "Batch material subtotal" : "Material subtotal"}</span>
              ...
              <div className="mt-3 flex gap-2">
                ...
                >
                  {batch.pages > 1 ? `Request quote for batch ${batch.page + 1}` : "Request quote"}
                </button>
              </div>
              {requestMessage.length > 5000 ? (
