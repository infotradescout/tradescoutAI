...
export function DealRoomPanel({ jobId, userRole }: DealRoomPanelProps) {
  ...
  const headerPrimary = isStandalone ? "Standalone Accounting" : "Project Workflow";
  ...
  const handleApproveEstimate = async () => {
    ...
    try {
      ...
      toast({
        title: "Could not approve estimate",
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "You can now send this invoice to the homeowner.",
      });
  ...
  const handleIssueReceipt = async () => {
    ...
    try {
      const res = await fetch(`/api/documents/${latestInvoice.id}/mark-paid`, {
        ...
        body: JSON.stringify({
          method: "other",
          reference: "Recorded manually from workflow panel",
        }),
      });
      if (!res.ok) {
        const code = await extractErrorCode(res);
  ...
  const handleShare = async (documentId: string) => {
    try {
      ...
      toast({
        ...
        description: "Send this link to anyone who needs to review.",
      });
    } catch (e: any) {
      toast({
    ...
    try {
      const res = await fetch(`/api/jobs/${jobId}/material-list`, {
        ...
        body: JSON.stringify({
          payload: {
            items: [],
            notes: "Starter material list draft created from the workflow panel.",
          },
        }),
      });
      if (!res.ok) {
      ...
      toast({
        title: "Material list created",
        description: "You can now edit and send this list.",
        variant: "default",
      });
      refetch();
    } catch (e: any) {
      toast({
        title: "Could not create material list",
        description: e?.message || "Please try again.",
        variant: "destructive",
      });
    }
  ...
  const handleSendDocument = async (id: string, label: string) => {
    try {
      ...
      toast({
        title: `Could not send ${label.toLowerCase()}`,
        description: e?.message || "Please try again.",
        variant: "destructive",
      });
    }
    ...
    if (latestContract) {
      ...
      if (latestContract.status === "draft" && userRole === "contractor") {
        return {
          ...
          kind: "simple" as PrimaryActionKind,
          label: "Send contract",
          explanation: "Send this contract so your client can review and sign it.",
    ...
    if (latestEstimate) {
      if (state.canApproveEstimate) {
        return {
          ...
          explanation: "Approve this estimate to move forward to a formal contract.",
      ...
      if (latestEstimate.status === "draft" && userRole === "contractor") {
        return {
          ...
          kind: "simple" as PrimaryActionKind,
          label: "Send estimate",
          explanation: "Send this estimate so your client can review and approve it.",
      ...
      if (latestEstimate.status === "sent") {
        return {
          ...base,
          kind: "none" as PrimaryActionKind,
          explanation: "Waiting on the homeowner to approve this estimate.",
        };
      }
      ...
      return {
        ...base,
        kind: "none" as PrimaryActionKind,
        explanation: "Estimate is approved. The next step is to finalize and send a contract.",
      };
    }
    ...
    if (latestMaterial) {
      if (latestMaterial.status === "draft" && userRole === "contractor") {
        return {
          ...
          explanation: "Send this material list so your client can choose materials and options.",
      ...
      if (latestMaterial.status === "sent") {
        return {
          ...base,
          kind: "none" as PrimaryActionKind,
          explanation: "Waiting on the homeowner to review and choose materials.",
        };
      }
    }
  ...
  return (
    <Card className="bg-tsCard/95 border-white/10 h-full flex flex-col">
      <CardHeader className="pb-3 space-y-1">
        ...
        <CardTitle className="text-sm text-white">Workflow Panel</CardTitle>
        ...
        {isLoading ? (
          ...
          <>
            <div className="rounded-md border border-white/10 bg-tsCard/95 p-3 mb-2">
              ...
              {primaryAction.kind === "invoiceFromContract" &&
                state.canCreateInvoice &&
                userRole === "contractor" && (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-white">{invoiceNumberDisplay}</p>
                    <p className="text-[11px] text-white/60">
                ...
                {invoiceStatusRaw === "paid" && (
                  <p className="text-[11px] text-emerald-300/90">
                    This invoice is locked for editing. Keep it as a permanent record in your files
                    and ledger.
                  </p>
                )}
                        ...
                        invoiceLines.map((line, idx) => {
                          ...
                          return (
                            <TableRow key={idx} className="border-white/10">
                              ...
                              <TableCell className="text-right text-white/70">{qty}</TableCell>
                ...
                <div className="mt-3 flex flex-col items-end gap-1 text-[11px] text-white/70">
                  {invoiceSubtotal !== null && (
                    <div className="flex justify-between gap-6 w-full max-w-xs">
                      ...
                      <span>
                        {invoiceSubtotal.toLocaleString(undefined, {
                          style: "currency",
                          currency: invoiceCurrency,
                        })}
                      </span>
                  ...
                  {invoiceTax !== null && (
                    <div className="flex justify-between gap-6 w-full max-w-xs">
                      ...
                      <span>
                        {invoiceTax.toLocaleString(undefined, {
                          style: "currency",
                          currency: invoiceCurrency,
                        })}
                      </span>
                ...
                {invoicePayment && (
                  <div className="mt-3 border-t border-white/10 pt-2 text-[11px] text-white/70">
                    ...
                    {invoicePayment.receivedAt && (
                      <p>Received {new Date(invoicePayment.receivedAt).toLocaleString()}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="mt-3 space-y-2 flex-1 overflow-auto">
              ...
              {documents.length === 0 ? (
                <p className="text-xs text-white/60">
                  No documents recorded yet. When you create or attach a document, it will show up
                  here.
                </p>
              ) : (
                <div className="space-y-2">
                  {documents.map((doc) => (
                    <div key={doc.id} className="border border-white/10 rounded-md p-2 bg-black/30">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-xs font-semibold text-white">{doc.type}</p>
