...
import { ExternalLink, ThumbsUp, Percent, ShoppingCart, Truck } from "lucide-react";
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between mb-1">
                    <h4 className="text-white font-medium text-sm truncate pr-2">{product.name}</h4>
                    {product.isSponsored && (
                      <Badge
                        variant="outline"
                  ...
                  <div className="flex items-center gap-2 mb-2 text-white/60 text-xs">
                    <ThumbsUp className="h-3 w-3 text-ts-orange" />
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {product.discount && (
                        <Badge className="bg-green-600 text-xs">-{product.discount}%</Badge>
                      )}
                    </div>
                    <ExternalLink className="h-4 w-4 text-white/60" />
                  </div>

                  <p className="text-white/70 text-xs mt-1 line-clamp-2">{product.description}</p>
                </div>
              </div>
            </div>
          ))}
...
function getContextualProducts(
  ...
  if (projectType) {
    ...
    relevantProducts = baseProducts.filter(
      (product) =>
