        ...
        <div key={event.id} className="morphic-card group cursor-pointer">
            ...
            <div className="absolute top-3 left-3 flex gap-2">
              <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-bold text-white">
                  {event.organizerVerified ? "Organizer verified" : "Community event"}
                </span>
              </div>
            </div>
          ...
          <div className="p-4">
            <div className="flex justify-between items-start mb-2">
              <h4 className="text-tsTextMain font-bold text-lg leading-tight flex-1">
                {event.title}
              </h4>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-tsBorder pt-4">
              <div className="flex items-center gap-2 text-[11px] text-tsTextMuted">
                ...
                <span>
                  Time: <span className="text-tsTextMain font-medium">{event.time}</span>
                </span>
              </div>
              <div className="flex items-center gap-2 text-[11px] text-tsTextMuted">
                <MapPin size={14} className="text-tsAccent" />
                <span>
                  Location: <span className="text-tsTextMain font-medium">{event.location}</span>
                </span>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {event.tags.map((tag, i) => (
                <span
                  key={i}
                  className="morphic-status-badge success bg-tsAccent/5 text-tsAccent border-tsAccent/10"
                >
                  {tag}
                </span>
              ))}
