      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className="grid grid-cols-2 gap-0.5">
             {[1,2,3,4].map(i => <div key={i} className="w-1.5 h-1.5 bg-tsAccent rounded-sm" />)}
          </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-bold text-tsAccent uppercase tracking-tighter">Live Results</span>
        </div>
      </div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className={`morphic-status-badge ${service.status === 'OPEN' ? 'success' : 'warning'}`}>
                {service.statusText}
              </div>
              <div className="flex items-center gap-1 text-xs text-tsTextMuted">
                <Star size={12} className="text-tsAccent" fill="currentColor" />
                <span className="font-bold text-tsTextMain">{service.rating.toFixed(1)}</span>
