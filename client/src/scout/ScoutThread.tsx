...
function clusterKindMeta(kind: ScoutCluster["kind"]) {
  switch (kind) {
    ...
    case "community":
      return { label: "Nearby activity", icon: MessageSquareText, emoji: "💬" };
    case "projects":
      return { label: "Saved request", icon: ClipboardList, emoji: "📋" };
    case "rules":
      return { label: "Rules and permits", icon: BadgeCheck, emoji: "📄" };
    case "site":
      return { label: "TradeScout search", icon: Search, emoji: "🔍" };
    case "account":
...
function defaultActionsForCluster(cluster: ScoutCluster): ScoutAction[] {
  if (cluster.kind === "pros") {
    return [
      { type: "NAVIGATE", label: "Create request", to: "/direct-connect", subtitle: "Review before sharing" },
      { type: "NAVIGATE", label: "Browse local help", to: "/direct-connect/pros" },
    ];
  }
  if (cluster.kind === "community") return [{ type: "NAVIGATE", label: "Open community", to: "/community" }];
  if (cluster.kind === "marketplace") return [{ type: "NAVIGATE", label: "Open Exchange", to: "/exchange" }];
  if (cluster.kind === "projects") return [{ type: "NAVIGATE", label: "Open saved requests", to: "/direct-connect" }];
  if (cluster.kind === "site") return [{ type: "NAVIGATE", label: "Open help", to: "/help" }];
  if (cluster.kind === "rules") return [{ type: "ASK_SCOUT", label: "Check rules", prompt: `Check local rules and permits for ${cluster.title || "this"}.` }];
  if (cluster.kind === "account") return [{ type: "NAVIGATE", label: "Open settings", to: "/settings" }];
  return [];
}
...
function buildAskScoutAction(cluster: ScoutCluster): ScoutAction {
  ...
  return { type: "ASK_SCOUT", label: "Ask Scout", prompt: `Tell me more about ${title} and what I can safely do next.${bodyHint}` };
}
...
function buildEvidenceChips(msg: ScoutMessage): string[] {
  ...
  if (provenance.confidenceBand) chips.push(`Confidence: ${humanizeToken(provenance.confidenceBand)}`);
  if (typeof provenance.knowledgeLayer === "number") chips.push(`Layer: ${provenance.knowledgeLayer}`);
  if (provenance.fallbackUsed) chips.push("Fallback: Active");
  if (provenance.degradationReason) chips.push(`Degraded: ${humanizeToken(provenance.degradationReason)}`);
  if (provenance.blockingReason) chips.push(`Authority: Gated (${humanizeToken(provenance.blockingReason)})`);
  else if (Array.isArray(provenance.allowedActions) && provenance.allowedActions.length > 0) chips.push("Authority: Clear");
  return chips;
}
...
function EvidenceStrip({ msg, enabled }: { msg: ScoutMessage; enabled: boolean }) {
  ...
  const evidenceSources = React.useMemo(() => {
    const sourceTitles = Array.isArray(msg.provenance?.sourceTitles) ? msg.provenance?.sourceTitles : [];
  ...
  return (
    <div className="mt-2 px-1" aria-label="Scout evidence and authority">
      <button
        ...
        style={{ color: "rgba(249,115,22,0.7)", background: "rgba(249,115,22,0.06)", border: "1px solid rgba(249,115,22,0.15)" }}
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      >
        <Shield size={10} />
        {open ? "Hide source" : "Why this answer"}
      </button>
      {open && (
        <div className="mt-2 space-y-1.5 rounded-xl p-3" style={{ background: "#111", border: "1px solid rgba(255,255,255,0.07)" }}>
          {chips.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {chips.map((chip) => (
                <span key={`${msg.id}-${chip}`} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(250,250,250,0.5)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  {chip}
                </span>
              ))}
            </div>
...
/* ----------------------------------------------------------
   ...
   ---------------------------------------------------------- */
function ClusterCard({ cluster, onAction }: { cluster: ScoutCluster; onAction?: (action: ScoutAction) => void }) {
      ...
      {cluster.items && cluster.items.length > 0 && (
        <ul className="mt-3 space-y-2 list-none p-0">
          {cluster.items.map((item) => (
            <li key={item.id} className="flex gap-2.5 rounded-xl p-2.5" style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.05)" }}>
              <Bookmark className="mt-0.5 h-3.5 w-3.5 shrink-0 text-ts-orange" />
              <span className="min-w-0">
                <span className="block text-[13px] font-semibold" style={{ color: "#fafafa" }}>{item.label}</span>
                {item.description && (
                  <span className="mt-0.5 block text-[11px]" style={{ color: "rgba(250,250,250,0.45)" }}>{item.description}</span>
                )}
              </span>
            </li>
          ))}
            ...
            {visibleActions.map((action) => (
              ...
              >
                ...
                <span className="flex flex-col items-start text-left">
                  ...
                  {action.subtitle && <span className="text-[10px] opacity-75">{action.subtitle}</span>}
                </span>
              </button>
            ))}
          </div>
      ...
      {cluster.ctaSource && cluster.ctaContextId && (
        <div className="mt-3 space-y-1">
          {cluster.ctaLabel && cluster.ctaSource === "trade_deal" && (
            <div className="text-[11px]" style={{ color: "rgba(250,250,250,0.4)" }}>{cluster.ctaLabel}</div>
          )}
          <CommunityCTA
            layout="inline"
...
function MessageExtras({
  ...
  const hasOnboardingPrompt = Boolean(msg.onboarding?.active && Boolean(msg.onboarding.question) && Boolean(onSendMessage));
  ...
  const dedupedSuggestions = React.useMemo(() => {
    ...
    for (const chip of msg.frame?.actionChips || []) { if (chip.label) taken.add(normalizeActionText(chip.label)); }
    for (const cluster of msg.clusters || []) {
      ...
      for (const action of cluster.actions || []) { if (action.label) taken.add(normalizeActionText(action.label)); }
    }
    const seen = new Set<string>();
  ...
  const hasAnything = hasActionChips || hasClusters || hasOverride || hasSuggestions || hasOnboardingPrompt;
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      {(hasActionChips || hasClusters || (showControllerExtras && hasOverride)) && (
        <div className="rounded-2xl p-3 space-y-3" style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.06)" }}>
          {/* Section label */}
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              style={{ color: "rgba(250,250,250,0.4)", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}
          ...
          {controllerOpen && (
            <div className="space-y-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (chip.kind === "NAVIGATE") {
                            onAction({ type: "NAVIGATE", label: chip.label, to: chip.target, path: chip.target, payload: chip.args && typeof chip.args === "object" ? (chip.args as Record<string, unknown>) : undefined });
                          }
                        }}
                        className="scout-tool-tray__btn scout-tool-tray__btn--secondary"
                        style={{ minHeight: "40px", fontSize: "12px", textTransform: "none", letterSpacing: "normal", padding: "0 14px" }}
                      >
                        <ArrowRight size={12} />
                        <div className="flex flex-col items-start text-left">
                          <span>{chip.label}</span>
                          {chip.subtitle && <span className="text-[10px] opacity-75">{chip.subtitle}</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                  {prioritizedActionChips.length > 2 && (
                    <button type="button" onClick={() => setControllerShowAll((v) => !v)} className="text-[10px] font-semibold rounded-full px-3 py-1" style={{ color: "rgba(249,115,22,0.7)", background: "rgba(249,115,22,0.06)", border: "1px solid rgba(249,115,22,0.15)" }}>
                      {controllerShowAll ? "Show fewer" : `More actions (${prioritizedActionChips.length - 2})`}
                    </button>
                  )}
                </div>
              )}
              ...
              {msg.clusters && msg.clusters.length > 0 && (
                <div className="space-y-2">
                  ...
                  {msg.clusters.length > 2 && (
                    <button type="button" onClick={() => setControllerShowAll((v) => !v)} className="text-[10px] font-semibold rounded-full px-3 py-1" style={{ color: "rgba(249,115,22,0.7)", background: "rgba(249,115,22,0.06)", border: "1px solid rgba(249,115,22,0.15)" }}>
                      {controllerShowAll ? "Show fewer sections" : `More sections (${msg.clusters.length - 2})`}
                    </button>
                  )}
                </div>
              )}
              ...
              {showControllerExtras && msg.overrideOption && (
                <div className="rounded-xl p-3" style={{ background: "#111", border: "1px dashed rgba(249,115,22,0.25)" }}>
                  <div className="text-[12px] mb-2" style={{ color: "rgba(250,250,250,0.6)" }}>{msg.overrideOption.message}</div>
                  <button
                    type="button"
                    onClick={() => onOverride && onOverride(msg.overrideOption!)}
                    disabled={overridePendingScope === (msg.overrideOption.scope ?? "global")}
                    className="scout-tool-tray__btn scout-tool-tray__btn--secondary"
                    style={{ minHeight: "36px", fontSize: "12px", textTransform: "none", letterSpacing: "normal", padding: "0 14px" }}
                  >
                    {overridePendingScope === (msg.overrideOption.scope ?? "global") ? "Logging override..." : msg.overrideOption.label}
                  </button>
                </div>
              )}
            </div>
      ...
      {shouldShowSuggestions && (
        <div className="rounded-2xl p-3" style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div className="flex flex-wrap gap-2">
              {dedupedSuggestions.map((act) => (
                <button
                  ...
                  style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(250,250,250,0.7)" }}
                >
                  {act}
                </button>
              ))}
      ...
      {msg.onboarding?.active && msg.onboarding.question && onSendMessage && (
        <OnboardingPrompt
          ...
          mode="card"
          onAnswer={(value) => onSendMessage({ onboardingAnswer: { sessionId: msg.onboarding!.sessionId, questionKey: msg.onboarding!.question!.key, value } })}
          onSkip={() => onSendMessage({ onboardingAnswer: { sessionId: msg.onboarding!.sessionId, questionKey: msg.onboarding!.question!.key, skipped: true } })}
        />
      )}
    </div>
  );
    ...
    <div className="space-y-1.5">
      ...
      <div className="scout-live-status">
        ...
        <span className="scout-live-status__text">
          {label} <span className="scout-live-status__highlight">{Math.round(progress * 100)}% complete</span>
        </span>
      ...
      <div className="h-0.5 w-full rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
        <div
          ...
          style={{ width: `${Math.max(5, Math.min(Math.round(progress * 100), 100))}%`, background: "linear-gradient(to right, #f97316, #ea580c)" }}
        />
      </div>
    </div>
  );
...
const ScoutThread: React.FC<ScoutThreadProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (lastMessage.role === "assistant") {
      ...
      if (target) { target.scrollIntoView({ block: "end", behavior: "smooth" }); return; }
    }
  ...
  if (status === "resolving_context") {
    ...
    const statusMessages =
      mode === "contractors"
        ? ["Reading your search...", "Finding nearby help...", "Checking trust and profile details..."]
        : mode === "marketplace"
          ? ["Searching local listings...", "Comparing matching offers...", "Checking recent marketplace updates..."]
          : mode === "admin"
            ? ["Checking system status...", "Gathering activity reports...", "Compiling control settings..."]
            : ["Searching TradeScout...", "Checking what's nearby...", "Preparing useful matches..."];
  ...
  return (
    ...
    >
            ...
            {isUser ? (
              ...
              <div className="scout-user-bubble">
                <div className="scout-user-bubble__meta">
                  ...
                  <div className="scout-user-bubble__avatar" aria-hidden="true">U</div>
                </div>
                <div className="scout-user-bubble__body">
                  {displayContent}
                </div>
              </div>
            ) : (
      ...
      {/* Live status bar — @reusable: ScoutLiveStatus */}
      {showProgress && statusLabel && (
        <ScoutLiveStatus label={statusLabel} progress={progress} />
      )}
    </div>
  );
};
