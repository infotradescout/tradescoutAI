 ...
 * Phase 3d-A: Checkbox UI for confirming Scout-inferred claims
 * 
 * Contract:
...
export function ClaimConfirmationCard({ data, onConfirm, onSkip, onEdit }: ClaimConfirmationCardProps) {
  ...
  return (
    <Card className="w-full max-w-2xl border-primary/20 bg-card/95 backdrop-blur">
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-2 justify-between">
        ...
        >
          Confirm {selectedClaims.size > 0 && `(${selectedClaims.size})`}
        </Button>
      </CardFooter>
    </Card>
  );
