    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-6 pt-10 pointer-events-none">
      ...
      <div className="max-w-2xl mx-auto relative pointer-events-auto">
        ...
        <div className="morphic-glass rounded-2xl px-6 py-3 flex items-center justify-between border-tsBorder/50">
          <NavItem icon={<Home size={20} />} label="Home" active={currentMode === 'home'} onClick={() => onNavigate('home')} />
          <NavItem icon={<LayoutGrid size={20} />} label="Directory" active={currentMode === 'directory'} onClick={() => onNavigate('directory')} />
          ...
          <div className="flex flex-col items-center -mt-8">
            ...
            <span className="text-[10px] font-bold text-tsAccent uppercase tracking-tighter mt-1">Scout OS</span>
          </div>

          <NavItem icon={<Bookmark size={20} />} label="Saved" active={currentMode === 'saved'} onClick={() => onNavigate('saved')} />
          <NavItem icon={<User size={20} />} label="Profile" active={currentMode === 'profile'} onClick={() => onNavigate('profile')} />
        </div>
