...
export default function AdminPanel() {
  ...
  return (
    <AdminWorkspace data-testid="admin-platform-settings-v2">
      ...
      <Tabs
        value={selectedTab}
        onValueChange={(value) => setSelectedTab(value as NativeSettingsTab)}
        className="space-y-6"
      >
        <AdminWorkspaceSubnav>
      ...
      {loading ? (
        ...
        <AdminList>
          {items.map((item) => {
            const key =
              type === "business-provider-settings"
                ? (item as ContractorSetting).setting
                : (item as SiteSetting).key;
            return (
              <div
                key={item.id}
                className="grid gap-4 px-3 py-4 sm:px-4 lg:grid-cols-[minmax(10rem,0.65fr)_minmax(12rem,0.8fr)_minmax(0,1.2fr)_auto] lg:items-center"
              >
                <div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate font-mono text-xs text-white/28">
        ...
        <AdminEmptyState
          ...
          description="Create the first record when a real configuration value is required."
        />
      )}
    </AdminSection>
  );
...
function AdvertisementList({
  ...
  return (
    <AdminSection
      ...
      description="Platform advertisement records and targeting configuration."
      className="pt-0"
      actions={
        <div className="flex gap-2">
          <Button
    ...
    >
      {loading ? (
        <QueueLoading label="Loading advertisements…" />
      ) : error ? (
        <QueueUnavailable label="Advertisements are unavailable. No record was changed." />
      ) : items.length ? (
        <AdminList>
          {items.map((item) => (
            <div
              key={item.id}
              className="grid gap-4 px-3 py-4 sm:px-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(12rem,0.7fr)_minmax(10rem,0.55fr)_auto] lg:items-center"
            >
              <div className="min-w-0">
                ...
                <p className="mt-2 line-clamp-2 text-sm text-white/48">{item.content}</p>
              </div>
              <div className="text-sm text-white/52">
                <p>
                  {readable(item.placement)} · {readable(item.targetAudience)}
                </p>
                <p className="mt-1 text-xs text-white/32">{item.targetLocation || "national"}</p>
              </div>
              <div className="text-sm text-white/52">
                <p>{item.viewCount || 0} views</p>
                <p className="mt-1">{item.clickCount || 0} clicks</p>
              </div>
              <div className="flex gap-2">
                <Button
                  type="button"
        ...
        <AdminEmptyState
          ...
          description="Create an advertisement only when a real placement and audience are defined."
        />
      )}
    </AdminSection>
  );
...
function PrizeList({
  ...
  return (
    <AdminSection
      title="Prize configurations"
      description="Giveaway prize records, probabilities, expirations, and active state."
      className="pt-0"
      actions={
        <div className="flex gap-2">
          <Button
    ...
    >
      {loading ? (
        <QueueLoading label="Loading prizes…" />
      ) : error ? (
        <QueueUnavailable label="Prize configurations are unavailable. No record was changed." />
      ) : items.length ? (
        <AdminList>
          {items.map((item) => (
            <div
              key={item.id}
              className="grid gap-4 px-3 py-4 sm:px-4 lg:grid-cols-[minmax(0,1fr)_minmax(10rem,0.55fr)_minmax(10rem,0.55fr)_auto] lg:items-center"
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                ...
                <p className="mt-2 line-clamp-2 text-sm text-white/48">
                  {item.description || "No description"}
                </p>
              </div>
              <div className="text-sm text-white/55">
                <p>{readable(item.prizeType)}</p>
                <p className="mt-1 text-xs text-white/32">{item.vendor || "No vendor"}</p>
              </div>
              <div className="text-sm text-white/55">
                <p>{item.value}</p>
                <p className="mt-1 text-xs text-white/32">
                  {(Number(item.probability || 0) * 100).toFixed(2)}% · {item.expirationDays || 30}{" "}
                  days
                </p>
              </div>
              <div className="flex gap-2">
        ...
        <AdminEmptyState
          title="No prizes"
          description="Create a prize only when its real terms and probability are defined."
        />
      )}
    </AdminSection>
  );
  ...
  return (
    <div className="grid gap-7 xl:grid-cols-[minmax(18rem,0.72fr)_minmax(0,1.28fr)]">
      <AdminSection
        ...
        description="Send one test to the current admin account and verify in-app and push delivery."
        className="pt-0"
      >
        <div className="border-y border-white/10 px-3 py-5 sm:px-4">
          <Button
            type="button"
            onClick={props.onTest}
            disabled={props.testPending}
            className="bg-orange-500 text-black hover:bg-orange-400"
          >
            <Bell className="mr-2 h-4 w-4" />
            {props.testPending ? "Sending…" : "Send test notification"}
          </Button>
          <p className="mt-4 text-sm leading-6 text-white/45">
            Confirm the notification appears in the header and full notification view. Push also
            requires a secure origin, valid browser permission, and an active device subscription.
          </p>
        </div>
      </AdminSection>
      <AdminSection
        title="Broadcast announcement"
        description="In-app delivery is always included. Email and push remain optional and respect existing preferences and subscriptions."
        className="pt-0"
      >
        <div className="space-y-5 border-y border-white/10 px-3 py-5 sm:px-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label>Segment</Label>
              <Select
                value={props.segment}
                onValueChange={(value) => props.onSegmentChange(value as BroadcastSegment)}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="homeowners">Homeowners and residents</SelectItem>
                  <SelectItem value="contractors">Contractors and trades</SelectItem>
                  <SelectItem value="pros">All professionals</SelectItem>
                  <SelectItem value="admins">Platform admins</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-3">
              <ToggleRow
                label="Email where allowed"
                checked={props.email}
                onChange={props.onEmailChange}
              />
              <ToggleRow
                label="Push where configured"
                checked={props.push}
                onChange={props.onPushChange}
              />
              <ToggleRow
                label="Marketing opt-in only"
                checked={props.marketingOnly}
                onChange={props.onMarketingOnlyChange}
              />
            </div>
          </div>
          <div className="space-y-2">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <TextInput
              ...
              onChange={props.onCampaignTypeChange}
            />
            <TextInput label="Tags" value={props.tags} onChange={props.onTagsChange} />
            <TextInput
              label="Target state"
              value={props.targetState}
              onChange={(value) => props.onTargetStateChange(value.toUpperCase().slice(0, 2))}
            />
          </div>
          <Button
            type="button"
            onClick={props.onBroadcast}
            disabled={props.broadcastPending || !props.title.trim() || !props.message.trim()}
            className="bg-orange-500 text-black hover:bg-orange-400"
          >
            <Megaphone className="mr-2 h-4 w-4" />
            {props.broadcastPending ? "Sending…" : "Send broadcast"}
          </Button>
        </div>
      </AdminSection>
    </div>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto border-white/12 bg-tsBg text-white sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-white">
            {editor.item ? "Edit" : "Create"} {readable(editor.type)}
          </DialogTitle>
          <DialogDescription className="text-white/45">
            Save only the configuration fields shown in this form.
          </DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(event) => {
        ...
        >
          {editor.type === "prizes" ? (
            <PrizeFields data={formData} update={update} />
          ) : editor.type === "advertisements" ? (
            <AdvertisementFields data={formData} update={update} />
          ) : (
            <SettingFields type={editor.type} data={formData} update={update} />
          )}
          <div className="flex justify-end gap-2 border-t border-white/10 pt-5">
            ...
            <Button
              ...
              className="bg-orange-500 text-black hover:bg-orange-400"
            >
              {saving ? "Saving…" : editor.item ? "Save changes" : "Create"}
            </Button>
          </div>
        </form>
      </DialogContent>
    ...
    <>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={data.prizeType || undefined}
            onValueChange={(value) => update("prizeType", value)}
          >
            <SelectTrigger className="border-white/10 bg-black/20 text-white">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <NumberInput
          label="Probability %"
          value={Number(data.probability || 0) * 100 || 5}
          onChange={(value) => update("probability", String(value / 100))}
        />
        <NumberInput
          label="Expiration days"
          value={Number(data.expirationDays || 30)}
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <TextInput
          ...
          value={data.targetAudience || "all"}
          onChange={(value) => update("targetAudience", value)}
        />
        <TextInput
          label="Target location"
          value={data.targetLocation || "national"}
          onChange={(value) => update("targetLocation", value)}
        />
        <NumberInput
          label="Priority"
          value={Number(data.priority || 0)}
          onChange={(value) => update("priority", value)}
        />
        <NumberInput
          label="Community score"
          value={Number(data.communityScore || 50)}
          onChange={(value) => update("communityScore", value)}
        />
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <DateInput
  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        />
        <TextInput
          label={type === "business-provider-settings" ? "Setting" : "Key"}
          value={data[settingKey] || ""}
          onChange={(value) => update(settingKey, value)}
          required
        />
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          value={jsonValue(data.value)}
          onChange={(event) => {
            try {
              update("value", JSON.parse(event.target.value));
            } catch {
              update("value", event.target.value);
            }
          }}
          className="min-h-36 border-white/10 bg-black/20 font-mono text-white"
          required
        />
      </div>
      <ToggleRow
...
function ToggleRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-y border-white/10 px-3 py-3 text-sm text-white/60">
      <span>{label}</span>
