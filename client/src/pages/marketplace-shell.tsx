...
export default function MarketplaceShellPage() {
  ...
  const listingsQuery = useQuery<MarketplaceListing[]>({
    queryKey: ["/api/marketplace/listings", stateCode, county, categoryFilter, searchTerm],
    queryFn: async () => {
      ...
      const raw = await apiRequest("GET", `/api/marketplace/listings?${params.toString()}`);
  ...
  const createListingMutation = useMutation({
    mutationFn: async (payload: CreateListingPayload) => {
      if (!stateCode || !county) {
        throw new Error("Set your location before posting to the marketplace.");
      }
    ...
    <MarketplaceShell locationOverride={location}>
      <div className="max-w-5xl mx-auto px-3 py-4 sm:px-4 sm:py-6">
        ...
        {listingsQuery.isLoading && (
          <div className="py-4 text-center text-white/60">Loading listings…</div>
        )}
        ...
        {!listingsQuery.isLoading && !listingsQuery.isError && listings.length === 0 && (
          <div className="py-6 text-center text-white/60" data-testid="marketplace-empty-state">
        ...
        >
          {listings.map((listing) => {
            ...
            const thumbnailUrl = images?.length ? (images[primaryIndex ?? 0] ?? images[0]) : null;

            const categoryName = (listing as any).categoryName as string | undefined;
            const sellerName = (listing as any).sellerName as string | undefined;
          ...
          >
            <div className="w-full max-w-md rounded-2xl border border-white/10 bg-tsBg p-4">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-white">New listing</h2>
                <button
                  type="button"
              ...
              <form className="mt-4 space-y-3" onSubmit={handleSubmitCreate}>
                <div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="w-32">
                    ...
                    <select
                      ...
                      onChange={(e) => setFormCategoryId(e.target.value || undefined)}
