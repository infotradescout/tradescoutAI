...
describe("request composer recovery behavior", () => {
  ...
  it.each(["skip_for_now", "link_existing"] as const)(
    "preserves HomeID draft deletions and %s when the packet reloads after remount",
    async (homeChoice) => {
      state.user = account;
      const props = {
        ...initialProps,
        entryLocation: `${entry}&homeId=home-original&homePacketId=packet-1`,
        prefillHomeId: "home-original",
        prefillHomePacketId: "packet-1",
        prefillHomeContextIntent: "update_from_request" as const,
      };
          ...
          requestPackets: [
            {
              id: "packet-1",
              requestType: "service",
              selectedDetailIds: ["detail-1"],
              status: "ready_for_handoff",
            },
          ],
          propertyDetails: [{ id: "detail-1", note: "Original HomeID detail" }],
        },
      };
      client.setQueryDefaults(["/api/homes"], {
        queryFn: async () => ({
          homes: [{ id: "home-original" }, { id: "home-chosen" }],
        }),
      });
      state.api.mockResolvedValue(packetResponse);
      await mount(props);
      await act(async () => {
        await new Promise((resolve) => setTimeout(resolve, 5));
      });
      expect(container.textContent).toContain("1 HomeID detail loaded");
      const requestType = () =>
        Array.from(container.querySelectorAll<HTMLButtonElement>("button")).find((node) =>
          node.textContent?.startsWith("Ongoing support")
        )!;
      await act(async () => requestType().click());
      await click("Review request");
      if (homeChoice === "skip_for_now") {
        await click("Skip for now");
      } else {
        await click("Use saved home details");
        const homeSelect = container.querySelector<HTMLSelectElement>("select")!;
        await act(async () => {
          homeSelect.value = "home-chosen";
          homeSelect.dispatchEvent(new Event("change", { bubbles: true }));
        });
      }
      await click("Back to details");
      await change(field("Original roof"), "");
      await change(field("Original repair details"), "");
      await unmount();

      client.removeQueries({ queryKey: ["direct-connect-homeid-handoff"] });
      let loadPacket!: (response: typeof packetResponse) => void;
      const reloadingPacket = new Promise<typeof packetResponse>((resolve) => {
        loadPacket = resolve;
      });
      state.api.mockImplementation((method) =>
        method === "GET" ? reloadingPacket : Promise.resolve({ id: "request-saved" })
      );
      await mount(props);
      expect(container.textContent).toContain("Loading saved HomeID request details");
      expect(container.querySelector("input")?.value).toBe("");
      expect(container.querySelector("textarea")?.value).toBe("");
      await act(async () => {
        loadPacket(packetResponse);
        await new Promise((resolve) => setTimeout(resolve, 5));
      });
      expect(container.textContent).toContain("1 HomeID detail loaded");
      expect(container.querySelector("input")?.value).toBe("");
      expect(container.querySelector("textarea")?.value).toBe("");
      expect(requestType().getAttribute("aria-pressed")).toBe("true");

      await change(container.querySelector<HTMLInputElement>("input")!, "Revised request");
      await change(
        container.querySelector<HTMLTextAreaElement>("textarea")!,
        "Only these revised details should be shared."
      );
      await send();
      const submitted = state.api.mock.calls.find(([method]) => method === "POST")?.[2];
      expect(submitted).toEqual(
        expect.objectContaining({
          title: "Revised request",
          description: "Only these revised details should be shared.",
          category: "business_request",
          targetProfileSlug: "bayou-roofing",
          ...(homeChoice === "link_existing"
            ? { homeId: "home-chosen", homeContextIntent: "link_existing" }
            : {}),
        })
      );
      if (homeChoice === "skip_for_now") {
        expect(submitted).not.toHaveProperty("homeContextIntent");
        expect(submitted).not.toHaveProperty("homeId");
        expect(submitted).not.toHaveProperty("assetComponentType");
        expect(submitted).not.toHaveProperty("assetComponentId");
        expect(submitted).not.toHaveProperty("assetLabel");
      }
      expect(submitted).not.toHaveProperty("homePacketId");
      expect(submitted).not.toHaveProperty("homePacketSelectedDetailIds");
    }
  );
  ...
  it.each([
    ...
    "uses the chosen county for review, recipient search, and submission: $expectedCounty/$stateCode",
    async ({ county, stateCode, expectedCounty, expectedState, label }) => {
      state.user = account;
      await mount({
        entryLocation: county ? `/direct-connect?county=${county}` : "/direct-connect",
        defaultCountyFips: county,
        defaultStateCode: stateCode,
        prefillTitle: "Repair the porch roof",
        prefillDescription:
          "Water enters above the porch after heavy rain. Please inspect next week.",
      });
      await click("Review request");
      ...
      await click("Review request details");
      await click("Send when ready");
      const lookup = state.api.mock.calls.find(
        ([method, url]) => method === "GET" && url.startsWith("/api/business-providers/search?")
      );
      expect(lookup).toBeTruthy();
      expect(new URL(lookup![1], "https://test.local").searchParams.get("county")).toBe(
        expectedCounty
      );
      expect(container.textContent).not.toContain("County context is missing");
      await click("Continue without selection");
      await act(async () => {
        await new Promise((resolve) => setTimeout(resolve, 5));
      });
      expect(state.api).toHaveBeenCalledWith(
