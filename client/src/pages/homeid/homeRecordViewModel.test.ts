...
describe("record workspace navigation and data boundaries", () => {
  ...
  it("uses only a saved project stage, not a location or project category", () => {
    expect(
      savedProjectStage({
        id: "p",
        status: "planning",
        metadata: { address1: "Somewhere", propertyAssigned: true },
      })
    ).toBeNull();
    expect(savedProjectStage({ id: "p", metadata: { currentStage: "Engineering" } })).toBe(
      "Engineering"
    );
  ...
  it("retains unknown system categories without upgrading review status", () => {
    const systems = [
      { id: "stone", type: "natural_stone", label: "Stone", status: "needs_review" as const },
      { id: "custom", type: "solar", label: "Solar", status: "unknown" as const },
    ];
    ...
    expect(groups.flatMap((group) => group.items).find((item) => item.id === "stone")?.status).toBe(
      "needs_review"
    );
  });
  it("distinguishes failed/malformed collections from real emptiness", () => {
    expect(rows({ projects: [] }, "projects")).toEqual([]);
    for (const projects of [null, undefined, 4, [null], ["not a project"]])
      expect(() => rows({ projects }, "projects")).toThrow();
    expect(() =>
      readRecordDetail(
        { home: { id: "wrong" }, records: [], documents: [], appliances: [] },
        "wanted"
      )
    ).toThrow();
    expect(() =>
      readRecordPersistence({
        persistence: { propertyDetails: [], requestPackets: [], evidence: [] },
      })
    ).toThrow();
    expect(() =>
      readRecordPersistence({
        persistence: {
          propertyDetails: [{ id: "bad" }],
          requestPackets: [],
          components: [],
          evidence: [],
        },
  ...
  it.each([
    ...
    "https://example.com/\nfile",
    "relative/path",
  ])("does not create an unsafe or ambiguous evidence link: %s", (value) => {
  ...
  it("keeps authenticated document downloads and project timelines tied to their owner records", () => {
    expect(documentDownloadHref("home/one", "doc?two")).toBe(
      "/api/homes/home%2Fone/documents/doc%3Ftwo/download"
    );
    ...
    expect(safeEvidenceHref("/api/homes/home/documents/doc/download")).toBe(
      "/api/homes/home/documents/doc/download"
    );
    expect(safeEvidenceHref("https://example.com/manual.pdf")).toBe(
...
describe("package planning never invents counts or approvals", () => {
  it("keeps missing arrays unknown and saved empty arrays zero", () => {
    ...
    const empty = launchWorkspaceModel({
      metadata: {
        launchBoard: { tasks: [] },
        partnerPipeline: { primaryTargets: [] },
        packageExecution: { anchorScopeMatrix: [], packageLevels: [] },
      },
    });
    expect(listCount(empty.launchTasks)).toBe("0");
    expect(listCount(empty.scopeMatrix)).toBe("0");
    expect(listCount(empty.packageLevels)).toBe("0");
    expect(listCount(empty.partnerTargets)).toBe("0");
  });
  it("rejects malformed operational data instead of rendering a default dashboard", () => {
    ...
    expect(() =>
      launchWorkspaceModel({ metadata: { packageExecution: { anchorScopeMatrix: [null] } } })
    ).toThrow();
  });
  it("counts explicit task statuses rather than stale board totals", () => {
    const model = launchWorkspaceModel({
      metadata: {
        launchBoard: {
          completedCount: 17,
          tasks: [{ status: "completed" }, { status: "blocked" }, { title: "No status" }],
        },
      },
    });
  ...
  it("preserves every primary/backup/incentive category entry", () => {
    const entries = collectPartnerTargets([
      { lane: "one", slug: "company", backupSlug: "backup", incentiveSlug: "incentive" },
      { lane: "two", slug: "company" },
    ]);
    expect(entries).toHaveLength(4);
    expect(entries?.filter((item) => item.slug === "company")).toHaveLength(2);
  });
  it.each([undefined, null, "", false, -1, NaN])(
    "does not fabricate a planning example from %s",
    (value) => {
  ...
  it("preserves real zeros rather than substituting example values", () => {
    expect(savedCount(0)).toBe("0");
    expect(savedCount(null)).toBe("Not recorded");
