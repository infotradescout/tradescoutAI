...
function render(options: { missingAddress?: boolean; failedProjects?: boolean; wrongViewer?: boolean; noHome?: boolean } = {}) {
  ...
  client.setQueryData(["/api/homes", id, "overview", viewer], { home, records: [
  ...
  client.setQueryData(["/api/homes", id, "projects", viewer], [{ id: "project-a", title: "Kitchen renovation", status: "planning", description: "Replace cabinets" }]);
  client.setQueryData(["/api/homes", id, "maintenance-schedules", viewer], []);
  client.setQueryData(["/api/homeid", id, "persistence", viewer], { propertyDetails: [{ id: "detail", status: "needs_review", note: "Check dimensions" }], components: [], requestPackets: [], evidence: [{ id: "ref" }] });
  if (options.failedProjects) client.getQueryCache().find({ queryKey: ["/api/homes", id, "projects", viewer] })!.setState({ status: "error", error: new Error("Unavailable") });
...
describe("actual HomeOverview render", () => {
  ...
  it("treats a missing address as missing information, not an invented construction stage", () => {
    ...
    expect(html).toContain("Add the property address");
