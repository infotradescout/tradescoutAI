...
export function readRecordDetail(value: unknown, homeId: string) {
  ...
  if (object(data.home).id !== homeId)
    throw new Error("The selected property could not be loaded.");
  return {
    home: data.home as HomeSummary,
    records: rows<HomeRecord>(data, "records"),
    documents: rows<HomeDocument>(data, "documents"),
    appliances: rows<HomeAppliance>(data, "appliances"),
  };
}
export function readRecordPersistence(value: unknown): RecordPersistence {
  ...
  if (
    propertyDetails.some(
      (item) =>
        ...
        !["known", "needs_review"].includes(item.status)
    )
  ) {
  ...
  if (
    requestPackets.some(
      (item) =>
        typeof item.id !== "string" ||
        !Array.isArray(item.selectedDetailIds) ||
        !Array.isArray(item.missingHelpfulInfo)
    )
  ) {
  ...
  return {
    propertyDetails,
    requestPackets,
    components: rows<HomeSystem>(data, "components"),
    evidence: rows<HomeEvidence>(data, "evidence"),
  };
}
...
export function selectRecordProject(
  projects: HomeProject[],
  requestedId: string | null
): HomeProject | null {
  return requestedId
    ? projects.find((item) => item.id === requestedId) || null
    : projects[0] || null;
}
...
export function recordedMissingInputs(
  project: HomeProject | null,
  packets: HomeIdRequestPacket[]
): string[] {
  return [
    ...new Set([
      ...textList(object(project?.metadata).requiredNextInputs),
      ...packets.flatMap((item) => textList(item.missingHelpfulInfo)),
    ]),
  ];
}
...
export function groupedSystems(components: HomeSystem[]) {
  const groups = SYSTEM_GROUPS.map(([name, types]) => ({
    name,
    items: components.filter((item) => (types as readonly string[]).includes(item.type)),
  }));
  ...
  if (other.length)
    groups.push({ name: "Other systems" as (typeof groups)[number]["name"], items: other });
  return groups;
}
