...
export const LAUNCH_TABS = [
  { id: "control", label: "Launch Control" }, { id: "scope", label: "Scope Matrix" },
  { id: "packages", label: "Package Levels" }, { id: "partners", label: "Partner Pipeline" },
  { id: "evidence", label: "Documents & references" }, { id: "release", label: "Release Gates" },
] as const;
...
function optionalObjects(value: unknown, name: string): SavedFields[] | null {
  ...
  if (!Array.isArray(value) || value.some((item) => !item || typeof item !== "object" || Array.isArray(item))) throw new Error(`Saved ${name} could not be read.`);
  return value as SavedFields[];
}
function optionalText(value: unknown, name: string): string[] | null {
  if (value === undefined || value === null) return null;
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) throw new Error(`Saved ${name} could not be read.`);
  return value;
}
...
export function collectPartnerTargets(primaryTargets: SavedFields[] | null): PartnerTarget[] | null {
  ...
  for (const item of primaryTargets) {
    ...
    for (const [key, role] of [["slug", "Primary"], ["backupSlug", "Backup"], ["incentiveSlug", "Incentive"]]) {
      const slug = item[key];
...
export function launchWorkspaceModel(project: HomeProject) {
  ...
  return {
    metadata, launchBoard, launchTasks, partnerPipeline, primaryTargets,
    partnerTargets: collectPartnerTargets(primaryTargets), packageExecution, scopeMatrix, packageLevels, executionSteps,
    currentCoverage: object(metadata.currentCoverage), commercialChecklists: object(metadata.commercialTermsChecklists),
    sourcePlan, coverageTargets: object(sourcePlan.commissionableCoverage), economics: object(sourcePlan.planningEconomicsExample),
    spaceExample: object(sourcePlan.mechanicalSpaceExample), sourceFiles: optionalObjects(metadata.sourceFilesUsed, "source files"),
    ...
    screeningTemplate: object(metadata.destinationScreeningTemplate), quoteTemplate: object(metadata.firstPackageQuoteTemplate),
    handoffTemplate: object(metadata.builderHandoffTemplate), ownershipTemplate: object(metadata.ownershipActivationTemplate),
...
export function savedCount(value: unknown): string {
  return typeof value === "number" && Number.isInteger(value) && value >= 0 ? String(value) : "Not recorded";
}
export function listCount(value: unknown[] | null): string { return value === null ? "Not recorded" : String(value.length); }
export function taskStatusCount(tasks: SavedFields[] | null, statuses: string[]): string {
  return tasks === null ? "Not recorded" : String(tasks.filter((item) => typeof item.status === "string" && statuses.includes(item.status.toLowerCase())).length);
}
...
export function recordedNumber(value: unknown, format: "currency" | "percent" | "area"): string {
  if (typeof value !== "number" && (typeof value !== "string" || !value.trim())) return "Not recorded";
  const amount = Number(value);
  if (!Number.isFinite(amount) || amount < 0) return "Not recorded";
  if (format === "currency") return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(amount);
