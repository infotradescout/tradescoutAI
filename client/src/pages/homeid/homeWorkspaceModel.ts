...
export type HomeDetailResponse = {
  home: HomeSummary;
  records: SavedRecord[];
  documents: SavedDocument[];
};
export type HomePersistenceResponse = {
  propertyDetails: SavedDetail[];
...
export function humanLabel(value?: string | null): string {
  return (
    value
      ?.trim()
      .replace(/[_-]+/g, " ")
      .replace(/^./, (letter) => letter.toUpperCase()) || "Not recorded"
  );
}
...
export function resolveHomeView(search: string, selectedHomeId: string | null): HomeView {
  ...
  const requestedLaunch =
    query.get("workspace") === "launch" ||
    (launchTab != null &&
      ["control", "scope", "packages", "partners", "evidence", "release"].includes(launchTab));
  if (selectedHomeId === PACKAGE_HOME_ID && requestedLaunch) return "launch";
  const tab = query.get("tab");
  if (HOME_SECTIONS.some((section) => section.id === tab && section.id !== "overview"))
    return "record";
  return "overview";
}
...
export function readHomeDetail(value: unknown, requestedId: string): HomeDetailResponse {
  ...
  return {
    home: home as HomeSummary,
    records: collection(data, "records"),
...
export function readPersistence(value: unknown): HomePersistenceResponse {
  ...
  return {
    propertyDetails: collection(data, "propertyDetails"),
    requestPackets: collection(data, "requestPackets"),
    components: collection(data, "components"),
    evidence: collection(data, "evidence"),
  };
}
...
export function dueMaintenance(items: SavedSchedule[], now: Date): SavedSchedule[] {
  ...
  return items
    .filter((item) => {
      ...
      const due = new Date(
        /^\d{4}-\d{2}-\d{2}$/.test(item.nextDueAt) ? `${item.nextDueAt}T12:00:00` : item.nextDueAt
      );
...
export function recentRecords(items: SavedRecord[]): SavedRecord[] {
  return items
    .filter((item) => !item.title?.startsWith("homeid:"))
    .slice()
    .sort((a, b) => {
      const stamp = (item: SavedRecord) => {
        const n = new Date(item.occurredAt || item.createdAt || "").getTime();
