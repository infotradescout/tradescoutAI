...
vi.mock("@/lib/queryClient", () => ({
  apiRequest: vi.fn(() => {
    throw new Error("Unexpected render-time IO");
  }),
}));
vi.mock("@/hooks/useAuth", () => ({
  useAuth: () => ({ user: { id: "viewer" }, isAuthenticated: true, isLoading: false }),
}));
vi.mock("@/lib/privateObjectUpload", () => ({ uploadPrivateObject: vi.fn() }));
vi.mock("@/lib/firstUseAnalytics", () => ({
  trackFirstUseGuidanceViewed: vi.fn(),
  trackFirstUseTaskPromptClicked: vi.fn(),
  trackFirstUseTaskPromptViewed: vi.fn(),
}));
function render(child: React.ReactNode) {
  ...
  const html = renderToStaticMarkup(
    <QueryClientProvider client={client}>
      <Router ssrPath="/homes">{child}</Router>
    </QueryClientProvider>
  );
  client.clear();
  return html;
...
describe("real record-screen rendering", () => {
  it("keeps an empty record neutral rather than declaring readiness or supplier coverage", () => {
    ...
    for (const invented of [
      "Ready for next gate",
      "No major planning gaps detected",
      "Preconstruction",
  ...
  it("renders recorded system status even for stone and cabinets and keeps custom categories", () => {
    const html = render(
      <Systems
        components={[
          { id: "stone", type: "natural_stone", label: "Stone selection", status: "needs_review" },
          { id: "solar", type: "solar", label: "Solar array", status: "unknown" },
        ]}
      />
    );
    expect(html).toContain("Stone selection");
    expect(html).toContain("Needs review");
    expect(html).toContain("Solar array");
    expect(html).toContain("Other systems");
  ...
  it("renders an authenticated stored-document action without treating an evidence URL as stored proof", () => {
    const html = render(
      <Documents
        homeId="home-one"
        documents={[{ id: "document-one", originalName: "permit.pdf" }]}
        evidence={[
          {
            id: "reference",
            title: "Reference",
    ...
    expect(html).toContain('aria-label="Open permit.pdf"');
    expect(html).toContain("Open / download");
    expect(html).toContain("Open file link");
    expect(html).toContain("not proof that the original file is stored");
  ...
  it("keeps stored documents usable while a separate reference load is failing", () => {
    const html = render(
      <Documents
        homeId="h"
        documents={[{ id: "d", originalName: "plan.pdf" }]}
        evidence={[]}
        referencesQuery={{ ...loaded, isError: true }}
        fileRef={React.createRef()}
        docType="other"
        setDocType={noop}
        docFile={null}
    ...
    expect(html).toContain("Open plan.pdf");
    expect(html).toContain("References could not be loaded");
    expect(html).toContain("Retry references");
  ...
  it("shows a missing selected project instead of a different project's details", () => {
    const html = render(
      <Build
        ...
        projectId="missing"
        project={null}
        projects={[{ id: "real", title: "Other project" }]}
        missing={[]}
        openProperty={noop}
        selectProject={noop}
      />
    );
    expect(html).toContain("This project is not available in this property");
    expect(html).toContain("No other project has been substituted");
  });
  it("retains the project ID when opening its build timeline", () => {
    ...
    expect(html).toContain("projectId=selected-project");
    expect(html).toContain("Open Build Timeline");
  ...
  it("keeps the sale path and treats preparation steps as guidance, not completed work", () => {
    const html = render(
      <Sale homeId="home" openProperty={noop} openDocuments={noop} openRequest={noop} />
    );
    expect(html).toContain("/homescout/new?homeId=home");
    expect(html).toContain("not completed tasks");
...
describe("real package-planning rendering", () => {
  it.each(LAUNCH_TABS)("retains $label without preset operational numbers", ({ id }) => {
    ...
    const html = render(
      <LaunchContent
        home={{ id: PACKAGE_HOME_ID, nickname: "Saved package home" }}
        project={project}
        model={launchWorkspaceModel(project)}
    ...
    expect(html).toContain("Saved package home");
    expect(html).toContain("Not recorded");
    expect(html).toContain("Open full property passport");
    expect(html).toContain("Open saved scope request");
    expect(html).not.toContain("$13,600");
    expect(html).not.toContain("Seventeen first-90-day");
    expect(html).not.toContain("18-line anchor");
    expect(html).not.toContain("22 source targets");
  });
  it("keeps every template item available, including items after the eighth", () => {
    const html = render(
      <Fields
        value={{
          requiredItems: Array.from({ length: 12 }, (_, index) => `Required item ${index + 1}`),
        }}
      />
    );
    expect(html).toContain("Required item 12");
    expect(html).not.toContain("more required items");
  });
  it("renders actual scope length and preserves additional line data", () => {
      ...
      metadata: {
        packageExecution: {
          anchorScopeMatrix: [
            ...
            { label: "Structure", status: "included" },
          ],
        },
      },
    };
    ...
    expect(html).toContain("2-line anchor metal-building scope matrix");
    expect(html).toContain("Stored responsibility");
    expect(html).not.toContain("18-line anchor");
  });
});
