...
describe("property identity editing", () => {
  ...
  it("only sends changed fields and makes clearing deliberate", () => {
    ...
    expect(
      homeIdentityChanges(identity, {
        ...draft,
        nickname: " New property name ",
        address1: "",
        yearBuilt: "",
      })
    ).toEqual({ nickname: "New property name", address1: null, yearBuilt: null });
    expect(homeIdentityChanges(identity, { ...draft, stateCode: "fl" })).toEqual({});
  });
  it.each(["ownerUserId", "id", "verified", "status", "createdByUserId", "sharing", "authority"])(
    "rejects authority/mass assignment through %s",
    (field) => {
  ...
  ])("rejects malformed changed fields %j", (changes) => {
  ...
  it("accepts known types, nullable fields, and ZIP+4 without adding a verification claim", () => {
    for (const [propertyType] of HOME_IDENTITY_TYPES)
      expect(update({ propertyType }).success).toBe(true);
    expect(update({ yearBuilt: null, zipCode: "32501-1234", stateCode: "fl" }).success).toBe(true);
    expect(
      homeIdentityUpdateSchema.parse({ revision, changes: { stateCode: "fl" } }).changes.stateCode
    ).toBe("FL");
  });
  it("requires the original revision and a nonempty patch", () => {
    expect(homeIdentityUpdateSchema.safeParse({ changes: { nickname: "Name" } }).success).toBe(
      false
    );
    expect(homeIdentityUpdateSchema.safeParse({ revision, changes: {}, extra: true }).success).toBe(
      false
    );
    expect(update({}).success).toBe(false);
  });
  it("matches the requested record exactly and handles legacy read data separately from edits", () => {
    expect(parseHomeIdentitySnapshot({ identity, revision }, identity.id)).toEqual({
      identity,
      revision,
    });
    expect(() => parseHomeIdentitySnapshot({ identity, revision }, "other-property")).toThrow();
    expect(() =>
      parseHomeIdentitySnapshot({ identity: { ...identity, city: {} }, revision }, identity.id)
    ).toThrow();
    expect(
      parseHomeIdentitySnapshot(
        {
          identity: { ...identity, zipCode: "legacy", propertyType: "older-custom-type" },
          revision,
        },
        identity.id
      ).identity.propertyType
    ).toBe("older-custom-type");
