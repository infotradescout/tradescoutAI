...
describe("JW Stone 2.0 marketplace journey", () => {
  ...
  it("puts First Cut before discovery and offers the complete inventory without reused cover stones", () => {
    act(() => root.render(<JwStoneMarketplacePage />));

    const firstCut = container.querySelector("#first-cut");
    const discovery = container.querySelector("#discover");
    expect(firstCut).not.toBeNull();
    expect(discovery).not.toBeNull();
    if (!firstCut || !discovery) throw new Error("Expected First Cut and discovery sections");
    expect(container.querySelector(".jw2-hero a")?.getAttribute("href")).toBe("#first-cut");
    expect(
      firstCut.compareDocumentPosition(discovery) & Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy();

    click(buttonContaining("Designer Selection Board", container));
    expect(buttonContaining("All current selections", container)).toBeDefined();
    expect(container.querySelectorAll(".jw2-color-choice img")).toHaveLength(0);
    click(buttonContaining("All current selections", container));

    expect(container.querySelectorAll(".jw2-designer-card, .jw2-trending-card")).toHaveLength(119);
    expect(
      Array.from(container.querySelectorAll("h3")).filter(
        (heading) => heading.textContent?.trim() === "Panda"
      )
    ).toHaveLength(1);
    expect(routeMock.navigate).toHaveBeenLastCalledWith("/jw-stone?buyer=designer&color=all", {
      replace: true,
    });

    const options = Array.from(container.querySelectorAll(".jw2-filter option"));
    expect(options.length).toBeGreaterThan(2);
    expect(options.every((option) => Boolean(option.textContent?.trim()))).toBe(true);
  });
