...
export default function NationwideExpansion() {
  ...
  const { data: metrics, isLoading: metricsLoading, isError: metricsError } = useQuery<NationwideMetrics | undefined>({
  ...
  const { data: topCounties = [], isLoading: countiesLoading, isError: countiesError } = useQuery<TopCounty[]>({
  ...
  const { data: expansionPipeline = [], isLoading: pipelineLoading, isError: pipelineError } = useQuery<ExpansionRecord[]>({
  ...
  const { data: foundationImpact, isLoading: impactLoading, isError: impactError } = useQuery<{
    totalRaised: number;
    totalDonors: number;
    activeCauses: number;
  ...
  const { isLoading: affiliateLoading, isError: affiliateError } = useQuery<{ message?: string } | undefined>({
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-slate-300 max-w-3xl mx-auto">
            This page reflects TradeScout's current nationwide footprint and community impact based on real activity.
          </p>
        </div>
        ...
        {metrics && !metricsError ? (
            ...
            >
              <CardContent className="p-4 text-center">
                ...
                {typeof metrics.totalProjects === "number" ? (
                  <div className="text-xl font-bold text-white">{metrics.totalProjects.toLocaleString()}</div>
                ) : (
          ...
          <div className="border border-slate-700 rounded-lg p-4 text-center text-slate-400" style={{ backgroundColor: "var(--surface-card)" }}>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
                  ...
                  {metrics && !metricsError ? (
                    <div className="space-y-2 text-sm text-slate-300">
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        ...
                        {typeof metrics.totalContractors === "number" && (
                          <li>{metrics.totalContractors.toLocaleString()} contractors with profiles</li>
                        )}
                        {typeof metrics.totalProjects === "number" && (
                          <li>{metrics.totalProjects.toLocaleString()} projects recorded in the system</li>
                        )}
                      </ul>
                    </div>
                  ) : (
          ...
          <TabsContent value="counties" className="space-y-6">
            <div className="space-y-4">
              ...
              {countiesLoading && (
                <p className="text-sm text-slate-400">Loading county activity…</p>
              )}
              {!countiesLoading && (countiesError || !topCounties.length) && (
                <p className="text-sm text-slate-400">
                  County-level contractor activity is not available right now. No rankings are shown.
                </p>
              )}
                  ...
                  >
                    <CardHeader className="pb-3">
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          <CardTitle className="text-white text-lg">County FIPS {county.fips}</CardTitle>
                        ...
                        <div className="text-right">
                          <div className="text-xl font-bold text-yellow-400">{county.avgRating.toFixed(1)}</div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-center">
                        <div className="space-y-1">
                          <div className="text-lg font-bold text-orange-400">{county.contractorCount}</div>
          ...
          <TabsContent value="expansion" className="space-y-6">
            <div className="space-y-6">
              ...
              {!pipelineLoading && !pipelineError && expansionPipeline.length > 0 && expansionPipeline.map((phase, index) => (
                <Card
                  key={index}
                  className="border-slate-700"
                  style={{ backgroundColor: "var(--surface-card)" }}
                ...
                >
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        <CardTitle className="text-white">{phase.county}, {phase.state}</CardTitle>
                        <Badge
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-purple-400">{(phase.population || 0).toLocaleString()}</div>
                  ...
                  <CardContent className="space-y-4">
                    <p className="text-slate-300">
                      This county appears in TradeScout's internal expansion planning records. Status and priority
                      reflect planning state only and are not guarantees of launch timing.
                    </p>
                  </CardContent>
                </Card>
              ))}
                ...
                {!impactLoading && !impactError && foundationImpact && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-pink-400 font-semibold">${foundationImpact.totalRaised.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Total Donors</span>
                      <span className="text-blue-400 font-semibold">{foundationImpact.totalDonors.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Active Causes</span>
                      <span className="text-green-400 font-semibold">{foundationImpact.activeCauses.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Counties Supported</span>
                      <span className="text-orange-400 font-semibold">{foundationImpact.countiesSupported.toLocaleString()}</span>
                    </div>
                  </div>
                )}
              </CardContent>
          ...
          <TabsContent value="affiliates" className="space-y-6">
            ...
            >
              ...
              <CardContent>
                ...
                {!affiliateLoading && (
                  <p className="text-sm text-slate-400">
                    Affiliate performance data is not available yet. No projections or earnings estimates are shown.
                  </p>
                )}
              </CardContent>
            </Card>
