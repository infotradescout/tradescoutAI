...
describe("reset password location params", () => {
  it("reads a prefilled email from the browser query", () => {
    expect(
      readResetPasswordParam(
        "email",
        "?email=profile-account%40example.test"
      )
    ).toBe("profile-account@example.test");
  });
  ...
  it("preserves a nested JW Stone profile-account continuation", () => {
    expect(
      readResetPasswordParam(
        "next",
        "?next=%2Fjw-stone%3FprofileAccount%3D1"
      )
    ).toBe("/jw-stone?profileAccount=1");
  });
