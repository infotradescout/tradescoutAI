...
import {
  ...
  resolveHomeView,
  type HomeSummary,
} from "./homeid/homeWorkspaceModel";
import "./homeid/HomeOverview.css";

const HomeIdWorkspace = lazy(() => import("./homeid/HomeIdWorkspace"));
const PropertyBlessingsLaunchWorkspace = lazy(
  () => import("./homeid/PropertyBlessingsLaunchWorkspace")
);
...
export default function Homes() {
  ...
  if (!isAuthenticated) {
    if (isLoading)
      return (
        <div className="ts-home-overview">
          <p role="status">Loading your property workspace…</p>
        </div>
      );
    return (
      <section className="ts-home-overview">
        <h1>Your property workspace</h1>
        <p>Sign in to view your private property records.</p>
        <Link
  ...
  if (view !== "overview") {
    return (
      <div className="ts-home-record-frame">
        <Link className="home-return-link" href={homeHref(selectedHomeId)}>
          ← Property overview
        </Link>
        <Suspense fallback={<p role="status">Loading the property tools…</p>}>
          {view === "launch" ? (
            <PropertyBlessingsLaunchWorkspace key={user!.id} />
          ) : (
            <HomeIdWorkspace key={`${user!.id}:${selectedHomeId || "new"}`} />
          )}
        </Suspense>
      </div>
    );
