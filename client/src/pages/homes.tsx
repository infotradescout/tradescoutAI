...
export default function Homes() {
  ...
  const homesQuery = useQuery({
    queryKey: ["/api/homes", user?.id], enabled: isAuthenticated,
  ...
  if (view !== "overview") {
    return <div className="ts-home-record-frame">
  ...
  return <HomeOverview key={`${user!.id}:${selectedHomeId || "none"}`} viewerId={user!.id} homeId={selectedHomeId} homes={homes}
    homesPending={homesQuery.isPending} homesError={homesQuery.isError} retryHomes={() => void homesQuery.refetch()}
