...
export default function AdminEcosystemTruthPage() {
  ...
  const termsNeedingEvidence = report?.summary.commercialRecordsNeedingEvidence;
  const termsEvidenceKnown =
    reportReady && termsNeedingEvidence !== null && termsNeedingEvidence !== undefined;
  const unlinkedOutcomes = report?.summary.unlinkedOutcomeEvents;
  const outcomeLinksKnown =
    reportReady && unlinkedOutcomes !== null && unlinkedOutcomes !== undefined;
        ...
        <AdminSummaryStrip
          items={[
            ...
            },
            {
              ...
              value: termsEvidenceKnown ? termsNeedingEvidence : "—",
              detail: termsEvidenceKnown
                ? "Not proven as signed terms"
                : reportReady
                  ? "Commercial coverage incomplete"
                  : "Checking commercial records",
              tone:
                termsEvidenceKnown && Number(termsNeedingEvidence) === 0
                  ? "good"
                  : "warning",
            },
            {
              label: "Unlinked outcomes",
              value: outcomeLinksKnown ? unlinkedOutcomes : "—",
              detail: outcomeLinksKnown
                ? "Missing parent or cross-system reference"
                : reportReady
                  ? "Outcome sources unavailable"
                  : "Checking evidence links",
              tone:
                outcomeLinksKnown && Number(unlinkedOutcomes) === 0
                  ? "good"
                  : "danger",
            },
          ]}
      ...
      {reportQuery.isLoading ? (
        ...
        <>
          ...
          <AdminSection
            ...
            description={
              report.commercialTerms.recordsReviewed === null
                ? "One or more commercial sources are unavailable. No zero totals were invented."
                : `${report.commercialTerms.recordsReviewed} record(s) reviewed · ${report.commercialTerms.needsEvidence ?? "unknown"} need more evidence · ${report.commercialTerms.conflicts ?? "unknown"} active conflict group(s).`
            }
            className="pt-0"
          >
                ...
                >
                  ...
                  <p className="text-sm text-white/52">
                    {source.recordCount === null
                      ? "Counts unavailable"
                      : `${source.linkedCount ?? 0} linked · ${source.unlinkedCount ?? 0} unlinked · ${source.recordCount} total`}
                  </p>
