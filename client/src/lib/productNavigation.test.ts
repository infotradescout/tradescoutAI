...
describe("TradeScout product navigation", () => {
  ...
  it.each([
    ...
    expect(
      PRODUCT_NAV_ITEMS.filter((item) => isProductNavItemActive(item, route)).map((item) => item.id)
    ).toEqual([id]);
  });

  it.each(["/direct-connector", "/community-builder", "/scout-info", "/not-a-tool"])(
    "does not activate a similarly named but unrelated route: %s",
    (route) => expect(getActiveProductNavItem(route)).toBeUndefined()
  );

  it("keeps the desktop parent selected for nested tools but never also selects Requests on Jobs", () => {
    const primary = PRODUCT_NAV_ITEMS.filter((item) =>
      (DESKTOP_PRODUCT_NAV_IDS as readonly string[]).includes(item.id)
    );
  ...
  it("finds tools by ordinary language, aliases, and mixed case without losing destinations", () => {
    ...
    expect(searchProductNavigation("financial reports").map((item) => item.id)).toContain(
      "reports"
    );
