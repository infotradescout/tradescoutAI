...
The current integration and acceptance order are in [Receiving integration release](RECEIVING_INTEGRATION_RELEASE.md). The verification ledger below records earlier branch work and does not attest the combined release.
...
Received lots carry explicit lot prices. Arrival inquiries remain available. The integrated production quote cart reviews these receipt prices without substituting an unrelated catalog-name rate. Existing catalog/workbook pricing is not overwritten. Payment checkout remains outside this implementation.
...
The earlier branch introduced a GitHub Actions workflow. It was removed during integration because current TradeScout governance does not use Actions. The actual repository release contract and executed native/browser checks remain required.
