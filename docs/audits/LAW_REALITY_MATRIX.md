...
| JW Stone private offers preserve gated contact and isolated customer identity | enforced | `server/routes/jw-stone-express.ts`, `server/routes/admin-jw-stone-offers.ts`, `server/tests/jw-stone-private-offers.integration.test.ts` | JW Express accounts do not attach to TradeScout users; unverified offers stay out of the operator queue; contact remains masked until a distinct audited review action. Accepting an offer does not reveal contact or create payment, reservation, title transfer, or a binding sale. |
...
| JW Stone container ordering is private inventory allocation, not paid platform visibility | enforced | `server/routes/admin-jw-stone-offers.ts`, `docs/jw-stone/PRIVATE_OFFERS_AND_CONTAINERS.md` | Eligible private container offers are ordered server-side by amount, submission time, and immutable ID. No customer sees rank, competing offers, or purchased visibility, and a lower-priority offer cannot be accepted while a higher eligible offer remains. |
