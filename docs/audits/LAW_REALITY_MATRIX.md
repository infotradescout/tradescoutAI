...
| Louisiana Stone Solutions direct-link release requires administrative source authority and actual company mailbox control; it grants no discovery, map ranking, indexing, credential badge, or contact access | enforced | `shared/publicProfileExposureRegistry.ts`, `server/services/ownerConfirmedDirectProfile.ts`, `server/tests/louisiana-stone-profile-exposure.test.ts` | Exact slug only; published profile, active linked business, matching claimed owner, local account, verified email, pending/approved verification state, explicit public profile selection, and discovery disabled. Missing or revoked authority fails closed through the canonical exposure decision. Existing request consent and private inbox controls remain in force. |
