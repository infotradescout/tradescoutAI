...
| Admin/UI never computes intelligence; jobs precompute snapshots | PARTIAL | `server/routes/admin.ts:506`, `server/routes.ts:1147`, `server/routes.ts:1209` | Some APIs still compute indices on read from live tables (`scout_interactions` + `county_metrics`). |
