...
### September 6 follow-up: address evidence and review

The subsequent checkpoint repairs the address failures identified above. The member form now accepts a private PDF, JPG, or PNG with an address, shows pending/rejected/verified states, preserves the saved request destination, and keeps failed submissions available for retry. Postcard and phone options remain visible as unavailable. Both legacy postcard endpoints return an unavailable response without generating, logging, redeeming, or claiming delivery of a code. The page no longer equates address verification with unrestricted platform access.

The old creation schema required caller-supplied account/deadline fields, while the old update handler spread arbitrary member input into the verification record. A separate strict input schema now accepts only address/document fields. The server owns account identity, deadline, submission state, review fields, and verification flags. Submission and review lock the account before the current verification. Review rejects stale timestamps and superseded records, and updates the decision and account flag in one transaction. The separate manual administrative onboarding exception is unchanged.

Independent same-provider Objector review found two additional concrete failures in the first address patch: a member could replace uploaded bytes after an admin viewed them, and reviewing an older duplicate record could change the current account flag. Both findings were sustained and repaired. Submission now reads bounded actual uploaded bytes and writes a new server-controlled evidence copy under a namespace that the member upload routes cannot address. Approval and authenticated admin download use and check that copy. Admin review resolves the canonical latest verification under the account lock. The reviewer closed both findings in source; this is an independent execution review, not provider-independent assurance.

Privacy classification: **enforced in source and controlled behavior tests** — member input cannot set authority fields, owner status excludes codes/private keys/internal notes, the admin queue projects only the required account fields, review/download are behind existing authentication and administrative guards, and evidence cannot be replaced through member upload paths. **policy_target** — native database concurrency, live storage behavior, authenticated browser accessibility/journeys, and real delivery remain unproved in this environment. No production data or real member documents were used.

Focused verification passes 57 tests across six files: 29 registered-handler behavior cases, five evidence cases, six member-form cases, three admin-review cases, and 14 existing business-verification/admin contracts. The evidence tests use real disposable local files plus a controlled R2 SDK transport. They exercise original-upload replacement, missing/invalid/oversized evidence, immutable-path ownership, and saved-copy availability. Handler transactions use a controlled database boundary; their rollback/ordering assertions do not prove PostgreSQL concurrency. A failed database transaction can leave an unreferenced private evidence copy; lifecycle cleanup remains follow-up work and no destructive cleanup was introduced.

The preceding request checkpoint also closes the owned-draft recovery finding: a signed-out or unrelated account cannot hydrate an account-owned shared recovery alias; the correct owner can resume after completing a missing county. The six composer behavior cases pass. Fresh full-check commands, actual skip counts, exact saved head/tree, and remaining database/browser blockers are recorded in draft PR #561 after this checkpoint is saved.

The first full check on address checkpoint `acdbf57e13e399e3856bdc4c7f7d2d932db4188d` stopped at the existing architecture guard: the changes exceeded the two route-file size ceilings. The next checkpoint extracts all eight address registrations into `server/routes/address-verification.ts` and moves the contact response implementation into its existing `direct-connect/authority.ts` owner. Authentication, rate limiting, registration order, handler bodies, and dependency owners are preserved; the ceilings are unchanged. A TypeScript-normalized comparison confirms all eight address registrations and the contact handler match the prior checkpoint, allowing only the injected database variable rename. The member/admin behavior tests now read the extracted address owner and verify its single bootstrap registration. Three further behavior cases execute the extracted contact HTTP handler, including anonymous/unrelated denial before receipt reads. The architecture guard passes with route files at 991,832 and 406,119 LF-normalized bytes, below their original limits.
