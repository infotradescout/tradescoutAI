...
The native tests exercise real storage, advisory locks, registered Passport verification callbacks, Express routes, session regeneration, same-origin return handling, and denial of account collisions. The HTTP harness replaces only the provider's token/profile response. Real Google/Facebook token exchange, production credentials, provider-console configuration, deployed browser behavior, and production account reconciliation require separate evidence.
