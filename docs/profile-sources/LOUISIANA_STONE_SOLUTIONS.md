...
Release preparation also fixes three existing TypeScript filter-narrowing errors without changing emitted behavior and replaces the undeclared development-only `nanoid` import with Node's built-in `randomUUID`. The integrated main branch preserves the canonical sitemap's final newline during generation. Verified unchanged public assets and the unchanged profile route source were normalized to canonical Git LF bytes to satisfy the existing strict byte and source guards; those guards were not weakened. Three existing IndexNow tests fail because the baseline publishes an additional unnamed gallery image. These represent a publication-boundary defect tracked in separate PR #574, not merely stale test expectations; this profile change does not repair or conceal them.

The release runner also loads the same database-security helper from either the isolated production image or the normal source checkout, with bundled-helper precedence and fatal import errors preserved. All 24 runner, TLS, and runtime-boundary tests passed, with independent security review. Tests simulate the production file layout; no container was run.

The initial minimum release run against commit `79eb608f980915606a5311925113f8b27f9d1f5d` passed types, build, 152 contracts, and 15 discovery tests, then stopped at the historical `0126` JW fixture precondition in `tradescout_test`. A resumed release run uses one new isolated disposable database with synthetic fixtures and the unchanged canonical migration chain; existing test and production records are preserved. It also includes the exact forward-only profile-account identity repair from PR #574 as migration `0129`, followed by `0130` for business-managed contact and neutral owner control. The final exact-commit gate and production outcome are recorded on PR #573.
