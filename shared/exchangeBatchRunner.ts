...
export type BatchResult = {
  key: string;
  status: "submitted" | "skipped" | "upload_failed" | "needs_check" | "stopped";
  message: string;
  listingId?: string;
};
...
export async function runExchangeBatch(options: BatchRunnerOptions): Promise<BatchResult[]> {
  if (options.rows.some((row) => row.errors.length))
    throw new Error("Fix all row errors before importing.");
  ...
  for (const row of options.rows) {
    ...
    if (previous?.status === "submitted") {
      report({
        ...
        message: "Already submitted from this browser; not submitted again.",
      });
      continue;
    }
    if (previous?.status === "submitting") {
      report({
        ...
        status: "needs_check",
        message:
          "A previous submission has an unconfirmed result. Check My Listings before trying this ID again.",
      });
      break;
    }
    const images: string[] = [];
      ...
      report({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Photo upload failed. No listing was submitted for this row.",
      });
    ...
    try {
      ...
      if (typeof candidate !== "string" || !candidate.trim())
        throw new Error(
          "The server did not return a listing ID. A review or verification step may be required."
        );
      ...
      report({
        ...
        status: "submitted",
        listingId: candidate,
        message: "Submitted to Exchange. Normal review and visibility rules apply.",
      });
    } catch (error) {
      report({
        key: row.key,
        status: "needs_check",
        message: `${error instanceof Error ? error.message : "Submission response could not be confirmed."} Check My Listings before retrying; this row will not be automatically submitted again.`,
      });
      break;
    }
  }
