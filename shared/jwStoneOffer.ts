...
export const jwStoneOfferInputSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.scope === "stone" && value.selection.lines.length !== 1) {
      context.addIssue({
        ...
        path: ["selection", "lines"],
        message: "Choose one stock selection for a stone offer.",
      });
    }
  });
...
export const JW_STONE_OFFER_TERMS =
  "This is a material-price offer, not an order or reservation. Tax and delivery are separate. No payment is accepted until JW Stone confirms the offer and final payable total.";
