...
export function parseJwStoneReceipt(value: unknown): JwStoneReceipt {
  ...
  const text = (key: string, max: number, optional = false) => {
    ...
    if ((!s && !optional) || s.length > max || /[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(s))
      throw new JwStoneReceivingInputError(`Check ${key}.`);
    return s;
  };
  const number = (
    key: string,
    ...
    if (
      typeof n !== "number" ||
      !Number.isFinite(n) ||
      n < (allowZero ? 0 : Number.MIN_VALUE) ||
      n > max ||
  ...
  if (!UUID.test(receiptId))
    throw new JwStoneReceivingInputError("Invalid receiving identifier. Start a new arrival.");
  const result: JwStoneReceipt = {
    receiptId,
    materialName: text("materialName", 160),
    materialFamily: text("materialFamily", 80),
    materialClass: oneOf("materialClass", ["natural_stone", "engineered_stone"]),
    lotLabel: text("lotLabel", 80),
    quantity: number("quantity", 10000, true)!,
    length: number("length", 10000)!,
    height: number("height", 10000)!,
    dimensionUnit: oneOf("dimensionUnit", ["in", "mm"]),
    thicknessMm: number("thicknessMm", 1000)!,
    finish: text("finish", 80),
    locationLabel: text("locationLabel", 160),
    ...
    landedCostCents: number("landedCostCents", 10000000, true, true, true),
    notes: text("notes", 2000, true),
  };
  if (Object.keys(v).some((key) => !Object.prototype.hasOwnProperty.call(result, key)))
    throw new JwStoneReceivingInputError("Unexpected receiving field.");
  if ((result.bundlePriceCents === null) !== (result.bundleMinSlabs === null))
    throw new JwStoneReceivingInputError(
      "Enter both the bundle rate and minimum slab quantity, or leave both blank."
    );
  if (!jwStoneReceivingMaterialSlug(result.materialName))
    throw new JwStoneReceivingInputError("Enter a material name containing letters or numbers.");
  return result;
}
...
export function jwStoneReceiptDimensions(receipt: JwStoneReceipt) {
  ...
  return {
    length: Math.round(receipt.length * scale * 1000) / 1000,
    height: Math.round(receipt.height * scale * 1000) / 1000,
    thickness: receipt.thicknessMm,
    unit: "mm" as const,
  };
}
export function jwStoneReceiptMemberPrice(receipt: JwStoneReceipt) {
  return {
    publicId: jwStoneReceiptPublicId(receipt.receiptId),
...
export function jwStoneReceivingCents(value: string): number | null {
  ...
  if (!/^\d{1,7}(?:\.\d{1,2})?$/.test(text))
    throw new JwStoneReceivingInputError(
      "Prices must be dollar amounts with no more than two decimal places."
    );
...
export function isManuallyAssignedJwStoneEmployee(
  userId: string,
...
export function jwStoneReceivingPhotoKey(receiptId: string, fileName: string): string | null {
  if (
    ...
    !/^[1-8]-[a-f0-9]{20}\.jpg$/.test(fileName)
  )
    return null;
