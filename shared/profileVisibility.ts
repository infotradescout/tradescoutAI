...
/**
 * Authorized profile provisioners may release their new seed. Existing rows
 * keep their database-owned status and release decision: omitting both fields
 * also preserves a revocation committed after the provisioner's earlier read.
 * Legacy preferences never supply release authority here.
 */
export function profileReleaseSeedFields(args: {
  existingProfile: object | null | undefined;
  releaseNewProfile: boolean;
}): { status?: "published"; publiclyReleased?: boolean } {
  return args.existingProfile
    ? {}
    : { status: "published", publiclyReleased: args.releaseNewProfile === true };
}
