...
export const JW_STONE_ADD_ONS = [
  "builders", "bidrock", "member_pricing", "cart", "bundles", "offers",
  "inventory_tools", "sales_automation",
] as const;
export type JwStoneAddOn = typeof JW_STONE_ADD_ONS[number];
export const JW_STONE_BASE = Object.freeze({
  website: true, catalog: true, contact: true, directConnect: true,
  accounts: true, requestHistory: true,
});
export type JwStoneFeatureState = {
  profileSlug: "jw-stone"; enabled: boolean; revision: number; configured: boolean;
};
export type JwStoneFeatureCommand = {
  enabled: boolean; expectedRevision: number; operationId: string;
  preserveBaseServices: true; note: string;
};
...
export function parseJwStoneFeatureCommand(value: unknown): JwStoneFeatureCommand {
  ...
  if (Object.keys(row).some(key => !keys.includes(key)) || typeof row.enabled !== "boolean" ||
      !Number.isSafeInteger(row.expectedRevision) || Number(row.expectedRevision) < 0 ||
      typeof row.operationId !== "string" ||
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(row.operationId) ||
      row.preserveBaseServices !== true || typeof row.note !== "string" ||
      row.note.trim().length < 3 || row.note.trim().length > 500) {
    throw new JwStoneFeatureError(400, "INVALID_FEATURE_CHANGE", "Enter a valid change, revision, and private reason; base services must stay available.");
  }
  return { enabled: row.enabled, expectedRevision: Number(row.expectedRevision),
    operationId: row.operationId.toLowerCase(), preserveBaseServices: true, note: row.note.trim() };
}
export function projectJwStoneFeatures(state: JwStoneFeatureState) {
  return { profileSlug: state.profileSlug, enabled: state.enabled, revision: state.revision,
    configured: state.configured, base: JW_STONE_BASE,
    features: Object.fromEntries(JW_STONE_ADD_ONS.map(key => [key, state.enabled])) as Record<JwStoneAddOn, boolean> };
}
export function unavailableJwStoneFeatures() {
  return projectJwStoneFeatures({ profileSlug: "jw-stone", enabled: false, revision: 0, configured: false });
}
/** A manifest grants feature availability only; authentication and membership still apply. */
export function parseJwStoneFeatureManifest(value: unknown): ReturnType<typeof projectJwStoneFeatures> {
  const row = value as Record<string, unknown> | null;
  if (!row || row.profileSlug !== "jw-stone" || typeof row.enabled !== "boolean" ||
      typeof row.configured !== "boolean" || !Number.isSafeInteger(row.revision) || Number(row.revision) < 0) {
    throw new Error("Invalid JW Stone feature state");
  }
  return projectJwStoneFeatures({ profileSlug: "jw-stone", enabled: row.enabled,
    configured: row.configured, revision: Number(row.revision) });
}
export type FeatureRequest = { path: string; method: string; body?: unknown; bidRockProfileSlug?: string };
...
export function classifyJwStoneFeatureRequest(request: FeatureRequest): JwStoneAddOn | null {
  ...
  const path = request.path.split("?")[0].split("/").map(segment => {
    try { return decodeURIComponent(segment).trim().toLowerCase(); }
    catch { return segment.toLowerCase(); } // Malformed parameters are rejected by the router.
  }).join("/").replace(/\/+$/, "");
  ...
  if (path === "/api/tradepartner-profiles/jw-stone/express-request") {
    return body.requestType === "make_offer" || Object.prototype.hasOwnProperty.call(body, "stoneOffer") ? "offers" : null;
  }
  ...
  if (request.bidRockProfileSlug === "jw-stone" && /^\/api\/(?:admin\/)?bidrock(?:\/|$)/.test(path)) {
    ...
    if (method === "POST" && /^\/api\/bidrock\/orders\/[^/]+\/(?:cancel|handoffs)$/.test(path)) return null;
    if (method === "POST" && /^\/api\/admin\/bidrock\/orders\/[^/]+\/(?:payment-settled|complete)$/.test(path)) return null;
    if (method === "PATCH" && /^\/api\/admin\/bidrock\/orders\/[^/]+\/system-links$/.test(path)) return null;
    return "bidrock";
  }
  return null;
}
