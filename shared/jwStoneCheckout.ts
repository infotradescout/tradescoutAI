...
export const jwStoneSaleLineSchema = z.object({
  ...
  unitRateCents: cents.positive(),
  lineTotalCents: cents.positive(),
...
export const jwStoneFinalQuoteSchema = z.object({
  ...
  decision: z.enum(["accept_offer", "counter_offer"]),
...
export const jwStoneSaleCommandSchema = z.discriminatedUnion("action", [
  z.object({ ...commandBase, action: z.literal("quote"), decision: z.enum(["accept_offer", "counter_offer"]), materialCents: cents.positive(), taxCents: cents, deliveryCents: cents, expiresAt: z.string().datetime(), notes: z.string().trim().max(2000) }).strict(),
...
export type JwStoneCheckoutAttempt = {
  ...
  url: string | null;
  outcome: JwStonePaymentOutcome | "creating";
};
export type JwStoneSaleState = {
  ...
  quote: JwStoneFinalQuote | null;
  attempt: JwStoneCheckoutAttempt | null;
