...
export function canonicalImportJson(value: unknown): string {
  ...
  if (isObject(value)) {
    return `{${Object.keys(value)
      ...
      .map((key) => `${JSON.stringify(key)}:${canonicalImportJson(value[key])}`)
      .join(",")}}`;
  }
  ...
  const value = {
    ...
    payload: {
      ...
      specifications: {
        ...
        externalListingId: normalizeImportKey(row.key),
      },
    },
    photoHashes,
  };
...
export function indexOwnImports(value: unknown, sellerId: string): Map<string, ExistingImport> {
  if (!sellerId || !Array.isArray(value))
    throw new Error("Could not check your existing listings. No new listings were submitted.");
  ...
  for (const listing of value) {
    if (
      ...
      throw new Error(
        "The listing response did not match the signed-in seller. Refresh before continuing."
      );
    }
    ...
    if (typeof keyValue !== "string" || !/^[a-z0-9][a-z0-9_-]{0,79}$/i.test(keyValue.trim()))
      continue;
    const key = normalizeImportKey(keyValue);
    if (imports.has(key))
      throw new Error(
        `More than one saved listing uses ${key}. Resolve that duplicate before importing.`
      );
 ...
 */
export async function runRecoverableExchangeBatch(
  options: ImportDependencies
): Promise<ImportResult[]> {
  if (
    !options.sellerId ||
    !options.rows.length ||
    options.rows.length > 100 ||
    options.rows.some((row) => row.errors.length)
  ) {
  ...
  if (new Set(keys).size !== keys.length)
    throw new Error("Duplicate listing IDs must be fixed before importing.");
  ...
  const report = (result: ImportResult) => {
    results.push(result);
  ...
  for (const row of options.rows) {
    ...
    fingerprints.set(
      normalizeImportKey(row.key),
      await fingerprintImportRow(
        row,
        row.photoIndexes.map((index) => hashes.get(index)!)
      )
    );
  }
  let hasConflict = false;
  for (const row of options.rows) {
    const key = normalizeImportKey(row.key),
  ...
  for (const row of options.rows) {
    ...
    try {
      for (const index of row.photoIndexes) {
        ...
        if (!url) {
          ...
          if (typeof url !== "string" || !url.trim())
            throw new Error("An image upload did not return a file URL.");
      ...
      report({
        ...
        message: `${error instanceof Error ? error.message : "Photo upload failed."} This row was not submitted.`,
      });
      break;
    }
    if (options.shouldStop()) break;
    ...
    try {
      ...
      if (!record || record.fingerprint !== fingerprint)
        throw new Error("The listing creation response could not be confirmed.");
      existing.set(key, record);
      report({
        key: row.key,
        status: "submitted",
        listingId: record.id,
        message: "Listing submitted. Normal review and visibility rules apply.",
      });
    } catch (error) {
        ...
        if (record) {
          ...
          if (result.status === "skipped") {
            report({
              ...
              status: "recovered",
              message:
                "Confirmed in your saved listings after an interrupted response. No duplicate was submitted.",
            });
            continue;
          }
          report(result);
      ...
      report({
        key: row.key,
        status: "needs_check",
        message: `${error instanceof Error ? error.message : "Submission was interrupted."} No saved listing could be confirmed. Import paused; check the connection or account requirements, then resume.`,
      });
      break;
    }
  }
