...
export function getJwStoneBundleProgress(lines: readonly JwStoneBundleCandidate[]) {
  ...
  return {
    ...
    unlocked: lines.length > 0 && lines.every((line) => line.status === "ready") && eligibleSlabs >= JW_STONE_BUNDLE_SLABS,
  };
}
...
export function priceJwStoneBundleLine(price: JwStoneBundlePricing, quantity: number, unlocked: boolean) {
  const cents = [price.slabRateCents, price.bundleRateCents, price.regularOneSlabCents, price.bundleOneSlabCents];
  if (!cents.every((value) => Number.isSafeInteger(value) && value > 0) ||
      !Number.isInteger(price.minimumSlabs) || price.minimumSlabs < 2 || price.minimumSlabs > 999 ||
      !Number.isInteger(quantity) || quantity < 1 || quantity > 999) throw new Error("Invalid bundle pricing inputs");
  const discounted = price.bundleRateCents < price.slabRateCents &&
  ...
  if (!Number.isSafeInteger(lineTotalCents) || !Number.isSafeInteger(price.regularOneSlabCents * quantity))
    throw new Error("Cart amount exceeds supported precision");
  return { pricingTier: discounted ? "bundle" as const : "slab" as const,
    unitRateCents: discounted ? price.bundleRateCents : price.slabRateCents, oneSlabTotalCents, lineTotalCents };
}
