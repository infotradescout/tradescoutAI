...
export const HOME_IDENTITY_TYPES = [
  ...
  ["condo", "Condo"],
  ["duplex", "Duplex"],
  ["triplex_fourplex", "Triplex or fourplex"],
  ["multi_family", "Multifamily"],
  ["manufactured_home", "Manufactured home"],
  ["mobile_home", "Mobile home"],
  ["new_build", "New build"],
  ["land_lot", "Land or lot"],
  ["commercial_residential_mixed", "Mixed-use property"],
...
export const HOME_IDENTITY_LABELS = {
  ...
  address2: "Unit or address line 2",
  city: "City",
  stateCode: "State",
  countyFips: "County or parish",
  zipCode: "ZIP code",
} as const;
...
export const HOME_LOCATION_FIELDS = [
  "address1",
  "address2",
  "city",
  "stateCode",
...
const identityFields = {
  ...
  stateCode: z
    ...
    .trim()
    .toUpperCase()
    .regex(/^[A-Z]{2}$/, "Choose a state.")
    .nullable(),
  countyFips: z
    .string()
    .regex(/^\d{5}$/, "Choose a county or parish.")
    .nullable(),
  zipCode: z
    .string()
    .trim()
    .regex(/^\d{5}(?:-\d{4})?$/, "Use a five-digit ZIP or ZIP+4.")
    .nullable(),
};
...
export const homeIdentityChangesSchema = z
  ...
  .refine(
    (value) =>
      ...
      HOME_IDENTITY_TYPES.some(([type]) => type === value.propertyType),
    {
      message: "Choose a supported property type.",
      path: ["propertyType"],
    }
  );
export const homeIdentityUpdateSchema = z
  .object({
...
export function homeIdentityDraft(identity: HomeIdentity): HomeIdentityDraft {
  return Object.fromEntries(
    HOME_IDENTITY_FIELDS.map((field) => [
      field,
      identity[field] == null ? "" : String(identity[field]),
    ])
  ) as HomeIdentityDraft;
}

/** Only changed fields are sent. Blank values deliberately clear an existing value. */
export function homeIdentityChanges(
  identity: HomeIdentity,
  draft: HomeIdentityDraft
): HomeIdentityUpdate["changes"] {
