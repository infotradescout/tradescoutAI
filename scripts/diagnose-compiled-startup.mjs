...
async function boot(filename,environment,tag){
 ...
 if(child.exitCode===null&&!child.signalCode){child.kill('SIGTERM');await Promise.race([new Promise(resolve=>child.once('exit',resolve)),new Promise(resolve=>setTimeout(()=>{child.kill('SIGKILL');resolve();},3000))]);}
 child=undefined;await file.close();
 const log=(await fs.readFile(logPath,'utf8')).replace(/postgres(?:ql)?:\/\/[^\s"']+/g,'[LOCAL_TLS_DATABASE]');
 const entry={tag,exitCode,signal,health,tail:log.slice(-18000)}; proof.checks.push(entry);
...
try{
 ...
 proof.bundleHeader=bundle.slice(0,6500);
 proof.topLevelAwaits=bundle.split('\n').map((line,index)=>({line:index+1,text:line})).filter(row=>/^await |^var .*await |^const .*await |^  await init_/.test(row.text)).slice(-150);
 console.log('STARTUP_BUNDLE_STRUCTURE '+JSON.stringify({header:proof.bundleHeader,awaits:proof.topLevelAwaits}));
 ...
 if(!actual.health){
   // Diagnostics only: instrument generated initializer wrappers outside tracked source.
   const pattern=/var __esm = \(fn, res\) => function __init\(\) \{[\s\S]*?\n\};/;
   const match=bundle.match(pattern);proof.initializerPatternMatched=Boolean(match);
   if(match){
     const replacement='var __esm = (fn, res) => function __init() { const name = fn && Object.keys(fn)[0]; if (!fn) return res; const work=fn; fn=0; console.error("INIT_START " + name); res=(0,work[Object.keys(work)[0]])(); if(res && typeof res.then === "function")res.then(()=>console.error("INIT_END "+name),error=>console.error("INIT_ERROR "+name+" "+error)); else console.error("INIT_END "+name); return res; };';
     await fs.writeFile('dist/index.diagnostic.js',bundle.replace(pattern,replacement));
     await boot('dist/index.diagnostic.js',environment,'instrumented-diagnostic-only');
   }
 }
...
finally{child?.kill('SIGKILL');await database?.stop();proof.finishedAt=new Date().toISOString();await fs.mkdir(output,{recursive:true});await fs.writeFile(path.join(output,'evidence.json'),JSON.stringify(proof,null,2));await fs.writeFile(path.join(output,'index.html'),'<meta name="robots" content="noindex,nofollow"><h1>Compiled startup diagnostic</h1><a href="evidence.json">Evidence</a>');await fs.rm(temp,{recursive:true,force:true});console.log('STARTUP_RESULT '+JSON.stringify(proof));}
