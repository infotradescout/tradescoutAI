...
async function page(url,agent,depth=0){
 const key=agent+new URL(url,base).href;if(visited.has(key)||depth>4)return;visited.add(key);
 try{
...
try{
 ...
 report.relevantFiles=tracked.filter(f=>/issa|search.console|indexnow|sitemap|project.index|selective.*(readme|skill|config)|agents\.md/i.test(f)).slice(0,140);
 ...
 const robot=await read('/robots.txt');report.robots={status:robot.status,headers:robot.headers,text:robot.body};
 console.log('ISSA_SEO_ROBOTS '+JSON.stringify({status:robot.status,disallow:[...robot.body.matchAll(/^Disallow:.*$/gm)].map(m=>m[0]).filter((s,i,a)=>a.indexOf(s)===i)}));
 const pages=['/u/issa-build','/business/issa-build','/contractors/issa-build','/u/honey-onyx','/issa-build/sitemap.xml','/api/health','/issa-build/onyx/inventory/honey-onyx','/pensacola'];
