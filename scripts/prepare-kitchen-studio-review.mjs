...
try{
 ...
 for(const [name,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
  ...
  await page.getByLabel('Cabinet door style',{exact:true}).selectOption('Shaker');
  await page.getByLabel('Cabinet finish',{exact:true}).selectOption('sage');
  await page.getByLabel('Cabinet hardware',{exact:true}).selectOption('Brushed brass');
  await page.getByLabel('Selected cabinet fronts',{exact:true}).selectOption('drawers');
  ...
  await page.getByRole('button',{name:'Undo',exact:true}).click();saved=await read();if(saved.cabinets.planner.modules.length!==1)throw Error('Undo did not restore one module');
  await page.getByRole('button',{name:'Redo',exact:true}).click();
  await page.getByTestId('steel-home-cabinet-view-3d').click();
  ...
  await page.screenshot({path:path.join(out,'proof','cabinets-'+name+'.png')});
  ...
  await page.getByRole('button',{name:'Load sample kitchen',exact:true}).click();
  await page.getByRole('button',{name:'Open Countertops',exact:true}).click();
  await page.getByRole('button',{name:'Scaled drawing',exact:true}).click();
  ...
  const downloadPromise=page.waitForEvent('download');await page.getByRole('button',{name:'Export drawing',exact:true}).click();const download=await downloadPromise;
  if(!download.suggestedFilename().endsWith('.svg')||await download.failure())throw Error('SVG export failed');
  await page.screenshot({path:path.join(out,'proof','countertops-'+name+'.png')});
