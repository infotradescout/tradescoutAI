...
try {
  head = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(); report.head = head;
  ...
  const cleanup = async (name, action) => {
    try { await action(); } catch (error) {
      report.passed = false;
      (report.cleanupErrors ??= []).push({ name, error: scrub(error.stack || error) });
    }
  };
  try {
    await cleanup('browser', () => browser?.close());
    await cleanup('fixture server', stopFixtureServer);
    await cleanup('private log', () => serverLog?.close());
    ...
    if (privateOutput) await cleanup('private fixture removal', () => fs.rm(privateOutput, { recursive: true, force: true }));
    report.finishedAt = new Date().toISOString();
    await fs.mkdir(output, { recursive: true });
    await fs.writeFile(path.join(output, 'report.json'), JSON.stringify(report, null, 2));
  } catch (error) {
    report.passed = false;
    report.finalizationError = scrub(error.stack || error);
    try {
      await fs.mkdir(output, { recursive: true });
      await fs.writeFile(path.join(output, 'report.json'), JSON.stringify(report, null, 2));
    } catch {}
  } finally {
    try { console.log(JSON.stringify(report, null, 2)); } catch (error) {
      report.passed = false; report.finalizationError = scrub(error.stack || error);
      try { await fs.writeFile(path.join(output, 'report.json'), JSON.stringify(report, null, 2)); } catch {}
    }
