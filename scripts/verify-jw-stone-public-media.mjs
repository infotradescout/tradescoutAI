...
const manifestPath = path.join(repoRoot, "scripts/data/jw-stone-public-media-manifest.json");
...
const bundledMediaRoot = path.join(repoRoot, "client/public/images/businesses/jw-stone");
...
const literalMediaPath =
  /\/images\/businesses\/jw-stone\/[A-Za-z0-9_./-]+\.(?:avif|bmp|gif|ico|jpe?g|mp4|png|svg|webp)/g;
...
if (bundledFiles.length > 0) {
  throw new Error(
    `JW Stone media returned to the client build (${bundledFiles.length} files). Upload it through the public-media migration instead.`
  );
}
...
if (missingManifestPaths.size > 0) {
  throw new Error(
    `Public JW Stone media references are missing from the migration manifest:\n${[
