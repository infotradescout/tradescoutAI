...
export async function prepareSupplierSubtotalFix() {
  ...
  assert(baseline.failed > 0 && baseline.pending === 0,'Baseline must reproduce the arithmetic defect, not skip it');
  ...
  const report = { before,prepared,branch,files:[file],patch,baseline,corrected,pushed:false,verified:false,productionChanged:false,transportStage:'not_started' };
  try {
    // Reuse only the configured checkout transport. No credential search,
    // extraction, logging, copying or alternate-account authorization.
    report.transportStage='local_origin';
    const rawOrigin=git(['remote','get-url','origin']);
    const localOrigin=rawOrigin.startsWith('file:')?fileURLToPath(rawOrigin):rawOrigin;
    assert(path.isAbsolute(localOrigin));
    report.transportStage='configured_upstream';
    ...
    if(!report.pushed)report.pushError='The configured transport did not authorize this ordinary feature-branch push.';
  }catch(error){report.pushError='Configured repository transport unavailable at '+report.transportStage;report.transportErrorType=error.name;}
