...
try {
  ...
  report(seo.title ? 'ok' : 'warn', 'SEO title set', seo.title ? undefined : 'falls back to an auto-generated title');
  report(
  ...
  if (seo.imageUrl) {
    ...
    report(check.ok ? 'ok' : 'fail', 'Share image (OG banner) reachable', `${seo.imageUrl} -> ${check.status ?? check.error}`);
  } else {
  ...
  if (seo.faviconUrl) {
    ...
    report(check.ok ? 'ok' : 'fail', 'Favicon reachable', `${seo.faviconUrl} -> ${check.status ?? check.error}`);
  } else if (seo.imageUrl) {
    report('warn', 'No favicon set', 'browser tab will fall back to the wide share image, not a square crop');
  }
  ...
  if (seo.customDomain) {
    ...
    report(domainCheck.ok ? 'ok' : 'fail', `https://${domain}/ reachable`, `-> ${domainCheck.status ?? domainCheck.error}`);
    ...
    const redirectCheck = await fetch(`https://www.thetradescout.com/u/${encodeURIComponent(slug)}`, {
      redirect: 'manual',
    ...
    const redirectsCorrectly =
      redirectCheck.status === 301 && location && location.replace(/\/$/, '') === `https://${domain}`;
    report(
      redirectsCorrectly ? 'ok' : 'fail',
      `/u/${slug} redirects to the custom domain`,
      redirectsCorrectly ? undefined : `got status=${redirectCheck.status} location=${location ?? 'none'}`
    );
    ...
    report(
      ...
      apexCheck.ok ? undefined : `${apexCheck.status ?? apexCheck.error} -- check DNS/certificate if this is meant to work`
    );
  } else {
    report('warn', 'No custom domain configured', `profile is only reachable at /u/${slug}`);
  }

  if (profile.business_id) {
    const { rows: bizRows } = await client.query(
      `SELECT status, public_discovery_enabled FROM businesses WHERE id = $1`,
      [profile.business_id]
    );
    const business = bizRows[0];
    if (business) {
      report(business.status === 'active' ? 'ok' : 'warn', `Linked business status is "${business.status}"`);
      report(
        business.public_discovery_enabled ? 'ok' : 'warn',
        'Public discovery enabled on linked business',
        business.public_discovery_enabled ? undefined : 'business won\'t surface in directory/search'
      );
    }
  }
