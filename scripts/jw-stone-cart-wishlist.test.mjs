...
test("fractional or corrupt inventory is not rounded up", () => { assert.equal(review({ quantity: 2.9, heldQuantity: 1 }).lines[0].status, "insufficient_quantity"); for (const n of [-1, NaN, Infinity]) assert.equal(review({ heldQuantity: n }).lines[0].status, "unavailable"); });
...
test("missing price does not create a partial subtotal", () => { const r = cart.reviewJwStoneCart("a", [...intent, { inventoryPublicId: SECOND, quantity: 1 }], [stock, { ...stock, publicId: SECOND, rate: null }]); assert.equal(r.lines[0].status, "ready"); assert.equal(r.subtotalCents, null); assert.equal(r.readyForRequest, false); });
...
test("containers, pieces and non-slab units cannot be priced as slab counts", () => { for (const patch of [{ assetKind: "container" }, { assetKind: "piece" }, { unit: "square_feet" }]) assert.equal(review(patch).lines[0].status, "unsupported_unit"); });
...
test("review is not a payment or reservation promise", () => { const r = review(); assert.equal(r.readyForRequest, true); assert.equal(r.readyForCheckout, false); assert.equal(r.reservationCreated, false); });
...
test("duplicate lot IDs cannot bypass a stock limit", () => assert.throws(() => cart.parseJwStoneCartRequest({ lines: [...intent, ...intent] }), /only once/));
test("invalid input shape and injected prices are rejected", () => { for (const body of [null, {}, { lines: [] }, { lines: intent, price: 1 }, { lines: [{ ...intent[0], sellPriceCents: 1 }] }, { lines: [{ inventoryPublicId: "catalog", quantity: 1 }] }]) assert.throws(() => cart.parseJwStoneCartRequest(body)); });
test("requested counts must be integers from 1 to 999", () => { for (const quantity of [0, -1, 1.5, 1000, "2", null, NaN, Infinity]) assert.throws(() => cart.parseJwStoneCartRequest({ lines: [{ inventoryPublicId: ID, quantity }] })); });
test("stored intent strips stale estimates and private price fields", () => { const line = cart.normalizeJwStoneCart([{ id: ID, kind: "lot", stoneName: "Test", quantity: 2, landedCostCents: 1, slabRateCents: 2, minimumTotalCents: 3 }])[0]; assert.deepEqual(Object.keys(line).sort(), ["id", "kind", "quantity", "stoneName"]); });
test("legacy carts become catalog requests, not assumed inventory", () => { const lines = cart.migrateJwStoneLegacyCart([{ id: ID, stoneName: "Test", quantity: 1, slabRateCents: 1 }]); assert.equal(lines[0].kind, "catalog"); assert.equal(lines[0].slabRateCents, undefined); });
test("cart limits are visible errors instead of silently dropping a selection", () => { const lines = Array.from({ length: 50 }, (_, i) => ({ id: String(i), kind: "catalog", stoneName: "Test", quantity: 1 })); assert.throws(() => cart.addJwStoneCartLine(lines, { id: "new", kind: "catalog", stoneName: "New" }), /50/); });
test("add, increment, decrement and remove preserve exact lot identities", () => { let lines = cart.addJwStoneCartLine([], { id: ID, kind: "lot", stoneName: "Test" }); lines = cart.addJwStoneCartLine(lines, lines[0]); assert.equal(lines[0].quantity, 2); assert.equal(cart.setJwStoneCartQuantity(lines, ID, 1)[0].quantity, 1); assert.deepEqual(cart.setJwStoneCartQuantity(lines, ID, 0), []); });
...
const storeModule = await load("client/src/features/jw-stone/jwStoneCartStore.ts", { "../../../../shared/jwStoneCart": cart });
test("account carts remain isolated; initial restore does not overwrite", () => { const m = memory(), a = storeModule.createJwStoneCartStore(m, "a"), b = storeModule.createJwStoneCartStore(m, "b"); a.add({ id: ID, kind: "lot", stoneName: "A" }); assert.equal(b.getSnapshot().lines.length, 0); assert.equal(storeModule.createJwStoneCartStore(m, "a").getSnapshot().lines.length, 1); });
...
test("cart clearing cannot resurrect legacy estimates", () => { const m = memory(); m.setItem(storeModule.JW_STONE_LEGACY_CART_STORAGE_PREFIX + "a", JSON.stringify([{ id: "old", stoneName: "Old", quantity: 1, slabRateCents: 100 }])); const a = storeModule.createJwStoneCartStore(m, "a"); assert.equal(a.getSnapshot().lines.length, 1); a.clear(); assert.equal(storeModule.createJwStoneCartStore(m, "a").getSnapshot().lines.length, 0); });
test("sequential edits from two tabs rebase on latest stored state", () => { const m = memory(), a = storeModule.createJwStoneCartStore(m, "a"), b = storeModule.createJwStoneCartStore(m, "a"); a.add({ id: ID, kind: "lot", stoneName: "One" }); b.add({ id: SECOND, kind: "lot", stoneName: "Two" }); a.refresh(); assert.equal(a.getSnapshot().lines.length, 2); b.clear(); a.refresh(); assert.equal(a.getSnapshot().lines.length, 0); });
test("storage failures retain in-memory edits and report them as unpersisted", () => { const m = memory(); m.setItem = () => { throw new Error("quota"); }; const a = storeModule.createJwStoneCartStore(m, "a"); a.add({ id: ID, kind: "lot", stoneName: "One" }); a.refresh(); assert.equal(a.getSnapshot().lines.length, 1); assert.equal(a.getSnapshot().persisted, false); });
test("storage subscribers observe changes without React updater side effects", () => { const a = storeModule.createJwStoneCartStore(memory(), "a"); let calls = 0; const unsubscribe = a.subscribe(() => calls++); a.add({ id: ID, kind: "lot", stoneName: "One" }); unsubscribe(); a.clear(); assert.equal(calls, 1); });
...
async function routeHarness(role = "member") {
  ...
  const routes = await load("server/routes/jw-stone-member-pricing.ts", {
    ...
    "../services/jwStoneCartService": { getJwStoneCartReview: async (viewerId, lines) => { reviews++; return cart.reviewJwStoneCart(viewerId, lines, [stock]); } },
  });
...
test("authenticated duplicate-ID requests fail before the inventory query", async () => { const h = await routeHarness(); const res = await h.call("POST", "cart-review", { id: "a" }, { lines: [...intent, ...intent] }); assert.equal(res.statusCode, 400); assert.equal(h.reviews, 0); });
...
async function serviceHarness({ received = true, published = true, workbookOffline = false, validReceipt = true, contextBusiness = "jw-business", expired = false } = {}) {
  ...
  const service = await load("server/services/jwStoneCartService.ts", {
    ...
    "./jwStoneDrivePricing": { getJwStonePricingSnapshot: async () => { workbookReads++; if (workbookOffline) throw new Error("offline"); return { prices: [{ stoneKey: "test-granite", slabPriceCents: 1250, bundlePriceCents: 1000, bundleMinSlabs: 6, landedCostCents: 100 }] }; } },
  ...
  return { run: () => service.getJwStoneCartReview("a", intent), get queries() { return queries; }, get workbookReads() { return workbookReads; } };
}
