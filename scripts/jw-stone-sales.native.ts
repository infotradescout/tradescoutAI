...
const {default:express}=await import("express");const {default:Stripe}=await import("stripe");const {chromium,expect}=await import("@playwright/test");
...
try{
  ...
  for(const user of [buyer,otherBuyer]){
    const businessProfile=randomUUID();await pool.query("INSERT INTO user_profiles(id,user_id,user_intent,verification_status,display_name) VALUES($1,$2,'business','pending','Synthetic order member')",[businessProfile,user]);
  ...
  const originalPriceSource=process.env.JW_STONE_PRICING_APPROVED_IMPORT,originalEvents=new Map<string,string>();
  async function offer(scope:"stone"|"cart",user=buyer,indexes=scope==="stone"?[0]:[0,1,2,3,4,5,6]){
    ...
    await db.insert(schema.workRequestEvents).values({workRequestId:request.id,type:"created",actorUserId:user,metadata});originalEvents.set(request.id,JSON.stringify(metadata));return {id:request.id,intake};
  }
  ...
  class FakeProvider implements JwStoneCheckoutProvider{
    sessions=new Map<string,{binding:JwStonePaymentBinding;session:JwStoneProviderSession}>();calls=0;created=0;loseNext=false;enabled=true;
    ...
    async create(binding:JwStonePaymentBinding){
      ...
      if(this.loseNext){this.loseNext=false;throw new Error("Synthetic provider response lost after session creation");}return {...saved.session};
    }
    async retrieve(binding:JwStonePaymentBinding){const saved=this.sessions.get(binding.attempt.id);assert(saved);assert.equal(saved.session.id,binding.attempt.sessionId);assert.equal(saved.binding.buyerId,binding.buyerId);return {...saved.session};}
    async webhookRequest(raw:Buffer,signature:string){const event=signingSdk.webhooks.constructEvent(raw,signature,webhookSecret);assert.equal(event.account,"acct_jwfixture");assert.equal(event.livemode,false);return (event.data.object as {metadata?:Record<string,string>}).metadata?.jwRequestId||null;}
  ...
  const main=await offer("cart");assert.equal(main.intake.lines.length,7);assert.equal(new Set(main.intake.lines.map(line=>line.materialName)).size,7);assert(main.intake.lines.every(line=>line.pricingTier==="bundle"));
  await reject(()=>sales.read(main.id,outsider),"jw_order_not_found");await reject(()=>sales.command(main.id,buyer,quoteCommand(main.intake)),"jw_seller_required");assert.equal(await held(),0);assert.equal(provider.created,0);
  note("actual seven-material offer review uses each source bundle rate and rejects unrelated readers/buyer quote authority");
  const firstQuote=quoteCommand(main.intake);await sales.command(main.id,owner,firstQuote);const first=await state(main.id);assert.equal(first.status,"quoted");assert(first.quote);
  ...
  await reject(()=>sales.command(main.id,buyer,payCommand(first)),"jw_order_changed");await reject(()=>sales.command(main.id,buyer,{...payCommand(quoted),quoteId:first.quote.id}),"jw_quote_changed");await reject(()=>sales.command(main.id,buyer,{...payCommand(quoted),totalCents:quoted.quote!.totalCents-1}),"jw_quote_changed");
  await assert.rejects(()=>sales.command(main.id,buyer,{...payCommand(quoted),acceptFinalQuote:false}));
  provider.enabled=false;await reject(()=>sales.command(main.id,buyer,payCommand(quoted)),"jw_payment_unavailable");provider.enabled=true;assert.equal(await held(),0);assert.equal(provider.created,0);
  ...
  // One pickup slab does not match this seven-slab delivery order. Matching holds
  // now transfer atomically; the separate purchase phase proves that positive path.
  await reject(()=>sales.command(main.id,buyer,payCommand(quoted)),"jw_cart_hold_mismatch");
  assert.equal(await held(),1);assert.equal(provider.created,0);assert.equal((await holds.get({buyerUserId:buyer,sellerBusinessId:seller.id,reservationId:temporary.reservationId})).status,"active");
  await holds.release({buyerUserId:buyer,sellerBusinessId:seller.id,reservationId:temporary.reservationId});
  provider.loseNext=true;const payment=payCommand(quoted);await assert.rejects(()=>sales.command(main.id,buyer,payment),/response lost/);assert.equal(await held(),7);assert.equal(provider.created,1);
  const uncertain=await state(main.id);assert.equal(uncertain.attempt?.outcome,"creating");await sales.command(main.id,buyer,payment);const opened=await state(main.id);assert(opened.attempt);assert.equal(opened.attempt.id,uncertain.attempt?.id);assert.equal(provider.created,1);assert.equal(await held(),7);
  await reject(()=>sales.command(main.id,buyer,payCommand(opened)),"jw_payment_in_progress");note("explicit payment allocates seven slabs once; a lost session response recovers the same attempt without duplicate sessions or allocations");
  provider.outcome(opened.attempt.id,"processing");await sales.reconcile(main.id,buyer);assert.equal((await state(main.id)).status,"processing");await holds.expireSeller(seller.id);assert.equal(await held(),7);
  const flags=createJwStoneFeatureStore(pool);await flags.change(owner,{enabled:false,expectedRevision:(await flags.read()).revision,operationId:randomUUID(),preserveBaseServices:true,note:"Synthetic order recovery test"});
  ...
  provider.outcome(opened.attempt.id,"paid");await sales.reconcile(main.id,buyer);assert.equal((await state(main.id)).status,"paid");assert.equal(await held(),7);await sales.reconcile(main.id,buyer);assert.equal(await held(),7);
  ...
  const paidRevision=(await state(main.id)).revision;await reject(()=>sales.command(main.id,owner,quoteCommand(main.intake,paidRevision)),"jw_payment_in_progress");
  ...
  for(const terminal of ["failed","expired"] as const){
    const single=await offer("stone");await sales.command(single.id,owner,quoteCommand(single.intake));await sales.command(single.id,buyer,payCommand(await state(single.id),"card"));const active=await state(single.id);assert(active.attempt);assert.equal(await held(),8);
  ...
  const raceA=await offer("stone",buyer,[7]),raceB=await offer("stone",otherBuyer,[7]);await sales.command(raceA.id,owner,quoteCommand(raceA.intake));await sales.command(raceB.id,owner,quoteCommand(raceB.intake));
  const[a,b]=await Promise.all([sales.read(raceA.id,buyer),sales.read(raceB.id,otherBuyer)]),outcomes=await Promise.allSettled([sales.command(raceA.id,buyer,payCommand(a.state)),sales.command(raceB.id,otherBuyer,payCommand(b.state))]);
  assert.equal(outcomes.filter(value=>value.status==="fulfilled").length,1);assert.equal(outcomes.filter(value=>value.status==="rejected").length,1);assert.equal(await held(),8);note("two independent buyers race for one final slab and only one payment attempt acquires it");
  await assert.rejects(()=>pool.query("UPDATE jw_stone_sale_events SET state=state WHERE request_id=$1",[main.id]),/append-only/);await assert.rejects(()=>pool.query("DELETE FROM jw_stone_sale_events WHERE request_id=$1",[main.id]),/append-only/);await assert.rejects(()=>pool.query("UPDATE jw_stone_sales SET buyer_user_id=$2,revision=revision+1 WHERE request_id=$1",[main.id,outsider]),/identity is immutable/);note("database rejects history mutation/deletion and order-owner replacement");
  const app=express();app.use(express.json({verify:preserveStripeWebhookRawBody}));app.use((req,_res,next)=>{const id=req.get("x-jw-fixture-actor");if(id&&[owner,buyer,otherBuyer,outsider].includes(id))req.user={id} as any;next();});
  ...
  server=await new Promise<Server>(resolve=>{const listener=app.listen(0,"127.0.0.1",()=>resolve(listener));});const address=server.address();assert(address&&typeof address==="object");const origin="http://127.0.0.1:"+address.port;
  ...
  assert.equal((await fetch(origin+"/api/u/jw-stone/orders")).status,401);assert.equal((await fetch(origin+"/api/u/jw-stone/orders/stripe/webhook",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"})).status,400);note("actual HTTP commands use the real same-origin guard and unsigned callbacks are rejected");
  ...
  for(const[device,viewport,method]of[["desktop",{width:1440,height:1000},"ach"],["touch",{width:390,height:844},"card"]] as const){
    const fresh=await offer(device==="desktop"?"cart":"stone"),contexts:BrowserContext[]=[],errors:string[]=[];
    const context=async(actor:string)=>{const context=await browser!.newContext({viewport,isMobile:device==="touch",hasTouch:device==="touch",extraHTTPHeaders:{"x-jw-fixture-actor":actor}});contexts.push(context);await context.route("**/*",route=>new URL(route.request().url()).origin===origin?route.continue():route.abort());const page=await context.newPage();page.setDefaultTimeout(30000);page.on("pageerror",error=>errors.push(error.message));return page;};
    const ownerPage=await context(owner);await ownerPage.goto(origin+"/jw-stone/orders?request="+fresh.id);await ownerPage.getByLabel("Confirmed tax ($) — enter 0 if applicable",{exact:true}).fill("5.00");await ownerPage.getByLabel("Confirmed delivery ($)",{exact:true}).fill("20.50");await ownerPage.getByLabel("Quote expires",{exact:true}).fill(new Date(Date.now()+86400000).toISOString().slice(0,16));await ownerPage.getByRole("button",{name:"Send final quote",exact:true}).click();await expect(ownerPage.getByRole("heading",{name:/Final quote/})).toBeVisible();
    const buyerPage=await context(buyer);await buyerPage.goto(origin+"/jw-stone/orders?request="+fresh.id);const pay=buyerPage.getByRole("button",{name:method==="ach"?"Pay by ACH bank debit":"Pay by card",exact:true});await pay.click();await expect(buyerPage.getByRole("alert")).toContainText("accept the final quote");await buyerPage.getByRole("checkbox").check();await pay.click();
    await expect(buyerPage.getByRole("link",{name:"Continue secure checkout"})).toHaveAttribute("href",/^https:\/\/checkout\.stripe\.com\//);const active=await state(fresh.id);assert(active.attempt);assert.equal(active.attempt.method,method);
    provider.outcome(active.attempt.id,"processing");await buyerPage.getByRole("button",{name:"Refresh order/payment status",exact:true}).click();await expect(buyerPage.getByText("Payment is processing. This is not a paid receipt.",{exact:true})).toBeVisible();provider.outcome(active.attempt.id,"paid");
    const payload=JSON.stringify({id:"evt_"+randomUUID(),type:"checkout.session.async_payment_succeeded",account:"acct_jwfixture",livemode:false,data:{object:{metadata:{jwRequestId:fresh.id},amount_total:1}}});const signature=signingSdk.webhooks.generateTestHeaderString({payload,secret:webhookSecret});const callback=await fetch(origin+"/api/u/jw-stone/orders/stripe/webhook",{method:"POST",headers:{"Content-Type":"application/json","stripe-signature":signature},body:payload});assert.equal(callback.status,200);
    await buyerPage.reload();await expect(buyerPage.getByText(/Payment confirmed\. JW Stone is holding/)).toBeVisible();assert.equal((await state(fresh.id)).status,"paid");assert.equal(await buyerPage.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false);
    await fs.mkdir(output,{recursive:true});await buyerPage.screenshot({path:path.join(output,device+"-order-status.png"),fullPage:true});assert.deepEqual(errors,[]);
    proof.devices.push({device,offerScope:fresh.intake.scope,method,realOrderPage:true,sellerQuotePublished:true,explicitBuyerConsent:true,hostedCheckoutLink:true,pendingNotPaid:true,signedSyntheticCallback:true,callbackAmountNotTrusted:true,paidStatusRecoveredOnReload:true,horizontalOverflow:false,providerNetworkUsed:false,authentication:"isolated fixture"});console.log("JW_SALES_BROWSER_DEVICE "+JSON.stringify(proof.devices.at(-1)));for(const context of contexts)await context.close();
  }
  for(const[requestId,original]of originalEvents){const stored=(await pool.query("SELECT metadata FROM work_request_events WHERE work_request_id=$1 AND type='created'",[requestId])).rows[0].metadata;assert.deepEqual(stored,JSON.parse(original));}
  assert.equal(process.env.JW_STONE_PRICING_APPROVED_IMPORT,originalPriceSource);assert.equal(Number((await pool.query("SELECT count(*) AS n FROM marketplace_transactions")).rows[0].n),0);assert.equal((await flags.read()).enabled,true);note("complete original pending-offer records, source prices, unrelated marketplace ledger and final ON state are preserved");proof.passed=true;
...
finally{await browser?.close();if(server)await new Promise<void>(resolve=>server!.close(()=>resolve()));await pool.end();proof.finishedAt=new Date().toISOString();await fs.mkdir(output,{recursive:true});await fs.writeFile(path.join(output,"native-evidence.json"),JSON.stringify(proof,null,2)+"\n");console.log("JW_SALES_NATIVE_RESULT "+JSON.stringify(proof));}
assert.equal(proof.passed,true,"Native order acceptance failed");
