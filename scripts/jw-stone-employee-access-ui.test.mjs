...
async function entry(access, user = { id: "owner" }) {
  ...
  const deps = {
    ...
    "./JwStoneEmployeeToolsLoader": { JwStoneEmployeeToolsLoader: loader },
  };
  ...
  return { result: module.namespace.JwStoneEmployeeReceiving(), loader };
}
test("approved employee entry mounts only the viewer-keyed lazy loader without staff authority", async () => {
  const h = await entry({ data: { viewerId: "owner", allowed: true, enabled: true, canManageStaff: false } });
  assert.equal(h.result.type, h.loader);
  assert.equal(h.result.key, "owner");
  assert.deepEqual(JSON.parse(JSON.stringify(h.result.props)), { viewerId: "owner", enabled: true, canManageStaff: false });
});
test("owner entry exposes management even while receiving writes await activation", async () => {
  ...
  assert.equal(h.result.type, h.loader);
  assert.equal(h.result.props.enabled, false);
  assert.equal(h.result.props.canManageStaff, true);
});
test("anonymous, unauthorized, loading, and wrong-viewer entries never mount the lazy employee loader", async () => {
  ...
  assert.equal((await entry({ isLoading: true })).result, null);
});
test("revocation hides the entry but a transient refresh error does not destroy an employee draft", async () => {
  ...
  for (const status of [401, 403]) assert.equal((await entry({ data, error: new ApiError("Denied", status) })).result, null);
  assert.ok((await entry({ data, error: new ApiError("Temporary failure", 503) })).result);
});
test("employee account changes give React a different loader key and no inherited staff authority", async () => {
  const owner = await entry({ data: { viewerId: "owner", allowed: true, enabled: true, canManageStaff: true } });
  const staff = await entry({ data: { viewerId: "staff-one", allowed: true, enabled: true } }, { id: "staff-one" });
  assert.equal(owner.result.key, "owner");
  assert.equal(staff.result.key, "staff-one");
  assert.equal(staff.result.props.canManageStaff, false);
});
