...
VALUES
  ('permit_inspection', 3, 6, 'B'),
  ('home_inspection', 4, 8, 'B'),
  ('insurance_claim', 4, 8, 'B'),
  ('pre_sale', 3, 6, 'B'),
  ('item_valuation', 2, 5, 'B'),
  ('equipment_condition', 3, 6, 'B')
ON CONFLICT (mode) DO UPDATE SET
  min_photos = EXCLUDED.min_photos,
  max_billable_photos = EXCLUDED.max_billable_photos,
  target_confidence_tier = EXCLUDED.target_confidence_tier,
  updated_at = NOW();
